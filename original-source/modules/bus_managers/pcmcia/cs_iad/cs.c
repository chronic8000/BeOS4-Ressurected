...
static void unreset_socket(u_long i)
{
	...
	if (s->state & SOCKET_SUSPEND) {
	    ...
	    s->state &= ~SOCKET_SETUP_PENDING;  /* bjs */
...
static int cs_request_irq(client_handle_t handle, irq_req_t *req)
{
...
#ifdef __BEOS__
	{
		...
		int i,j;
		void *handle;
		u_int add_mask = 0, del_mask = 0;
		if(handle = load_driver_settings("pcmcia")){
			settings = get_driver_settings(handle);
			for(i=0;i<settings->parameter_count;i++){
				p = settings->parameters + i;
#if 0
				if(!strcmp(p->name,"enable_irq")){
					for(j=0;j<p->value_count;j++){
						add_mask |= 1<<strtoul(p->values[j],NULL,0);
					}
				}
#endif
				if(!strcmp(p->name,"disable_irq")){
					for(j=0;j<p->value_count;j++){
						del_mask |= 1<<strtoul(p->values[j],NULL,0);
					}
				}
			}
			unload_driver_settings(handle);
			s->cap.irq_mask |= add_mask;
			s->cap.irq_mask &= (~del_mask);
		}
	}
#endif		
...
static status_t std_ops(int32 op, ...)
{
	...
	case B_MODULE_INIT:
		...
		DEBUG(0, ("%s\n", version));
		init_timer();
		if (get_module(B_ISA_MODULE_NAME, (mod_t **)&isa) != B_OK) goto fail1;
		if (get_module(B_PCI_MODULE_NAME, (mod_t **)&pci) != B_OK) goto fail2;
		if (get_module(B_CONFIG_MANAGER_FOR_BUS_MODULE_NAME, (mod_t **)&cm) != B_OK) goto fail3;
		if (get_module(SS_MODULE_NAME("i82365"), &i82365) != B_OK) goto fail4;
		return B_OK;
		
	case B_MODULE_UNINIT:
		printk(KERN_INFO "Refusing to unload PCMCIA Card Services\n");
		return B_ERROR;
	}
...
static cs_client_module_info cs_client_info = {
    { { CS_CLIENT_MODULE_NAME, 0, &std_ops }, NULL },
...
static cs_socket_module_info cs_socket_info = {
    { { CS_SOCKET_MODULE_NAME, 0, &no_ops }, NULL },
    &register_ss_entry,
    &unregister_ss_entry,
    &add_timer,
    &del_timer,
