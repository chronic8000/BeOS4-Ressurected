...
void smb_encrypt(const unsigned char *Password, unsigned char *Challenge, char *Response)
{
	...
	{
		dprintf("Pass from uspace is %s\n",Password);
	}
	
	// Should we upper case here?
	if ((len = strlen(Password)) <= 14) {
		memcpy(p14, Password, len);
	} else {
		memcpy(p14, Password, 14);
	}	
	p14[14] = '\0';
	{ unsigned char *p = p14; while (*p) { *p = toupper(*p); p++; }};

	E_P16(p14,p21);
	E_P24(p21,Challenge,(unsigned char *)Response);
}
