...
InitNode(vnode* rec, char* buf, int* bytesRead, uint8 joliet_level)
{
	...
	//dprintf("InitNode - ENTER, vnode %Ld, bufstart is %lu, record length is %d bytes\n", rec->id, buf, recLen);
	...
	if (recLen > 0) {
		...
		if (rec->fileIDLen > 0) {
			...
			// these in iso, joliet or otherwise
			if ((rec->fileIDLen == 1) && (buf[0] == 0)) {
				rec->fileIDString = strdup(".");
				rec->fileIDLen = 1;
			} else if((rec->fileIDLen == 1) && (buf[0] == 1)) {
				rec->fileIDString = strdup("..");
				rec->fileIDLen = 2;
			} else	if (joliet_level > 0) {
				// JOLIET extension:
				...
				if(rec->fileIDString == NULL) {
					// Error
					dprintf("iso9660: InitNode - unable to allocate memory!\n");
					return ENOMEM;
				}
					
				srcLen = rec->fileIDLen;
				dstLen = rec->fileIDLen * 2;
	
				err = unicode_to_utf8(buf, &srcLen, rec->fileIDString, &dstLen);
				if(err < B_NO_ERROR) {
					dprintf("iso9660: InitNode - error converting unicode->utf8\n");
...
ConvertRecDate(ISORecDate* inDate, time_t* outDate)
{
	...
	if (year < 0) {
		...
		days += (year+2)/4 - (year + 70) / 100;
		if((year+70) >= 100) days++; // correct for 2000
		if (IS_LEAP_YEAR(year + 1970))
			if (inDate->month < 3) days--;
		
		// Add up the number of days in the last year, up to the month
		for (i = 1; (i < inDate->month) && (i < 12); i++) {
			days += monlen[i-1];
		}
		
		// Add the last month's days
		days += inDate->date - 1;
		...
		// sign extend the timezone field
		if (tz & 0x80) {
			tz |= (-1 << 8);	
		}
