...
int v_flush(hfsvol *vol, int umounting)
{
  if (! (vol->flags & HFS_READONLY)) {
	...
	if (umounting && !(vol->mdb.drAtrb & HFS_ATRB_UMOUNTED)) {
	  vol->mdb.drAtrb |= HFS_ATRB_UMOUNTED;
	  vol->flags |= HFS_UPDATE_MDB;
	} else if (!umounting && (vol->mdb.drAtrb & HFS_ATRB_UMOUNTED)) {
	  vol->mdb.drAtrb &= ~HFS_ATRB_UMOUNTED;
	  vol->flags |= HFS_UPDATE_MDB;
	}
 ...
 */
int v_newfolder(hfsvol *vol, long parid, char *name, int hidden)
{
  CatKeyRec key;
  CatDataRec data;
  long id;
  ...
  data.u.dir.dirUsrInfo.frFlags = (hidden?0x4000:0);
  ...
  if (bt_insert(&vol->cat, record, reclen) < 0) {
	printf("hfs: v_newfolder couldn't create directory record\n");
    return -1;
  }
  ...
  if (bt_insert(&vol->cat, record, reclen) < 0) {
	printf("hfs: v_newfolder couldn't create directory thread\n");
	return -1;
  }

  if (v_adjvalence(vol, parid, 1, 1) < 0) {
	printf("hfs: v_newfolder couldn't adjust parent valence\n");
    return -1;
  }
...
int v_scavenge(hfsvol *vol)
{
  ...
  printf("hfs: safeguarding unmarked blocks:\n");
  printf("hfs:   marking first extent blocks\n");
  markexts(vbm, &vol->mdb.drXTExtRec);
  printf("hfs:   marking first catalog blocks\n");
  ...
  printf("hfs:   marking extent overflows\n");
  if (n.nnum > 0) {
	if (bt_getnode(&n) < 0)
	  return -1;
  ...
  nnodes = vol->cat.hdr.bthNNodes;
  visited = (block*)malloc((nnodes/8) + 1);
  memset(visited,0,(nnodes/8)+1);
  printf("hfs:   marking catalog b-tree nodes\n");
  
  printf("hfs:   starting at node %d\n",n.nnum);
  if (n.nnum > 0) {
	if (BMTST(visited,n.nnum)) {
	  printf("hfs:   loop in catalog b-tree nodes\n");
	  return -1;
	}
	BMSET(visited,n.nnum);
	...
	while (1) {
	  ...
	  while (n.rnum >= n.nd.ndNRecs) {
		...
		/*
		printf("hfs:   following flink to node %d\n",newnode);
		*/
		if (BMTST(visited,newnode)) {
		  printf("hfs:   loop in catalog b-tree nodes\n");
		  free(visited);
		  return -1;
		}
  ...
  printf("hfs:   counting free blocks\n");
  for (blks = 0, pt = vol->mdb.drNmAlBlks; pt--; ) {
	if (! BMTST(vbm, pt))
	  ++blks;
  }
  
  printf("hfs:   %d free blocks counted\n",blks);
...
int v_remark_vbm(hfsvol *vol)
{
  int vbmsz = vol->mdb.drNmAlBlks;
  int i;
