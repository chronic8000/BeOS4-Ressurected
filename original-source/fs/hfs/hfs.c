...
static int   get_device_block_size(int fd);
static off_t get_num_device_blocks(int fd, int block_size);

static int
get_logical_block_size(int fd)
{
...
static off_t
get_num_device_blocks(int fd, int block_size)
{
    struct stat st;
    device_geometry dg;
...
hfsvol *hfs_mount(char *path, int pnum, int flags)
{
  ...
  if (vol == 0) {
	...
	{
	  int   multiplier, dev_block_size, logical_block_size;
	  off_t num_device_blocks;
	  
	  dev_block_size = get_device_block_size(vol->fd);
	  if (dev_block_size == 0) {
	    ERROR(errno, "HFS: block size of zero is not valid!\n");
	    v_destruct(vol);
		
	    return 0;
	  }

	  logical_block_size = get_logical_block_size(vol->fd);
	  if (logical_block_size == 0) {
		logical_block_size = 512;
		isRaw = 1;
	  };
	  if (logical_block_size > HFS_MAXBLOCKSZ)
		logical_block_size = HFS_MAXBLOCKSZ;

	  vol->blockSize = logical_block_size;
	  printf("hfs: dbs = %d, lbs = %d\n",dev_block_size,logical_block_size);
	  printf("hfs: vol->blockSize = %d\n",vol->blockSize); 
	  
	  multiplier = dev_block_size / logical_block_size;
	  
	  num_device_blocks = get_num_device_blocks(vol->fd, vol->blockSize);
	  if (num_device_blocks == 0) {
		dprintf("HFS: can't figure out the size of device\n");
		close(vol->fd);
		v_destruct(vol);
	  ...
	  printf("hfs: cache init: dev %d, max blocks %Ld\n", vol->fd,
			 num_device_blocks * multiplier);
	  init_cache_for_device(vol->fd, num_device_blocks * multiplier);
	}
	...
	if (isRaw) {
	  int e;
	  printf("hfs: device is raw; scanning for partition map\n");
	  e = l_readblock0(vol);
	  if (e < 0)
		return 0;
	};
...
#if 1
	/*
	  XXXdbg we don't read block0 any more because partition
	  handling is done for us by BeOS (DriveSetup).
      */
	if (l_readmdb(vol) < 0)
#else
	...
	if (vol->mdb.drAlBlkSiz % vol->blockSize != 0) {
	  remove_cached_device_blocks(vol->fd, 0);
	  close(vol->fd);
	  v_destruct(vol);
	...
	if (vol->vlen == 0) {
	  ...
	  if (ioctl(vol->fd,B_GET_GEOMETRY,&dg) < 0) {			
		if(fstat(vol->fd, &st)<0) {
			remove_cached_device_blocks(vol->fd, 0);
			close(vol->fd);
			v_destruct(vol);
			printf("hfs: couldn't get device geometry\n");
			return NULL;
		} else {
			size = st.st_size;
		}
		  ...
		  size =
			(off_t)dg.cylinder_count    *
			(off_t)dg.sectors_per_track *
			(off_t)dg.head_count        *
			(off_t)dg.bytes_per_sector  ;
	  }

	  vol->vlen = (unsigned int)(size/512);
	  printf("hfs: vlen = %d (%d physical blocks)\n",vol->vlen*512,vol->vlen);
	};
...
int hfs_format(char *path, int pnum, char *vname)
{
  ...
  if (vol.vlen < 800 * 1024 / vol.blockSize)
    {
      close(vol.fd);
  ...
  vol.mdb.drAlBlkSiz = vol.lpa * vol.blockSize;
  ...
  ext->hdr.bthFNode    = 0;
  ext->hdr.bthLNode    = 0;
  ext->hdr.bthNodeSize = vol.blockSize;
  ...
  cat->hdr.bthFNode    = 0;
  cat->hdr.bthLNode    = 0;
  cat->hdr.bthNodeSize = vol.blockSize;
  ...
  if (result == 0 && v_newfolder(&vol, HFS_CNID_ROOTPAR, vname, 0) < 0)
    result = -1;
...
int hfs_readdir(hfsdir *dir, hfsdirent *ent)
{
  ...
  printf("hfs_readdir reached beyond the while(1)\n");
  return -1;
}
  ...
  while (count)
    {
      ...
      bnum  = file->pos / file->vol->blockSize;
      offs  = file->pos % file->vol->blockSize;
...
long hfs_write(hfsfile *file, void *buf, unsigned long len)
{
  ...
  while (count)
      ...
      if (file->pos + chunk > *pylen)
	{
      ...
      if (offs > 0 || chunk < file->vol->blockSize)
	{
	  if (f_getblock(file, bnum, &b) < 0)
	    return -1;
	}
...
int hfs_mkdir(hfsvol *vol, char *path)
{
  ...
  if (v_newfolder(vol, parid, name, 0) < 0)
    return -1;
