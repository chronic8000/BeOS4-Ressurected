...
int l_readblock0(hfsvol *vol)
{
  ...
  switch (rec.sbSig)
    ...
    case 0x4552:  /* block device with a partition table */
      {
	if (rec.sbBlkSize != vol->blockSize)
	  {
...
int l_readmdb(hfsvol *vol)
{
  ...
  vol->lpa = mdb->drAlBlkSiz / vol->blockSize;
...
int l_checkvbm(hfsvol *vol)
{
  ...
  while (vbmsz--) {
	...
	bp++;
    /*
	printf("hfs: VBM per-block checksum = %Lx\n",perblock);
    */
	checksum += perblock;
  };

  printf("hfs: VBM checksum = %Lx\n",checksum);
}
