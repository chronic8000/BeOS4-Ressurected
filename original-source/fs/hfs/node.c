...
# define NODESPACE(n)  \
  (512 - (n).roff[(n).nd.ndNRecs] - 2 * ((n).nd.ndNRecs + 1))
/*  ((n).bt->f.vol->blockSize - (n).roff[(n).nd.ndNRecs] - 2 * ((n).nd.ndNRecs + 1)) */
...
int n_search(node *np, unsigned char *key)
{
  ...
  r_unpackcatkey(key,&ckr);
  /*
  printf("n_search(%d:'%s')\n",ckr.ckrParID,ckr.ckrCName);
  */
  for (i = np->nd.ndNRecs; i--; )
    {
      ...
      rec = HFS_NODEREC(*np, i);
	  r_unpackcatkey(rec,&ckr);
	  /*
	  printf("Checking against (%d:'%s')\n",
			 ckr.ckrParID,ckr.ckrCName);
	  */
      if (HFS_RECKEYLEN(rec) == 0)
...
int n_delete(node *np, unsigned char *record, int *flag)
{
  ...
  if ((np->nd.ndNRecs == 0) && (np->nnum != np->bt->hdr.bthRoot)) {
	if (np->nd.ndBLink) {
	  left.bt   = np->bt;
	  left.nnum = np->nd.ndBLink;
	  if (bt_getnode(&left) < 0)
		return -1;
	  left.nd.ndFLink = np->nd.ndFLink;
	  bt_putnode(&left);
	}
	if (np->nd.ndFLink) {
	  right.bt   = np->bt;
	  right.nnum = np->nd.ndFLink;
	  if (bt_getnode(&right) < 0)
		return -1;
	  right.nd.ndBLink = np->nd.ndBLink;
	  bt_putnode(&right);			
	}
