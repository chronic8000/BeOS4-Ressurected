...
int main(int nargs,char *args[])
{
	...
	for (i = 0; i<2; i++)
		for (j=0; j < num_outputs; j++)
		{
			...
			printf("Play buffers[%d][%d]: 0x%p\n",i,j,pPlay[i][j]);
		}
	for (i = 0; i<2; i++)
		for (j=0; j < num_inputs; j++)
		{
			pRecord[i][j] = (int32 *) MBL.record_buffers[i][j].base;
			printf("Record buffers[%d][%d]: 0x%p\n",i,j,pRecord[i][j]);			
		}
	...
	while (1)
	{
		for ( i = 0; i < half_buffer_size; i ++ ) {
			pPlay[cntr&1][0][i] = pRecord[(cntr-0)&1][0][i];
			pPlay[cntr&1][1][i] = pRecord[(cntr-0)&1][1][i];
			pPlay[cntr&1][2][i] = pRecord[(cntr-0)&1][2][i];
			pPlay[cntr&1][3][i] = pRecord[(cntr-0)&1][3][i];
			if (!is_gina) {
				pPlay[cntr&1][4][i] = pRecord[(cntr-0)&1][4][i];
				pPlay[cntr&1][5][i] = pRecord[(cntr-0)&1][5][i];
				pPlay[cntr&1][6][i] = pRecord[(cntr-0)&1][6][i];
				pPlay[cntr&1][7][i] = pRecord[(cntr-0)&1][7][i];
				pPlay[cntr&1][8][i] = pRecord[(cntr-0)&1][8][i];
				pPlay[cntr&1][9][i] = pRecord[(cntr-0)&1][9][i];
			}
			else {
