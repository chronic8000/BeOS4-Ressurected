...
for (i = sb->wss_base - 4; i < sb->wss_base + 4; i++)
  ...
  mc_change(sb, 7, 0, 0); /* master volume to max */
  mc_change(sb, 8, 0, 0);
				...
				res_type type)
{
  ...
  if (pnp->get_current_configuration_for(cookie, config, size) == B_OK) {
	...
	while (pnp->get_nth_resource_descriptor_of_type(config, i++, B_IO_PORT_RESOURCE,
	  ...
	  if ((type == RES_MC
		   && base >= 0x100 && base <= 0xffe && (base & 1) == 0 && len == 2
		   && (base < 0x300 || base > 0x334))
		  || (type == RES_WSS
			  && base >= 0x530 && base <= 0xf48 && (base & 7) == 0 && len == 8)
		  || (type == RES_MPU
			  && base >= 0x330 && base <= 0x334 && (base & 1) == 0 && len == 2))
	  {
		if (irq_res == NULL)
		  return config;
  ...
  while (pnp->get_next_device_info(B_ISA_BUS, &cookie, &dev.info, sizeof(dev))
	...
	if (dev.info.flags & B_DEVICE_INFO_ENABLED)
	  if (same_card_p(sb, &dev))
		if (compatible_p(&dev, 'Y', 'M', 'H', 0x002, 1)) {
  ...
  while (pnp->get_next_device_info(B_ISA_BUS, cookie, &dev.info, sizeof(dev))
	...
	if (dev.info.flags & B_DEVICE_INFO_ENABLED)
	  if (compatible_p(&dev, 'Y', 'M', 'H', 0x002, 1)) {
		...
		if (config) {
		  sb->mc_base = port_res.d.r.minbase;
		  mc_change(sb, 3, 0, 5);
		  free(config);
		  config = allocate_config(*cookie, &port_res, NULL, RES_WSS);
		  if (config) {
			sb->wss_base = port_res.d.r.minbase + 4;
			setup_wss(sb, config);
