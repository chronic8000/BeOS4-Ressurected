...
static int32 sonic_vibes_interrupt(void * data);
...
{
	char * name = card->name;
	sprintf(name, "sonic_vibes/%d", card-cards+1);
...
dprintf("sv: '%s'\n", card->name);
	sprintf(name, "%.25s_low", card->name);
	...
	if (curarea >= 0) {	/* area there from previous run */
		...
		addr = ainfo.address;
		if (trysize < low_size) {
			ddprintf("sonic_vibes: too small (%x)\n", trysize);
			goto allocate;
		}
...
dprintf("sonic_vibes: physical %x  logical %x\n", where.address, ainfo.address);
		goto a_o_k;
	}
...
allocate:
	...
	ddprintf("sonic_vibes: create_area(%x) returned %x logical %x\n", 
	...
	ddprintf("sonic_vibes: physical %x\n", where.address);
	if ((uint32)where.address & 0xff000000) {
		delete_area(curarea);
		curarea = B_ERROR;
		goto oops;
	...
	sonic_vibes_dev * card)
{
	...
	ddprintf("sonic_vibes: setup_sonic_vibes(%x)\n", card);
	...
	while ((*pci->get_nth_pci_info)(ix, &info) == B_OK) {
		if (info.vendor_id == SONIC_VIBES_VENDOR_ID &&
			...
			if (setup_sonic_vibes(&cards[num_cards])) {
				dprintf("Setup of SonicVibes %d failed\n", num_cards+1);
			}
			else {
				num_cards++;
			}
...
uninit_driver(void)
{
	...
	for (ix=0; ix<cnt; ix++) {
		/*	Make SURE the interrupt handler is gone!	*/
		remove_io_interrupt_handler(cards[ix].info.u.h0.interrupt_line, sonic_vibes_interrupt, &cards[ix]);
