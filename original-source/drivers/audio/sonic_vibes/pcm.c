	...
	pcm_dev * port)
{
	...
	ddprintf("play_buf_size %x   rec_buf_size %x\n", 
...
/* here, we should mute the PCM output to avoid clicking */
	...
	ddprintf("sonic_vibes: DMA starts as %x/%x\n", port->config.format, port->open_mode);
	set_indirect(port->card, 0x13, port->open_mode&~kRecord, 0xff);
	if (port->open_mode&kRecord) {
		spin(500);	/* delay starting record, so that SR modulation will work well */
...
{
	status_t err = B_OK;
	int m, n, r;	/* parameters for the PLL sample rate synthesizer */
	...
	if (!config->play_buf_size) {
		/* default is 512 frames for a comfy 6 ms latency */
		s = 512*config->channels*(config->format&0xf);
	}	/* minimum is 128 frames for a 2.9 ms latency -- can't do less, because of the sample rate lock code */
	else for (s = 128*config->channels*(config->format&0xf); s < MIN_MEMORY_SIZE/2; s = (s<<1)) {
	...
	if (asr > -1 || n > -1) { /* new sampling rate */
		...
		port->demodulate = 0;
		port->orig_sr_factor = (int32)(port->config.sample_rate*65535/48000);
		port->sr_factor = port->orig_sr_factor;
		port->min_sr_factor = port->orig_sr_factor*0.995;
		port->max_sr_factor = port->orig_sr_factor*1.005;
		if (port->max_sr_factor > 65535) port->max_sr_factor = 65535;
	}
...
apply_sr_factor(
	pcm_dev * port)
{
	set_indirect(port->card, 0x1e, ((uint32)port->sr_factor), 0xff);
	set_indirect(port->card, 0x1f, ((uint32)port->sr_factor)>>8, 0xff);
}
	...
	size_t len)
{
	...
	case SOUND_SET_PARAMS: {
		...
		/* Huh-huh; he said "hard-code"! */
		/* ignore sample rate */
		if (sound->sample_rate == kHz_48_0) {
			if (config.sample_rate < 47999 || config.sample_rate > 48001) {
...
//		break;
	...
	case SOUND_SET_PLAYBACK_PREFERRED_BUF_SIZE:
		config.play_buf_size = (int32)data < 1024 ? 1024 : (int32)data;
		configure = true;
		err = B_OK;
		break;
	case SOUND_SET_CAPTURE_PREFERRED_BUF_SIZE:
		config.rec_buf_size = (int32)data < 1024 ? 1024 : (int32)data;
		configure = true;
		err = B_OK;
		break;
	...
	case SOUND_GET_CAPTURE_PREFERRED_BUF_SIZE:
		*(int32*)data = config.rec_buf_size;
		...
		break;
	default:
		OLDAPI("sonic_vibes: unknown code %d\n", iop);
		err = B_BAD_VALUE;
		break;
	}
...
first_time:	/* we need to check whether anything's available first */
	...
	/*	modulate playback sampling rate */
	...
	diff = port->next_rd_time - port->wr_time;
	if (diff > 1400LL) {	/*	modulate playrate up */
		if (port->demodulate > 0) {
			port->sr_factor += port->demodulate/2;
			port->demodulate = 0;
			apply = 1;
		}
		else if (port->sr_factor < port->max_sr_factor) {
			port->sr_factor++;
			port->demodulate--;
			apply = 1;
		}
	}
	else if (diff < 800LL) {	/*	modulate playrate down */
		if (port->demodulate < 0) {
			port->sr_factor += port->demodulate/2;
			port->demodulate = 0;
			apply = 1;
		}
		if (port->sr_factor > port->min_sr_factor) {
			port->sr_factor--;
			port->demodulate++;
			apply = 1;
		}
	}
	else if (port->demodulate) {
		port->sr_factor += port->demodulate/2;
		port->demodulate = 0;
		apply = 1;
	}
	if (apply) {
		apply_sr_factor(port);
	}
	...
	*nwritten = 0;
	...
	while (to_write > 0) {
		...
		put_cnt++;
		{
			...
			if (delta < 1) {
				ddprintf("sonic_vibes: delta %Ld (low!) #%d\n", delta, put_cnt);
			}
			else if (delta > 2000) {
				ddprintf("sonic_vibes: delta %Ld (high!) #%d\n", delta, put_cnt);
			}
		}
		if (put_cnt != int_cnt) {
	static int last;
			if (last != int_cnt-put_cnt)
				OLDAPI("sonic_vibes: %d mismatch\n", int_cnt-put_cnt);
...
/*	KTRACE(); /* */
	...
	if (port->wr_cur == ptr) {
		port->wr_skipped++;
		OLDAPI("sonic_vibes: write skipped %d\n", port->wr_skipped);
	}
	port->wr_cur = ptr;
	port->was_written = 0;
...
/*	KTRACE(); /* */
	...
	if (port->rd_cur == ptr) {
		port->rd_skipped++;
		OLDAPI("sonic_vibes: read skipped %d\n", port->rd_skipped);
	}
