  ...
  for (i = 1, sc = &s->right, cc = &port->setup.right;
	   ...
	   i--, sc = &s->left, cc = &port->setup.left)
	{
	  if (sc->adc_gain != cc->adc_gain
		...
		ichange (sb, CS_left_adc_control + i, 0x00, 
				 (sc->adc_gain & 0x0f) +
				 (sc->adc_source << 6) +
