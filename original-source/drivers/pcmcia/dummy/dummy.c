/*======================================================================
    ...
    dummy.c 1.3 1998/10/03 01:20:25
...
"dummy.c 1.3 1998/10/03 01:20:25 (David Hinds)";
...
#if __BEOS__
    sprintf(dev->node.dev_name, "bus/pcmcia/dummy/dummy%d", nd);
#else
    sprintf(dev->node.dev_name, "dummy/dummy%d", nd);
#endif
...
static status_t dummy_open(const char *name, uint32 flags,
			   void **cookie)
{
    int nd;
...
static status_t dummy_close(void *cookie)
{
    dev_link_t *link = cookie;
    DEBUG(0, ("dummy_close('%s')\n", link->dev->dev_name));
    return B_OK;
} /* dummy_close */

static status_t dummy_free(void *cookie)
{
    dev_link_t *link = cookie;
    DEBUG(0, ("dummy_free('%s')\n", link->dev->dev_name));
    return B_OK;
} /* dummy_free */

static status_t dummy_read(void *cookie, off_t pos,
			   void *data, size_t *len)
{
    dev_link_t *link = cookie;
    DEBUG(0, ("dummy_read('%s')\n", link->dev->dev_name));
    return B_OK;
} /* dummy_read */

static status_t dummy_write(void *cookie, off_t pos,
			    const void *data, size_t *len)
{
    dev_link_t *link = cookie;
    DEBUG(0, ("dummy_write('%s')\n", link->dev->dev_name));
    return B_OK;
} /* dummy_write */

/*====================================================================*/

static device_hooks dummy_device_hooks = {
    &dummy_open,
    &dummy_close,
    &dummy_free,
    NULL,
...
static status_t init_cs(void)
{
	...
	printk("dummy_cs: Card Services release does not match!\n");
	return B_ERROR;
    }
