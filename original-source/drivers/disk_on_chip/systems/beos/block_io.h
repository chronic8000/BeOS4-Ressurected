...
extern status_t mem2veccpy(const iovec *destvec, size_t destveccount, uint32 deststartbyte,
							uint8 *srcbuf, size_t size);

extern status_t vec2memcpy(uint8 *destbuf, const iovec *srcvec, size_t srcveccount,
