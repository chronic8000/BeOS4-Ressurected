/*
 * $Log:   V:/Flite/archives/FLite/src/FATLITE.C_V  $
 * 
 *    Rev 1.86   Mar 12 2000 19:51:24   veredg
 * Remove flCall. Now there's only bdCall in blockdev.c
 * 
 *    Rev 1.85   Jan 20 2000 19:18:36   vadimk
 * check for write protection in flcall 
 *
 *    Rev 1.84   Jan 20 2000 16:52:04   danig
 * Fixed DEBUG_PRINT in readFile.
 *
 *    Rev 1.83   Jan 17 2000 13:54:24   vadimk
 * Remove fatformat function
 *
 *    Rev 1.83   Jan 17 2000 12:54:50   vadimk
 * Remove fatformat
 *
 *    Rev 1.82   Jan 13 2000 18:10:52   vadimk
 * TrueFFS OSAK 4.1
 *
 *    Rev 1.81   Aug 05 1999 18:03:10   marinak
 * Bug fix:in case of full root directory file operations
 * returned flSectorNotFound status.
 *
 *    Rev 1.80   Jul 20 1999 18:25:26   marinak
 * In spite of the parent directory is read only,
 * the file that is not read only may be deleted.
 *
 *    Rev 1.79   Jul 20 1999 17:48:54   marinak
 * Set attributes bug fixed
 *
 *    Rev 1.78   Jul 12 1999 16:53:24   marinak
 * dosFormat call is passed from blockdev.c to fatlite.c
 *
 *    Rev 1.77   May 25 1999 16:49:38   Amirban
 * Missing Big-Endian handling in splitFile
 *
 *    Rev 1.76   15 Apr 1999 15:44:38   marina
 * Fix compilation problem in case FAT 12 bit support is off
 *
 *    Rev 1.75   14 Apr 1999 14:28:56   marina
 * Now may be compiled if FILES==0
 *
 *    Rev 1.74   04 Apr 1999 17:36:38   marina
 * fix createMutex in flInit
 *
 *    Rev 1.73   03 Mar 1999 11:36:46   marina
 * fix writeMultiSector bug
 *
 *    Rev 1.72   01 Mar 1999 14:08:06   marina
 * writeMultiSector bug fix
 *
 *    Rev 1.71   23 Feb 1999 20:42:20   marina
 * check if volume flag & VOLUME_MOUNTED in flCall
 *
 *    Rev 1.70   08 Feb 1999 15:37:48   marina
 * return flDataError in place of dataErrorObject
 *
 *    Rev 1.69   03 Feb 1999 18:19:34   marina
 * doc2map edc check bug fix
 *
 *    Rev 1.68   31 Jan 1999 20:03:58   marina
 * read/write multiple sectors in read/write file:
 *   improves ATA performance
 *
 *    Rev 1.67   31 Dec 1998 16:27:44   marina
 * Exit call passed to blockdev.c
 *
 *    Rev 1.66   23 Dec 1998 17:37:38   marina
 * Pass flMsecCount to blockdev.c
 *
 *    Rev 1.65   22 Dec 1998 14:57:34   marina
 * Block Device driver and file system were separated
 *
 *    Rev 1.64   15 Dec 1998 16:37:38   marina
 * In function flCall return status of setBusy
 *
 *    Rev 1.63   25 Nov 1998 18:08:52   marina
 * In absWrite: #if FILES>0 invalidate buffer.
 *
 *    Rev 1.62   23 Nov 1998 12:17:48   amirban
 * setBusy & interval routine corrections
 *
 *    Rev 1.61   08 Nov 1998 17:03:34   marina
 * new user call FLUSH_BUFFER was added
 *
 *    Rev 1.60   25 Oct 1998  3:14:38   Dimitry
 * Add flSectorsInVolume function
 * Add passing irFlags to physicalRead and physicalWrite functions
 *   to manage EDC and EXTRA modes
 *
 *    Rev 1.59   26 Oct 1998 11:48:28   marina
 * Initializations, HIGH_SECURITY option
 *
 *    Rev 1.58   18 Oct 1998 14:39:54   marina
 * In flCall: check noOfDrives if DRIVES==1
 *
 *    Rev 1.57   06 Sep 1998 11:00:04   danig
 * Close new file when splitFile fails
 *
 *    Rev 1.56   03 Sep 1998 13:58:56   ANDRY
 * better DEBUG_PRINT
 *
 *    Rev 1.55   16 Aug 1998 20:30:54   amirban
 * Addition for TL interface changes & compression
 *
 *    Rev 1.54   02 Aug 1998 17:05:30   danig
 * Fixed bug in flSplitFile
 *
 *    Rev 1.53   10 Mar 1998 10:08:28   Yair
 * Added casts
 *
 *    Rev 1.52   01 Mar 1998 13:00:46   amirban
 * Added flAbsAddress
 *
 *    Rev 1.51   24 Feb 1998 12:46:40   marina
 * MK fixed appendFile bug
 *
 *    Rev 1.50   23 Feb 1998 17:07:50   Yair
 * Added casts. Always dismount, disregarding the value of FILES
 *
 *    Rev 1.49   26 Nov 1997 18:21:46   danig
 * Call updateSocketParams from flCall
 *
 *    Rev 1.48   23 Nov 1997 17:18:10   Yair
 * Get rid of warnings (with Danny)
 *
 *    Rev 1.47   28 Oct 1997 16:36:08   danig
 * FAT_LAST_CLUSTER for 12-bit FAT & fixed joinFile
 *
 ...
 * BPB redefined
 *
 *    Rev 1.19   03 Nov 1996 17:40:14   amirban
...
FLStatus flushBuffer(Volume vol)
{
  if (buffer.dirty) {
    ...
    if (&vol != bufferOwner) {
      ...
      checkStatus(setBusy(bufferOwner,ON));
    }
#else
				 ...
				 unsigned *offsetInSector)
{
  ...
  if (offsetInCluster == 0) {	/* This cluster is finished. Get next */
    if (!(file->flags & FILE_IS_ROOT_DIR)) {
      if ((file->currentPosition >= file->fileSize) && (file->currentPosition>0)) {
     #ifndef FL_READ_ONLY
                                  ...
                                  SectorNo* sectors)
{
  ...
  while(stillToWrite>=((sectorCount+1)<<SECTOR_SIZE_BITS)){
    if(offsetInCluster>=vol.bytesPerCluster) {
      if ((file->currentPosition+(sectorCount<<SECTOR_SIZE_BITS))>= file->fileSize) {
...
FLStatus writeFile(File *file, IOreq FAR2 *ioreq)
{
  ...
  while (stillToWrite > 0) {
    ...
    if (stillToWrite < SECTOR_SIZE || offsetInSector > 0) {
      ...
      tffscpy(buffer.data + offsetInSector,userData,shortWrite);
    }
    else {
      SectorNo sectorCount;
      ...
      if (((sectorToWrite+sectorCount > buffer.sectorNo) && (sectorToWrite <= buffer.sectorNo)) &&
          (&vol == buffer.owner)) {
