...
/*
 * $Log:   V:/Flite/archives/FLite/src/REEDSOL.C_V  $
 * 
 *    Rev 1.11   Jan 26 2000 15:12:14   vadimk
 * Remove all  functions that are enclosed under EDC_IN_SOFTWARE define
 *
 *    Rev 1.10   22 Dec 1999 13:46:10   dimitrys
 * Fix Error only in Data Area ( 512 bytes )
 *
 *    Rev 1.9   23 Feb 1998 17:08:42   Yair
 * Added casts
 *
...
EDCstatus flCheckAndFixEDC(char FAR1 *block, char *syndrom, FLBoolean byteSwap)
{
  ...
  if(status == CORRECTABLE_ERROR)       /* Fix the errors if possible */
  {
    ...
    for (i=0; i < errorsNum; i++)
      if( (errorLocs[i] ^ byteSwap) < BLOCK )  /* Fix only in Data Area */
