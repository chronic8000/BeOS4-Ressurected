...
/* Bus Types */
#define NE2K__BUS_MSK			0x0007
#define NE2K__BUS_ISA			0x0001
#define NE2K__BUS_PCI			0x0002
#define NE2K__BUS_PCMCIA		0x0004
...
typedef struct {
	...
	int			rx_current_page;			/* rx current page pointer */

	/* Useless? */
	int			ring_buf_start, ring_buf_size;		/* Total buffer start and size */

	fc_lock_t		fc_rx_lock, fc_tx_lock;			/* fc locks for tx/rx */
	...
	int			dbuf_write_index;			/* double buffer write index (0/1) */
	uint32			dbuf_pending;				/* pending buffer count */
	int			dbuf_size[2];				/* The size of both buffered packets */
	...
	int32			stat_rx_missed_errors;
#ifdef STAT_COUNTING
	/* Statistics counters */
	int32			stat_rx_packets;
	int32			stat_rx_size_errors;
	int32			stat_rx_fifo_errors;
	int32			stat_rx_bogus_errors;
	int32			stat_rx_resets;

	int32			stat_tx_packets;
	...
	int32			stat_tx_triggers;
	int32			stat_tx_buffered;
	int32			stat_tx_busy_bail;
	int32			stat_tx_not_pending_bail;

	int32			stat_rdma_conflicts;
	int32			stat_rdma_timeouts;

	int32			stat_ovw_rw_busy_defer;
	int32			stat_ovw_reentry_defer;

	int32			stat_collisions;
	
	int32			stat_int_calls;
	int32			stat_ints;
	int32			stat_ints_dropped;
	int32			stat_int_prx;
	int32			stat_int_ptx;
	int32			stat_int_rxe;
	int32			stat_int_txe;
...
struct global_dev_info {
	...
	int		cardType;	/* Card type from the PCI or ISA card enums */
	int		debug;		/* Per driver debug level */
	int		flags;		/* ISA/PCI/ISAPnP/PCMCIA bus? */
	int		irq;		/* irq level */
...
\****************************************************************************/
enum {
	NE2K__16BIT_IO_ONLY	=0x080,
	NE2K__32BIT_IO_ONLY	=0x040,
	NE2K__STOP_PG_0x60	=0x100,
	NE2K__FORCE_FDX		=0x200,
	NE2K__REALTEK_FDX	=0x400,
	NE2K__HOLTEK_FDX	=0x800,
};
...
static status_t init_ne2k(device_info_t *device) {
	...
	write8(device->base + NE2K0_DCR_W, NE2K0_DCR_WTS_WORD | NE2K0_DCR_BOS_LEND | 
						...
						NE2K0_DCR_ARM_SCNE | NE2K0_DCR_FTS_4WD);
...
static int32 get_pci_list(struct global_dev_info info[], int32 maxEntries) {
	...
	for (i=0, entries=0; entries<maxEntries; i++) {
		...
		ETHER_DEBUG( gDevID, INFO, "%s devID# %d found at IRQ %x \n", pci_clone_list[foundID].name, 
...
static int32 get_isa_list(struct global_dev_info info[], int32 maxEntries, int32 curEntries) {
	...
	if ( status == B_OK ) {
		...
		for (i = 0, entries=curEntries; entries < maxEntries; i++) {
			...
			if (size < 0) {
				ETHER_DEBUG(gDevID, ERR, "Error getting size of configuration %x\n", size);
				continue;
			}
			...
			if (status < 0) {
				ETHER_DEBUG(gDevID, ERR, "Error getting IRQ %x\n", status);
				free(config);
				continue;
			}
			...
			free(config);
			if (status < 0) {
				ETHER_DEBUG(gDevID, ERR, "Error getting I/O range %x\n", status);
				continue;
			}

			if (resourceDesc.d.r.len != 0x20) {
				ETHER_DEBUG(gDevID, ERR, "I/O range must be 0x20 not %x\n", resourceDesc.d.r.len);
				continue;
			}

			gDev[entries].port = resourceDesc.d.r.minbase;
			gDev[entries].flags |= NE2K__BUS_ISA;
			ETHER_DEBUG(gDevID, INFO, "device %d found at irq %x port %x\n", entries, gDev[curEntries].irq, gDev[curEntries].port);
			entries++;
			gDevCount += 1;
		}
...
static status_t enable_addressing(device_info_t *device) {
	...
	ETHER_DEBUG(device->devID, INFO, "base=%x\n", device->base);
...
static int32 load_ne2k_settings(struct global_dev_info info[], int32 maxEntries, int32 curEntries) {
	...
	ETHER_DEBUG(gDevID, FUNCTION, "load_ne2k_settings ENTRY with %d entries\n", curEntries);
	...
	if (settings) {
		...
		for (i=0;((i<entries) && ((curEntries + i) <= maxEntries));i++) {
			...
			ETHER_DEBUG(gDevID, INFO, "loading kernel settings for device (%d)\n", (curEntries + i));
...
static status_t allocate_resources(device_info_t *device) {
	...
	if ((result = create_fc_lock( &device->fc_tx_lock, 2, kDevName " tx" )) < 0 ) {
		delete_fc_lock(&device->fc_rx_lock);
		dprintf(kDevName " create fc_tx_lock failed %lx \n", result);
		return (result);
	}
	...
	if((device->sem_reset_lock = create_sem(0, kDevName " reset" )) < 0 ) {
		delete_fc_lock(&device->fc_rx_lock);
		delete_fc_lock(&device->fc_tx_lock);
		dprintf(kDevName " create sem_reset_lock failed %lx \n", result);
		return (B_ERROR);
	}
...
retry_get_packet:
		...
		&& pkt_head.next != (next_frame + 1 - num_rx_pages))
	{
		if (retry_count-- > 0) {
			...
			ETHER_DEBUG(device->devID, ERR, "retrying packet header read\n");
...
static status_t read_hook(void *_device, off_t pos, void *buf, size_t *len) {
	...
	ETHER_DEBUG(device->devID, INFO, "waiting on hw_lock in read_hook\n");
...
mout_sanity_retry:
	...
	if ( acquire_sem_etc( device->sem_rdma_done, 1, B_RELATIVE_TIMEOUT, 10000 ) != B_NO_ERROR ) {
		ETHER_DEBUG(device->devID, ERR, "timeout waiting for RDC! (status_ovw %d, tx_lock_count=%d)\n", device->status_ovw, device->fc_tx_lock.count);
...
static status_t ne2k_trigger_send(device_info_t *device) {
	...
	ETHER_DEBUG(device->devID, INFO, "triggered write from page %d @ %d\n", (device->dbuf_read_index ^ 0x01),
...
static status_t write_hook(void *_device,off_t pos,const void *buf,size_t *len) {
	...
	if ( *len > NE2K__MAX_FRAME_SIZE ) {
		ETHER_DEBUG(device->devID, ERR, ": write %d > 1514 too long\n",*len);
	...
	status = fc_wait( &device->fc_tx_lock, device->blockFlag & B_TIMEOUT ? 0: NE2K__TX_TIMEOUT , 1 );

	if ( status == B_TIMED_OUT ) {
		fc_lock( &device->fc_tx_lock );	/* Lock the tx_lock in order to signal (since the fc_wait timed out) */
	    	ETHER_DEBUG( device->devID, ERR, "Transmitter timeout in write_hook -> Resignalling transmit locks\n" );
		fc_signal( &device->fc_tx_lock, 1, B_DO_NOT_RESCHEDULE );
		fc_lock( &device->fc_tx_lock );	/* Lock the tx_lock again since we're about to modify the ring */
	} else if ( status != B_NO_ERROR ) {
		ETHER_DEBUG(device->devID, FUNCTION, "write_hook EXIT (prematurely killed before fc_tx_lock freed)\n");
		*len = 0;
		return B_ERROR;
	}

	/* We want to spinlock all accesses to all indices and counters relating to
	 * our double buffers.  We will just 're-use' the fc_tx_lock for this. */
...
retry:
	...
	if (read8(device->base + NE2K_CR) & NE2K_CR_TXP) {
	    	fc_lock( &device->fc_tx_lock );
...
static int32 interrupt_hook(void *_device) {
	...
	//if ( (status == 0) || (device->status_ovw) ) {
	if (status == 0) {
		if ( gDev[device->devID].flags & NE2K__BUS_PCI ) {
			/* shared interrupts go to next devices ISR */
			return B_UNHANDLED_INTERRUPT;
		} else {
		    	ETHER_DEBUG( device->devID, INTERRUPT, "interrupt_hook called with ISR=%#2.2x, ovw=%d\n", status, device->status_ovw );
	...
	while (loop_count-- > 0) {
		ETHER_DEBUG(device->devID, INTERRUPT, "Interrupt status=%x\n", status);
#ifdef STAT_COUNTING
		device->stat_ints++;
#endif
		...
		if(status & NE2K0_ISR_RST) {
			...
			if ( status & NE2K0_ISR_OVW ) {	/* Overwrite warning */
				...
				if ( !device->status_ovw ) {
					ETHER_DEBUG(device->devID, ERR, "Interrupt ISR_OVW, ISR=%2.2x\n", status);
#ifdef STAT_COUNTING
					device->stat_int_ovw++;
#endif
		...
		if (status & NE2K0_ISR_PTX) {					/* Packet transmitted */
			...
			handled = ne2k_interrupt_ptx(device);
#ifdef STAT_COUNTING
			device->stat_int_ptx++;
#endif
		} 
...
#endif
		ETHER_DEBUG(device->devID, INFO, "TX Collision!! tx_status=%2.2x, status_ovw=%d\n", status, device->status_ovw);
	}
...
static status_t ne2k_cleanup_thread(void *_device) {
	...
	while (1) {
		...
		while ( ( --roto_root_count > 0 ) && ( ne2k_copy_packets(device, NULL) != -1 ) ) {
		    	/* Do Nothing */
		}
	    	roto_root_count = 10; /* Reset the roto-root counter for the next iteration */
...
static int ne2k(int argc, char **argv) {
	...
	for (i= argc, j= 1; i > 1; i--, j++) {
		...
		case 'S':
		case 's':
			kprintf(kDevName " STATISTICS (Card %d):\n", gDevID);			
			kprintf("------------\n");
#ifdef STAT_COUNTING
			kprintf("General: \n");
			...
			kprintf("------------\n");
			kprintf("Interrupts: \n");
			kprintf("total ints %d: prx %d, ptx %d, rxe %d, txe %d\n", gDev[gDevID].devInfo->stat_ints, 
			...
			kprintf("ovw %d, cnt %d, rdc %d, rst %d\n",
			...
			kprintf("tx aborted errors %d, tx carrier errors %d, tx fifo errors %d\n", 
			...
			kprintf("tx heartbeat errors %d, tx window errors %d, collisions %d\n", 
