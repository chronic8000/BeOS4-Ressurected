...
static void *morecore(ptrdiff_t increment, malloc_state *)
{
	...
	heapUsed += increment;
	printf("*** Now using %ld bytes for transaction heap.\n", heapUsed);
	return r;
}
...
#if BINDER_DEBUG_LIB
	area = create_area("binder I/O",&base,B_ANY_ADDRESS,HEAP_SIZE,B_NO_LOCK,B_READ_AREA|B_WRITE_AREA);
#else
	area = create_area("binder I/O",&base,B_ANY_KERNEL_ADDRESS,HEAP_SIZE,B_NO_LOCK,B_READ_AREA|B_WRITE_AREA);
#endif
...
void set_root(binder_node *n)
{
	...
	rootNode = n;
	UNLOCK(rootNodeLock);
	if (old) old->Release(PRIMARY, &rootNode);
