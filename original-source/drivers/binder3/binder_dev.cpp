...
init_driver(void)
{
	...
	if ((err = add_debugger_command("binder3", binder_debug, "dump binder3 info")) == 0) goto err2;

	return B_OK;

//err3:
	remove_debugger_command("binder3", binder_debug);
err2:
	uninit_binder();
err1:
...
binder_close(void *cookie)
{
	...
	ddprintf("*** CLOSING DEVICE FOR THREAD %ld!  Refs remaining: (%ld,%ld)\n",
