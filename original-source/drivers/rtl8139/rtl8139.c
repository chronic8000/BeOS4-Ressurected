...
/* Size of the in-memory receive ring. The next 4 defines and BUFFER_SIZE are interdependent change one, change all */
#define RX_BUFFERS					32			/* RX_BUFFERS * BUFFER_SIZE = RX_BUF_LEN */
#define RX_BUF_LEN_IDX	3						/* 0==8K, 1==16K, 2==32K, 3==64K */
#define RX_BUF_LEN (8192 << RX_BUF_LEN_IDX)
#define RX_BUF_MASK 0xFFFF						/* RX_BUF_LEN -1 */
...
typedef struct {
	...
	uint32          cur_rx, rx_queued;			/* Index into the Rx buffer of next Rx pkt. */
	uint32			rx_overflow;				/* flag to reset receiver on overflow */
...
static void reset_device(dev_info_t * device) {
	...
	ETHER_DEBUG(WARN, device->debug, "Chip reset after %ld\n", i);

	/* zero indexes */
	device->tx_acked = device->tx_sent = device->cur_rx =  device->rx_queued = 0;

	get_mac_address(device);

	/* set chips address filter to receive frames sent to its mac address */

	write32(device->base + MAC0,  *((uint32 *) device->mac_address.ebyte));
	write16(device->base + MAC4,  *((uint16 *) &device->mac_address.ebyte[4]));
		
	ETHER_DEBUG(WARN, device->debug, "MAC0 %8.8lx %8.8lx\n",
		read32(device->base+MAC0),read32(device->base+MAC4));
...
static status_t open_hook(const char *name, uint32 flags, void **cookie) {
	...
	ETHER_DEBUG(FUNCTION, device->debug, "open %s device=%p\n", name, device);
	...
	if ((status = allocate_resources(device)) != B_OK) {
		goto err1;
	}	
	...
	if ((status = init_ring_buffers(device)) != B_OK) { /* allocate and initialize frame buffer rings & descriptors */
		goto err2;
	}
...
static status_t free_hook(void * cookie) {
	...
	ETHER_DEBUG(FUNCTION, device->debug, ": free %p\n",device);
...
static status_t control_hook(void * cookie,uint32 msg,void *buf,size_t len)
{
	...
	ETHER_DEBUG(FUNCTION, device->debug, "IO control %lx\n",msg);
...
//				dprintf("%2.2x%c", ((uint8 *)buf)[i], i==5?' ':':');
			}
//			dprintf("\n");
			return B_OK;
		}
		...
		default:
			ETHER_DEBUG(ERR,device->debug, "unknown iocontrol %lx\n",msg);
			return B_ERROR;
	}
...
static status_t  read_hook(void *_device,off_t pos, void *buf,size_t *len)
{
	...
	if (device->rx_overflow) {  /* dump the rx queue - reset pointers and semaphore */
		ETHER_DEBUG(ERR,device->debug,"read_hook() rx overflow\n");
		device->rx_overflow = false;
		delete_sem(device->ilock);
		if ((device->ilock = create_sem(0, kDevName " rx")) < 0) {
			dprintf(kDevName " RX overlfow - create rx sem failed %lx\n", device->ilock);
			return (device->ilock);
		}
		...
		*len = 0;
		...
		/* reset the receiver */
		device->cur_rx =  device->rx_queued = 0;
...
//	   device->cur_rx, read16(device->base+RxBufAddr), read16(device->base+RxBufPtr), read8(device->base + ChipCmd));
	...
	int32 ring_offset = device->cur_rx;
	uint32 rx_status = *(uint32*)(rx_ring + ring_offset);
	int rx_size = rx_status >> 16;
	
		if ((rx_size <= 0) || (rx_size > 1518)) {
			ETHER_DEBUG(ERR,device->debug, "RX LEN!, cur %4.4lx queued %4.4lx, BufAddr %4.4x, Cmd %2.2x.\n",
				device->cur_rx, device->rx_queued, read16(device->base + RxBufAddr), read8(device->base + ChipCmd));
			device->readLock = 0;
			*len = 0; device->cur_rx = device->rx_queued;
...
//		ETHER_DEBUG(WARN, device->debug, "rx len=%x rx_size=%x\n", *len, rx_size);
		if (rx_status & RxTooLong) {
				ETHER_DEBUG(ERR, device->debug, "Ethernet frame tooo big, status %4.4lx!\n",rx_status);
				*len = 0;
				status=B_ERROR;
				/* The chip hangs here?. */
		} else if (rx_status & (RxBadSymbol|RxRunt|RxTooLong|RxCRCErr|RxBadAlign)) {
				ETHER_DEBUG(WARN, device->debug, "Ethernet frame error status=%4.4lx.\n", rx_status);
			/* Reset the receiver, based on RealTek recommendation. (Bug?) */
#if 0
			device->cur_rx = device->rx_queued = 0;  write8(device->base + ChipCmd, CmdTxEnb);
			write8(device->base + ChipCmd, CmdRxEnb | CmdTxEnb);
			write32(device->base + RxConfig, (RX_FIFO_THRESH << 13) | (RX_BUF_LEN_IDX << 11) | (RX_DMA_BURST<<8));
			rx_init(device);
#endif
			*len = 0; status = B_ERROR;
		} else {
			if (ring_offset + rx_size + 4 > RX_BUF_LEN) {  /* frame wraps - copy in 2 parts */
				/* The calculations here a little more complex because *len is max 1514, but
				   the NIC uses a len of 1518 which includes the 4 byte CRC */
				int first_part = RX_BUF_LEN - ring_offset - 4;
				{
					memcpy(buf, &rx_ring[ring_offset + 4], min(MTU_SIZE,first_part));
					if (first_part < MTU_SIZE)
						memcpy(buf+first_part, &rx_ring[0], *len-first_part);
				}
		...
		device->cur_rx = ((device->cur_rx + rx_size + 4 + 3) & ~3) & RX_BUF_MASK;
		write16(device->base + RxBufPtr, device->cur_rx - 16);

		{	
//		int32 semcount;
//		int32 BufAddr = read16(device->base + RxBufAddr);
//		get_sem_count(device->ilock, &semcount);
		
//		ETHER_DEBUG(WARN,device->debug, "DoneRx, sem=%d cur %4.4x queued %4.4x, BufAddr %4.4x, Cmd %2.2x.\n",
//			semcount, device->cur_rx, device->rx_queued, BufAddr, read8(device->base + ChipCmd));
...
static status_t write_hook(void *_device,off_t pos,const void *buf,size_t *length)
{
	...
	ETHER_DEBUG(FUNCTION, device->debug, " write buf %p len %ld (decimal)\n", buf, *length);
...
//		if (device->rx_overflow) {
//			dprintf(" OVRFLW\n");
//			break;
//		}
		if (status & (RxOK|RxUnderrun|RxOverflow|RxFIFOOver)) {
			if (status & RxOverflow) {
				ETHER_DEBUG(ERR,device->debug,"RX OVERFLOW\n");
//				device->rx_overflow=true;
...
{		int32 semcount;
		...
		ETHER_DEBUG(ERR,device->debug, " sem=%d cur %4.4x queued %4.4x, BufAddr %4.4x, Cmd %2.2x.\n",
			semcount, device->cur_rx, device->rx_queued, BufAddr, read8(device->base + ChipCmd));
}
#endif
				/* clear RxOverflow !? */
				write16(device->base + IntrStatus, status & ~RxOverflow);
			...
			if ((read8(device->base + ChipCmd) & RxBufEmpty) == 0) {
				int ring_offset = device->rx_queued;
				uint32 rx_status = *(uint32 *)(device->rx_buf_virt[0] + ring_offset);
				int rx_size = rx_status >> 16;
				int BufAddr = read16(device->base + RxBufAddr);
				int rx_count = 0;
				if (BufAddr < ring_offset) BufAddr += RX_BUF_LEN; /* BufAddr wrapped */
				while ((ring_offset < BufAddr)) {
					if ((rx_size > 1518) || (rx_size < 60)) {
						ETHER_DEBUG(WARN, device->debug, "RX_SIZE=%x  cur=%lx q=%lx Buf=%x ring_offset=%x count=%d\n",
							rx_size, device->cur_rx, device->rx_queued, BufAddr, ring_offset, rx_count);

						break; /* this occurs when a frame is partially received */
					}
					rx_count++;
					ring_offset = ((ring_offset + rx_size + 4 + 3) & ~3);
					device->rx_queued = ((device->rx_queued + rx_size + 4 + 3) & ~3) & RX_BUF_MASK;
					rx_status = *(uint32 *)(device->rx_buf_virt[0] + (ring_offset & RX_BUF_MASK));
					rx_size = rx_status >> 16;
				}
//				ETHER_DEBUG(WARN, device->debug, "rx_isr-AFTER  cur=%x q=%x Buf=%x count=%d\n",
//					device->cur_rx, device->rx_queued, BufAddr, rx_count);
				if (rx_count) 
		...
		if (status & (PCIErr|PCSTimeout |RxUnderrun|RxFIFOOver|TxErr|RxErr)) {
				ETHER_DEBUG(ERR, device->debug, "ISR err %8.8x.\n", status);
		...
		if (--worklimit < 0) {
			ETHER_DEBUG(WARN, device->debug, "Too many interrupts status=0x%4.4x.\n",status);
			break;
		}
...
static status_t enable_addressing(dev_info_t *device)
{
	...
	ETHER_DEBUG(PCI_IO,  device->debug, "base=%lx\n", device->base);
...
static status_t init_ring_buffers(dev_info_t *device)
{
	...
	if ((device->tx_buf_area = create_area( kDevName "tx buffers", (void **) device->tx_buf_virt,
		...
		ETHER_DEBUG(ERR, device->debug, "create tx buffer area failed %lx \n", device->tx_buf_area);
	...
	for ( i = 0; i < TX_BUFFERS; i++) {
		...
		if (device->debug & WARN)
			dprintf(kDevName " tx_buf_virt[%3.3d]=%p tx_buf_phys[%3.3d]=%8.8lx\n",
	...
	if ((device->rx_buf_area = create_area( kDevName " rx buffers", (void **) device->rx_buf_virt,
		...
		ETHER_DEBUG(ERR, device->debug, "create rx buffer area failed %lx \n", device->rx_buf_area);
	...
	if (device->debug & WARN) {
		dprintf( kDevName "rx_buf_virt=%p rx_buf_phys=%8.8lx\n", device->rx_buf_virt[0], device->rx_buf_phys[0]);
	}	
...
//	ETHER_DEBUG(WARN, device->debug, "rx_init prev config = %8.8x.\n", read32(device->base + RxConfig));
	...
	// To Do: hash multicast addresses - up to 32 may be filtered on this hardware
	// Right now we accept all or no multicast addresses.
	if (device->nmulti) {
		dprintf(kDevName ": To do: add multicast hash filters\n");
	...
	/* Receiver and filters may be set without stopping the chip */
	write8(device->base + RxConfig, rx_mode);
	write32(device->base + MAR0 + 0, 0);
	write32(device->base + MAR0 + 4, 0);
...
static status_t allocate_resources(dev_info_t *device) {
	...
	/* Setup Semaphores */
	if ((device->ilock = create_sem(0, kDevName " rx")) < 0) {
		dprintf(kDevName " create rx sem failed %lx \n", device->ilock);
		return (device->ilock);
	}
	set_sem_owner(device->ilock, B_SYSTEM_TEAM);
	...
	if ((device->olock = create_sem(TX_BUFFERS-1, kDevName " tx")) < 0) {
		dprintf(kDevName " create tx sem failed %lx \n", device->olock);
		return (device->olock);
	}
...
{
	uint16 i,j;
	const char * usage = "usage: rtl8139 [globals,rx,tx,warnings]\n";	
	...
	for (i= argc, j= 1; i > 1; i--, j++) {
		switch (*argv[j]) {
		case 'g':
//			kprintf("rtl8139 mac %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
//				gdev->mac_address.ebyte[0], gdev->mac_address.ebyte[1], gdev->mac_address.ebyte=[2],
//				gdev->mac_address.ebyte[3], gdev->mac_address.ebyte[4], gdev->mac_address.ebyte=[5]);
			kprintf( "base addr=%lx nmulti=%x \n", gdev->base, gdev->nmulti);
			kprintf("tx_sent=%lx tx_acked=%lx\n",gdev->tx_sent, gdev->tx_acked);
			kprintf("tx_virt[0]=%p tx_virt[1]=%p tx_virt[2]=%p tx_virt[3]=%p\n",
					gdev->tx_buf_virt[0], gdev->tx_buf_virt[1], gdev->tx_buf_virt[2], gdev->tx_buf_virt[3]);
			kprintf("tx_phys[0]=%lx tx_phys[1]=%lx tx_phys[2]=%lx tx_phys[3]=%lx\n",
					gdev->tx_buf_phys[0], gdev->tx_buf_phys[1], gdev->tx_buf_phys[2], gdev->tx_buf_phys[3]);
			kprintf("tx_threshold= %lx\n", gdev->tx_threshold);
			kprintf("cur_rx=%lx rx_queued=%lx rx_ovfl=%lx\n",gdev->cur_rx, gdev->rx_queued, gdev->rx_overflow);				

			{
				int ring_offset = gdev->cur_rx & RX_BUF_MASK;
				//uint32 rx_status = *(uint32 *)(gdev->rx_buf_virt[0] + ring_offset);
				//int rx_size = rx_status >> 16;
				int BufAddr = read16(gdev->base + RxBufAddr);
				int BufPtr = read16(gdev->base + RxBufPtr);
				int i, len;
				
				kprintf("RxBufAddr=%x RxBufPtr=%x\n", BufAddr, BufPtr);
				if (BufAddr < ring_offset) BufAddr += RX_BUF_LEN;
				len = BufAddr - ring_offset;
				kprintf("RX Q DUMP cur=%lx que=%lx buf=%x len=%x\n",gdev->cur_rx,gdev->rx_queued,BufAddr,len);
				for (i = 0; i < RX_BUF_LEN; i+=4) {
					if (((ring_offset+i) & 0x1f) == 0) kprintf("\n%8.8x: ",(ring_offset+i) & RX_BUF_MASK);
					kprintf("%8.8lx ", *(uint32 *)(gdev->rx_buf_virt[0] + ((ring_offset + i) & RX_BUF_MASK)));
				}
			}
			break; 
		case 'b':
			kprintf("bump sem by 10\n");
				release_sem_etc(gdev->olock, 1, 0);
			break; 
		case 'r':
			gdev->debug ^= RX;
			if (gdev->debug & RX) 
...
fixed_driver_info rtl8139_driver=
{
	"Realtek 8139 driver",
...
REGISTER_STATIC_DRIVER(rtl8139_driver);
#endif
