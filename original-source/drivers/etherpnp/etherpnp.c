...
/*
 Add some stuff to suport pnp - Russ
*/
static uint8 debug = 1;  /* 1 is the shipping default. 5 turns on all. */
int	g_init;				/* already inited? */
short g_irq_level;		/* our ISA IRQ level */
short g_port;	
           ...
           )
{
	...
	/* From init_driver() the ether_probe fuction is called to get the pnp irq and
	   port. The info is saved in g_irq and g_port. This is lame and should use the
	   the global device list and multiple card support that tlan, hamachi, eep100
	   all use. That support should be added in the next go around. This round I
	   just wanted the device to use pnp then publish itself. */
	
	data->irq_level = g_irq_level; 
	data->port = g_port;
	data->mem = 0;
	
	if (debug > 3)
		dprintf("ether: probing at port %03x, mem %08x (irq %d).\n", g_port, 0, g_irq_level);
	...
	ports_for_irqs[g_irq_level] = g_port; // list
	install_io_interrupt_handler(g_irq_level, ether_interrupt, data, 0);	
...
static int ether_probe()
{
	 ...
	 */
	if (g_init) {
		dprintf("already inited!\n");
		return (B_ERROR);
	}

    error = get_module(B_CONFIG_MANAGER_FOR_DRIVER_MODULE_NAME, (module_info **)&module);
	if (error != B_OK) {
		dprintf("Error getting configuration manager module\n");
		return B_ERROR;
	}

    /* See the Device manager config info panel. VendorID, Product and Revision
	   are used below. */ 
	MAKE_EISA_PRODUCT_ID((EISA_PRODUCT_ID *)&pid1, 'A', 'X', 'E', 0x220, 1);	
	MAKE_EISA_PRODUCT_ID((EISA_PRODUCT_ID *)&pid2, 'D', 'L', 'K', 0x220, 1);
	
	
	while (module->get_next_device_info(B_ISA_BUS, &cookie, &info.info, sizeof(info)) == B_OK) {
		struct device_configuration *config;
		resource_descriptor r;
		status_t size;

//dprintf("ETHERPNP: get_nth_isa %x\n", (EISA_PRODUCT_ID)info.vendor_id);

		/* match the vendor id */
		if ((!EQUAL_EISA_PRODUCT_ID((EISA_PRODUCT_ID)pid1, (EISA_PRODUCT_ID)info.vendor_id)))
			 if (!EQUAL_EISA_PRODUCT_ID((EISA_PRODUCT_ID)pid2, (EISA_PRODUCT_ID)info.vendor_id))
				 if (!(0x6000d041 == info.vendor_id))	/* realtek 8019AS */
					continue;

		/* found a card */
		size = module->get_size_of_current_configuration_for(cookie);
		if (size < 0) {
			dprintf("Error getting size of configuration\n");
			continue;
		}
		...
		error = module->get_current_configuration_for(cookie, config, size);
		if (error < B_OK) {
			dprintf("Error getting current configuration\n");
			free(config);
			continue;
		}

		/* sanity check card */
		if ((module->count_resource_descriptors_of_type(config, B_IRQ_RESOURCE) != 1) ||
				(module->count_resource_descriptors_of_type(config, B_DMA_RESOURCE) != 0) ||
				(module->count_resource_descriptors_of_type(config, B_IO_PORT_RESOURCE) != 1) ||
				(module->count_resource_descriptors_of_type(config, B_MEMORY_RESOURCE) != 0)) {
			dprintf("Bad resource counts for NE2000\n");
			free(config);
			continue;
		}

		error = module->get_nth_resource_descriptor_of_type(config, 0, B_IRQ_RESOURCE, &r, sizeof(r));
		if (error < 0) {
			dprintf("Error getting IRQ\n");
			free(config);
			continue;
		}

		g_irq_level = log2(r.d.m.mask);
		
		error = module->get_nth_resource_descriptor_of_type(config, 0, B_IO_PORT_RESOURCE, &r, sizeof(r));
		free(config);
		if (error < 0) {
			dprintf("Error getting I/O range\n");
			continue;
		}

		if (r.d.r.len != 0x20) {
			dprintf("I/O range must be len 0x20\n");
			continue;
		}

		g_port = r.d.r.minbase;
		g_init = true;
		
		// Only picking up first card now.
		return (B_NO_ERROR);
	}

	/* couldn't find a NE2000 card that we know about */
	error = B_ERROR;
...
init_driver()
{
	...
	get_module(isa_name, (module_info **)&isa);
	if (ether_probe() != B_NO_ERROR)
		return B_ERROR;
	else
		return B_NO_ERROR;
}
...
static char *ether_name[] = { "net/etherpnp/0", NULL };
