...
status_t 
send_binder_request(binder_node *node, binder_request_basic *request,
                    size_t request_size, port_pool **reply_port)
{
	status_t err;
	bigtime_t timeout = BINDER_REQUEST_TIMEOUT;
	port_id outPort = node->nodeid.port;

	if(reply_port != NULL && is_local(node->nodeid.port)) {
		dprintf("binder(%d) -- send_binder_request: WARNING port %d "
		        "owned by current team\n"
		        "                  waitReply %d, port %d, thread %d, cmd %d\n",
		        (int)find_thread(NULL), (int)node->nodeid.port,
		        reply_port != NULL,
		        (int)node->nodeid.port, (int)find_thread(NULL),
		        request->type);
	...
	if(reply_port == NULL)
		...
		if(*reply_port != NULL) {
			dprintf("binder(%d) -- send_binder_request: *reply_port not NULL,"
	...
	request->token = node->nodeid.token;
	
#if TRACE_COMMUNICATION
	dprintf("binder(%d) -- send_binder_request: write command to port %d\n",
	        (int)find_thread(NULL), (int)outPort);
#endif	
#if TRACE_COMMUNICATION_DATA
	{
		uint8 *datap = (uint8*)request;
		unsigned int i = 0;
		dprintf("binder(%d) -- send_binder_request: send data, len %d",
		        (int)find_thread(NULL), (int)request_size);
		while(i < request_size) {
			if(i % 16 == 0)
				dprintf("\n    %02x:", i);
			dprintf(" %02x", *datap++);
			i++;
	...
	if(err < B_OK) {
		if(reply_port != NULL)
			release_port(*reply_port);
		dprintf("binder(%d) -- send_binder_request: port write on %d failed, "
...
status_t 
send_binder_request_no_arg(binder_node *node, binder_request_type type,
...
status_t 
send_binder_request_one_arg(binder_node *node, binder_request_type type,
                            uint32 arg, port_pool **reply_port)
{
	binder_request_one_arg req;
	req.basic.type = type;
	req.arg = arg;
	return send_binder_request(node, &req.basic, sizeof(req), reply_port);
}

status_t 
send_binder_request_name_and_data(binder_node *node, binder_request_type type,
                                  const char *string, void *data,
                                  size_t data_size, port_pool **reply_port)
{
	status_t err;
	...
	err = send_binder_request(node, &reqp->basic, request_size, reply_port);
	if(reqp != &small_req)
		free(reqp);
	return err;
}
...
static status_t 
internal_get_binder_reply(binder_node *node, port_pool *reply_port,
                          ...
                          size_t *reply_buf_size)
{
	...
	if(reflection->active) {
		dprintf("binder(%d) -- get_binder_reply: reflection already active\n",
		        (int)find_thread(NULL));
		return B_OK;
	}
	if(!is_port_valid(reply_port)) {
		dprintf("binder(%d) -- get_binder_reply: bad reply port\n",
		        (int)find_thread(NULL));
		return B_ERROR;
	}
...
#if TRACE_COMMUNICATION
	dprintf("binder(%d) -- get_binder_reply: wait for reply on port %d\n",
	        (int)find_thread(NULL), (int)reply_port->port);
#endif
	err = size = port_buffer_size_etc(reply_port->port,
	                                  B_TIMEOUT /*| B_CAN_INTERRUPT*/, timeout);
	if(err < B_OK) {
		dprintf("binder(%d) -- get_binder_reply: port stat on %d failed "
		        "(request on %d), %s\n",
		        (int)find_thread(NULL), (int)reply_port->port,
		        (int)node->nodeid.port, strerror(size));
		goto err;
	}
	if(size > 0) {
		if(*reply_buf == NULL) {
			...
			*reply_buf = (binder_reply_basic *)malloc(size);
			if(*reply_buf == NULL) {
				dprintf("binder(%d) -- get_binder_reply: malloc failed for "
				        "reply size %d\n",
			...
			if(size != (ssize_t)*reply_buf_size) {
				dprintf("binder(%d) -- get_binder_reply: bad reply size %d "
	...
	err = read_port_etc(reply_port->port, &type, *reply_buf, size,
	                    B_TIMEOUT | B_CAN_INTERRUPT, timeout);
	if(err != size) {
		dprintf("binder(%d) -- get_binder_reply: port read on %d failed "
		        "(request on %d), %s\n",
		        (int)find_thread(NULL), (int)reply_port->port,
		        (int)node->nodeid.port, strerror(err));
		if(err >= 0)
			err = B_ERROR;
		goto err;
	}
#if TRACE_COMMUNICATION_DATA
	{
		uint8 *datap = (uint8*)*reply_buf;
		int i = 0;
		dprintf("binder(%d) -- get_binder_reply: got data, len %d",
		        (int)find_thread(NULL), (int)size);
		while(i < size) {
			if(i % 16 == 0)
				dprintf("\n    %02x:", i);
...
#if TRACE_REFLECTION
		dprintf("binder(%d) -- get_binder_reply: reflect punt\n",
		        (int)find_thread(NULL));
#endif
		add_reflection_command(reflection, REFLECT_CMD_PUNT, reply_port->port);
		return B_OK;
	}
	if(type != BINDER_REPLY_CODE) {
		dprintf("binder(%d) -- get_binder_reply: invalid message type, %d, "
		        "on replyport %d\n",
	...
	if((*reply_buf)->sync_port >= 0)
		register_reply_thread((*reply_buf)->sync_port);
	else
		release_port(reply_port);
	//if(!do_not_release)
...
err:
	pp_delete_port(reply_port);
	return err;
}

status_t
get_binder_reply_dynamic(binder_node *node, port_pool *reply_port,
                         reflection_struct *reflection,
...
status_t 
get_binder_reply(binder_node *node, port_pool *reply_port,
                 reflection_struct *reflection,
...
status_t 
get_binder_reply_status(binder_node *node, port_pool *reply_port,
                        reflection_struct *reflection)
{
	status_t err;
	binder_reply_status reply;
	binder_reply_basic *reply_buf = &reply.basic;
