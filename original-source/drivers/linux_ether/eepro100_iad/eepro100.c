...
/* 
 * Intel EtherExpress driver written for Linux adapted for 
 * BeOS works with i82557 and i82558 boards. 
 *  
 * Revision history: 
 *           	7/15/98 written for v3.1. by 3rd party
 *				Takekazu Okamoto tokamoto@novell.co.jp based
 *              on sample driver
 * 				9/20/98 update to support v4 pci modules and work
 *				with updated wrapper by russ
 *
 * Important Note:
 *              This code was taken from GPL and links with BeOS kernel
 *              it should in no way (penalty is death) go out from the
 *              network team. We are negotiating with Donald Becker, this
 *              message will be removed once Be owns the code. Questions?
 *              see or email russ, hank, howard @be.com
 *
 */
...
#include <PCI.h> /* russ++ */
extern pci_module_info	*pci; /* russ++ */

struct pt_regs;
struct ifreq;

#define virt_to_bus(a) ex_virt_to_bus(a) /* russ++ */

/* unsigned long virt_to_bus(volatile void * address) russ-- */
unsigned long ex_virt_to_bus(void * address) /* russ++ */
{
	physical_entry	table[1];

	get_memory_map(address, 2000, table, 1);
	/* return (unsigned long)ram_address(table[0].address);  russ-- */
	return (unsigned long)(*pci->ram_address)(table[0].address); /* russ++ */
/*	return (unsigned long)address; */
}

/* 
* Was removed in v4 wrapper, and I want to keep the wapper pure so
* added kfree call. russ++
*/
void kfree(void * obj)
{
	free(obj);
...
#if defined(__BEOS__) || (LINUX_VERSION_CODE < 0x20123)
/* TOkamoto #if (LINUX_VERSION_CODE < 0x20123) */
...
#if __BEOS__
	u32 v_rx_buf_addr[RX_RING_SIZE];
#endif
...
#if __BEOS__
static int speedo_init(struct device *dev) {return 0;}
#endif
...
static int32 speedo_interrupt(int irq, void *dev_instance, struct pt_regs *regs);
...
/*
* Cleaned up complicated if __BEOS__, in v4 pcibios_read_config_x
* is the same as linux. The linux wrapper was changed. russ--
*/
...
#if (!defined(__BEOS__) && LINUX_VERSION_CODE >= 0x20155) || PCI_SUPPORT_1
/* TOkamoto #if LINUX_VERSION_CODE >= 0x20155  ||  PCI_SUPPORT_1 */
		{
...
#if 0 || __BEOS__
{
	...
	lock_memory(self_test_results, sizeof(s32)*6 - (self_test_results - str), B_DMA_IO);
	get_memory_map(self_test_results, sizeof(s32)*6 - (self_test_results - str), table, 1);
	/* where = ram_address(table[0].address); russ-- */ 
	where = (*pci->ram_address)(table[0].address); /* russ++ */
	outl(((unsigned long)where) | 1, ioaddr + SCBPort);
#else
...
#if 0 || __BEOS__
	unlock_memory(self_test_results, sizeof(s32)*6 - (self_test_results - str), B_DMA_IO);
}
#endif
...
speedo_open(struct device *dev)
{
...
#if __BEOS__
	lock_memory(sp, sizeof(*sp), B_DMA_IO+B_READ_DEVICE);
#endif
#ifdef notdef
...
#if 0 /* where did this come from? it breaks things! */
...
#endif /* where did this come from? it breaks things! */
	...
	/* Set the timer.  The timer serves a dual purpose:
	   ...
	   hangs. */
	linux_init_timer(&sp->timer);
	sp->timer.expires = RUN_AT((24*HZ)/10); 			/* 2.4 sec. */
	sp->timer.data = (unsigned long)dev;
	sp->timer.function = &speedo_timer;					/* timer handler */
	linux_add_timer(&sp->timer);
...
speedo_init_rx_ring(struct device *dev)
{
    ...
    ether_setup(dev); /* v3.1 used static buffers, v4 uses areas russ++ */
...
#endif
#if __BEOS__
		sp->v_rx_buf_addr[i] = (u32)skb->tail;
#endif
		rxf->count = 0;
		rxf->size = PKT_BUF_SZ;
	}
...
/* TOkamoto #if 1 || USE_IP_CSUM */
...
#if	__BEOS__
					   (char *)sp->v_rx_buf_addr[entry], pkt_len);
#else
...
#if	__BEOS__
				if ((char *)sp->v_rx_buf_addr[entry] != temp)
#else
	...
	for (; sp->dirty_rx < sp->cur_rx; sp->dirty_rx++) {
		...
		if (sp->rx_skbuff[entry] == NULL) {
			...
			rxf->rx_buf_addr = virt_to_bus(skb->tail);
#if __BEOS__
			sp->v_rx_buf_addr[entry] = (u32)skb->tail;
#endif
		} else {
...
/* hmm, these don't seem to be implemented in the linux wrapper.
	this is probably a bug   - Howard      */
