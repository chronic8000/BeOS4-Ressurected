#if __BEOS__
/* 
 * Linux driver adapted for BeOS all changes to support
 * Be should go between __BEOS__ idefs. It makes editing
 * this driver much easier. This driver code has been purchased
 * from Donald Becker of Linux fame. See his notes below.
 */
#include "fakeout.h"

#define VORTEX_DEBUG 1
#define DISABLE_ADV_905B 1
...
// BEOS  NIC's regs (p.2-9)
#define OFS_TXSTATUS	0x1b
#define OFS_DMACTRL		0x20
#define  DISABLE_UPALT	(1<<16) 		// (p. 6-16)
#define  DEFEAT_MWI 	(1<<20)
#define  DEFEAT_MRL		(1<<21)
#define OFS_TXPOLL		0x2d
...
static char *version = "3c9xx(B) Be99H-WOL 2/24/99\n";
...
#endif
#ifdef __BEOS__
#define le32_to_cpu(val) (val) // Intel only define
#define cpu_to_le32(val) (val) // Intel only define
#define test_and_set_bit(val, addr) set_bit(val, addr)
void tx_diags(int);
#endif //BEOS
...
static struct pci_id_info pci_tbl[] = {
	...
	{"3c900 Boomerang 10Mbps Combo", 0x10B7, 0x9001, 0xffff,
	 PCI_USES_IO|PCI_USES_MASTER, IS_BOOMERANG, 64, vortex_probe1},
#ifdef __BEOS__
	{"3c900B Cyclone 10baseT",	0x10B7, 0x9004, 0xffff,
	...
	{"3c905B-FX Cyclone 100baseFx",	0x10B7, 0x905A, 0xffff,
	 PCI_USES_IO|PCI_USES_MASTER, IS_CYCLONE, 128, vortex_probe1},
#ifdef __BEOS__
	{"3c905C-TX Tornado 100baseTx",	0x10B7, 0x9200, 0xffff,
	 PCI_USES_IO|PCI_USES_MASTER, IS_CYCLONE, 128, vortex_probe1},
#endif /*__BEOS__*/
...
struct pt_regs;
struct ifreq;
//#define SIOCDEVPRIVATE 69 // Russ added so it would compile.
static int vortex_init(struct device *dev) {return 0;}
#endif
...
#ifdef __BEOS__
static uint32 vortex_interrupt(int irq, void *dev_id, struct pt_regs *regs);
#else
...
#ifdef __BEOS__
	{
		int cards_found = vortex_scan(0, pci_tbl);
		
		if (cards_found == 0)
			printk("No 3Com Vortex/Boomerang cards found.\n");
		printk("Found 3Com Vortex/Boomerang cards found.%d\n", cards_found);	
...
#ifndef __BEOS__ // No BEOS Card bus 5/12/99
...
vortex_open(struct device *dev)
{
	...
	if (vp->full_bus_master_rx) { /* Boomerang bus master. */
		...
		// Store for ring[].next
		// We do it here because these entries are static,
		// unlike skbufs which are dynamic
		printk("Storing TxRing\n");
		for (i = 0; i <= TX_RING_SIZE; i++) {
			ring_map_n_store_addr((uchar *) &vp->tx_ring[i],
							sizeof(struct boom_tx_desc));
		}
		printk("Storing RxRing\n");
		for (i = 0; i <= RX_RING_SIZE; i++) {
			ring_map_n_store_addr((uchar *) &vp->rx_ring[i],
							sizeof(struct boom_rx_desc));
		}

		// compatibility mode for the Cyclone
#if DISABLE_ADV_905B
		dma_ctrl = inl(ioaddr + OFS_DMACTRL);
		printk("DmaCtrl was=%8x, ", dma_ctrl);
		// if(bits & pieces)
		dma_ctrl = dma_ctrl | DEFEAT_MWI | DEFEAT_MRL | DISABLE_UPALT;
		outl(dma_ctrl, ioaddr + OFS_DMACTRL);
		printk("now=%8x\n", dma_ctrl);
#endif
#endif // BEOS
...
#ifdef __BEOS__
			vp->rx_ring[i].next = ring_virt_to_bus(&vp->rx_ring[i+1]); 
#else
...
#ifdef __BEOS__
		vp->rx_ring[i-1].next = ring_virt_to_bus(&vp->rx_ring[0]);  
		outl(ring_virt_to_bus(&vp->rx_ring[0]), ioaddr + UpListPtr);
#else	
...
#ifdef __BEOS__
			outl(ring_virt_to_bus(&vp->tx_ring[vp->dirty_tx % TX_RING_SIZE]), 
													 ioaddr + DownListPtr);
#else	
...
#ifdef __BEOS__
		prev_entry->next = ring_virt_to_bus(&vp->tx_ring[entry]);
#else
...
#ifdef __BEOS__
			outl(ring_virt_to_bus(&vp->tx_ring[entry]), ioaddr + DownListPtr);
#else
...
#ifdef __BEOS__
					ring_virt_to_bus(&vp->tx_ring[entry]))
#else				
...
#ifdef __BEOS__
				if (skb == NULL) {
					dprintf("3Com driver: NULL BUFFER - this should never happen\n");	
				} else {
#endif /*__BEOS__*/
