...
#define LARGE_BLOCKS	1	// 1 * 32 = 32 ( 64 KB total )
...
status_t init_mempool( void )
{
	...
	if( (mp.dataAreaID = create_area( "prism net_mem_pool", &mp.areaBase, B_ANY_KERNEL_ADDRESS, size, B_FULL_LOCK, B_READ_AREA | B_WRITE_AREA )) < 0 )
...
void *new_buf( size_t size )
{
	if( size <= LARGE_BSIZE )
		...
		dprintf("new_buff %d > %d - CANNOT ALLOCATE!\n", size, LARGE_BSIZE);
		return NULL;
	}
}
