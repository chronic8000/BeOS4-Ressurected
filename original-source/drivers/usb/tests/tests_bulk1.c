...
static void init_sin_buf_4(void* buf, size_t num_samples)
{
	int		i;
	int16*	ptr = buf;
	double	x;
	...
	static int16 a[4] = {0, 7500, 0, -7500};
	...
	for(i=0; i<num_samples; i++)
	{
		...
		ptr[4*i] = ptr[4*i+1] = ptr[4*i+2] = ptr[4*i+3] = s; 
	}
}
...
static void
put_sin_buf_4(void* buf, size_t num_samples)
{
	int		i;
	int16*	ptr = buf;
	static double	x = 0.0;
...
static void memcpy_1_2(void* dest, const void* src, size_t src_size)
{
	int i;
...
get_data_size(const rlea* rle_array)
{
	...
	return s*4;	/* sample size is 4 bytes */
}
...
#if SIN_OUTPUT
		if(!is_in)
			init_sin_buf_4(ch->iso_packets[pn].buffer, buf_size/8);
#endif
...
queue_packet_out(iso_channel* ch)
{
	...
	if(s = usb->queue_bulk(ch->ep, packet->buffer, qs_out,
...
queue_packet_in(iso_channel* ch)
{
	...
	if(s = usb->queue_bulk(ch->ep, packet->buffer, qs_in,
		cb_notify_in, packet))
	{
	...
	for(i=0;i<PDEV_MAX;i++){
		if(!pdev[i]){
			if(p = (testsdev *) malloc(sizeof(testsdev))){
				...
				in = conf->interface[0].active->endpoint[0].handle;
				out = conf->interface[0].active->endpoint[1].handle;
...
void cb_notify_in(void *cookie, uint32 status, void *data, uint32 actual_len)
{
	...
	if(status != 0)
		dprintf(ID "packet %p status %d, actual %d\n", packet, status, actual_len);
		//dprintf("s%d\n", status);

	packet->status = status;
	if (0)  // if(packet->rle_array->num_valid != 1)
	{
	...
	{
		data_size = packet->buffer_size = actual_len; //get_data_size(packet->rle_array);
		qs_out = data_size*2;
	}
	...
	if(status == 0)
	{
		...
		for(; ptr<(uint8*)buf+actual_len; ptr += 64)
		{
			...
			if(++magic != B_BENDIAN_TO_HOST_INT16(*((uint16*)ptr)) )
			{
				...
				for(i=0,ptr = buf; ptr<(uint8*)buf+actual_len; ptr += 64,i++)
				{
					if((i & ((qs_in-1)/64)) == 0)
						dprintf("\n");
					if((i & 0xF) == 0)
...
tests_read(void *cookie, off_t pos, void *buf, size_t *count)
{
	...
	data_size = channel->current_rw->buffer_size; //get_data_size(channel->current_rw->rle_array);
...
tests_write(void *cookie, off_t pos, const void *buf, size_t *count)
{
	...
	*count = 0;
	...
	if(channel->remain >= 2*(*count))
	{
		memcpy_1_2(channel->buf_ptr, buf, *count);
		channel->remain -= 2*(*count);
		channel->buf_ptr += 2*(*count);
	}
	else
	{
		memcpy_1_2(channel->buf_ptr, buf, channel->remain/2);
		*count = channel->remain/2;
		channel->remain = 0;
