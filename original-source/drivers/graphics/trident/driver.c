/* :ts=8 bk=0
 *
 * driver.c:	Core driver for Intel Trident card.
 *
...
static void clearinterrupts (register struct tri_card_info *ci);
static void enableinterrupts (register struct tri_card_info *ci,
			      int enable);
static int32 trident_interrupt (void *data);
...
static status_t setdpmsmode (register union vgaregs *vregs, int mode);
static int tridump (int argc, char **argv);
...
init_locks (struct devinfo *di, team_id team)
{
	 ...
	 */
	if ((retval = create_sem (1, "Trident CRTC lock")) < 0)
		return (retval);
	ci->ci_CRTCLock = retval;
	set_sem_owner (retval, team);

	if ((retval = create_sem (1, "Trident CLUT lock")) < 0)
		return (retval);
	ci->ci_CLUTLock = retval;
	set_sem_owner (retval, team);

	/*  VBlank semaphore gets zero thread count  */
	if ((retval = create_sem (0, "Trident VBlank semaphore")) < 0)
		return (retval);
...
{
	tri_card_info	*ci = di->di_TriCardInfo;
	status_t	retval;
	uint32		mappedio;
	int		len;
	...
	strcpy (name + len, " 3D regs");
	retval =
	 map_physical_memory (name,
			      (void *) di->di_PCI.u.h0.base_registers[2],
			      di->di_PCI.u.h0.base_register_sizes[2],
			      B_ANY_KERNEL_ADDRESS,
			      B_READ_AREA | B_WRITE_AREA,
			      (void **) &ci->ci_BaseAddr2);
	if (retval < 0)
		goto err_base2;	/*  Look down  */
	di->di_BaseAddr2ID = retval;

	if (retval < 0) {
		delete_area (di->di_BaseAddr2ID);
		di->di_BaseAddr2ID = -1;
		ci->ci_BaseAddr2 = NULL;
err_base2:	delete_area (di->di_BaseAddr1ID);
		di->di_BaseAddr1ID = -1;
...
{
	register tri_card_info *ci = di->di_TriCardInfo;
...
probe_devices (void)
{
	...
	while ((count < MAXDEVS)  &&
	       ((*pci_bus->get_nth_pci_info)(pci_index, &(di->di_PCI)) ==
	        B_NO_ERROR))
	{
		if (di->di_PCI.vendor_id == VENDORID_TRIDENT  &&
		    (di->di_PCI.device_id == DEVICEID_CYBERBLADE_I7  ||
		     di->di_PCI.device_id == DEVICEID_CYBERBLADE_I7_MVP4))
		{
			/*
			...
			sprintf (di->di_Name,
				 ...
				 di->di_PCI.function);
			tridd->dd_DevNames[count] = di->di_Name;
dprintf (("+++ making /dev/%s\n", di->di_Name));

			/*  Initialize to...  uh...  uninitialized.  */
			di->di_BaseAddr0ID		=
			di->di_BaseAddr1ID		=
			di->di_TriCardInfo_AreaID	= -1;
			di->di_NInterrupts		=
			di->di_NVBlanks			=
			di->di_LastIrqMask		= 0;
 ...
 */
static void
clearinterrupts (register struct tri_card_info *ci)
{
	/*
...
static void
enableinterrupts (register struct tri_card_info *ci, int enable)
{
}

static int32
trident_interrupt (void *data)
{
	...
	if (bits) {
		...
		dprintf (("+++ Weird Trident interrupt (tell ewhac): \
IIR = 0x%08lx\n", bits));
...
init_hardware (void)
{
	...
	for (idx = 0;
	     ...
	     idx++)
	{
		if (pcii.vendor_id == VENDORID_TRIDENT  &&
		    (pcii.device_id == DEVICEID_CYBERBLADE_I7  ||
		     pcii.device_id == DEVICEID_CYBERBLADE_I7_MVP4))
		{
...
init_driver (void)
{
	...
	if (!(tridd = (driverdata *) calloc (1, sizeof (driverdata)))) {
		retval = B_NO_MEMORY;
		goto err1;	/*  Look down  */
	}

	if ((retval = BInitOwnedBena4 (&tridd->dd_DriverLock,
				       "Trident kernel driver",
				       B_SYSTEM_TEAM)) < 0)
	{
		free (tridd);
...
#if (DEBUG  ||  FORCE_DEBUG)
	add_debugger_command ("tridump",
			      tridump,
			      "tridump - dump Trident kernel driver data");
...
find_device (const char *name)
{
	...
	for (index = 0;  tridd->dd_DevNames[index];  index++) {
		if (strcmp (name, tridd->dd_DevNames[index]) == 0)
...
dprintf (("+++ uninit_driver\n"));
	...
	if (tridd) {
		...
		remove_debugger_command ("tridump", tridump);
#endif
		BDisposeBena4 (&tridd->dd_DriverLock);
		free (tridd);
		tridd = NULL;
	}
...
dprintf (("+++ firstopen(0x%08lx)\n", di));
	...
	n = (sizeof (struct tri_card_info) + B_PAGE_SIZE - 1) / B_PAGE_SIZE;
	if ((retval = create_area (shared_name,
				   (void **) &di->di_TriCardInfo,
				   B_ANY_KERNEL_ADDRESS,
				   n * B_PAGE_SIZE,
				   B_FULL_LOCK,
				   B_READ_AREA | B_WRITE_AREA)) < 0)
		goto err0;	/*  Look down  */
	di->di_TriCardInfo_AreaID = retval;

	/*  Read/write globals  */
	strcpy (shared_name + len, " (R/W)");
	n = (sizeof (struct tri_card_ctl) + B_PAGE_SIZE - 1) / B_PAGE_SIZE;
	if ((retval = create_area (shared_name,
				   (void **) &di->di_TriCardCtl,
				   B_ANY_KERNEL_ADDRESS,
				   n * B_PAGE_SIZE,
				   B_FULL_LOCK,
				   B_READ_AREA | B_WRITE_AREA)) < 0)
		goto err1;	/*  Look down  */
	di->di_TriCardCtl_AreaID = retval;
	...
	if ((retval = trident_init (di)) < 0) {
		(gpm->gpm_FreePoolInfo) (gfbpi);
		gfbpi = NULL;
err3:		unmap_device (di);
err2:		dispose_locks (di);
		if (di->di_TriCardCtl_AreaID >= 0) {
			delete_area (di->di_TriCardCtl_AreaID);
			di->di_TriCardCtl_AreaID = -1;
			di->di_TriCardCtl = NULL;
		}
err1:		if (di->di_TriCardInfo_AreaID >= 0) {
			delete_area (di->di_TriCardInfo_AreaID);
			di->di_TriCardInfo_AreaID = -1;
			di->di_TriCardInfo = NULL;
		}
err0:		return (retval);
	}
	...
	install_io_interrupt_handler
	 (di->di_PCI.u.h0.interrupt_line, trident_interrupt, (void *) di, 0);
...
dprintf (("+++ lastclose() begins...\n"));
	...
	enableinterrupts (di->di_TriCardInfo, FALSE);
	clearinterrupts (di->di_TriCardInfo);
	remove_io_interrupt_handler (di->di_PCI.u.h0.interrupt_line,
				     trident_interrupt, di);

	if (di->di_TriCardInfo->ci_PoolID)
		(gpm->gpm_DeletePool) (di->di_TriCardInfo->ci_PoolID);
...
dprintf (("+++ open_hook(%s, %d, 0x%08x)\n", name, flags, cookie));
	...
	BLockBena4 (&tridd->dd_DriverLock);

	/*  Find the device name in the list of devices.  */
	/*  We're never passed a name we didn't publish.  */
	for (n = 0;
	     tridd->dd_DevNames[n]  &&  strcmp (name, tridd->dd_DevNames[n]);
	     n++)
		;
	di = &tridd->dd_DI[n];
...
err:	BUnlockBena4 (&tridd->dd_DriverLock);
...
dprintf (("+++ free_hook() begins...\n"));
	...
	BUnlockBena4 (&tridd->dd_DriverLock);
...
//if (msg != MAXIOCTL_TRIDENT + B_SET_CURSOR_SHAPE  &&
//    msg != MAXIOCTL_TRIDENT + B_SHOW_CURSOR)
//     dprintf (("+++ ioctl: %d, buf: 0x%08x, len: %d\n", msg, buf, len));
	...
	case B_GET_ACCELERANT_SIGNATURE:
		strcpy ((char *) buf, "trident.accelerant");
		retval = B_OK;
		break;

#if _SUPPORTS_FEATURE_POWER_SAVE
	case B_SET_POWERSAVE:
		retval = setdpmsmode (&ci->ci_Regs->loregs.vga,
	 ...
	 */
	case TRIDENT_IOCTL_GETGLOBALS:
#define	GG	((trident_getglobals *) buf)

		/*
		 * Check protocol version to see if client knows how to talk
		 * to us.
		 */
		if (GG->gg_ProtocolVersion == TRIDENT_IOCTLPROTOCOL_VERSION) {
			GG->gg_GlobalArea_CI = di->di_TriCardInfo_AreaID;
			GG->gg_GlobalArea_CC = di->di_TriCardCtl_AreaID;
			retval = B_OK;
		}
		break;

	case TRIDENT_IOCTL_GET_PCI:
#define	GSP	((trident_getsetpci *) buf)
	 ...
	 */
	/*  Mode configuration  */
	case MAXIOCTL_TRIDENT + B_ACCELERANT_MODE_COUNT:
		*(uint32 *) buf = di->di_NDispModes;
		retval = B_OK;
		break;

	case MAXIOCTL_TRIDENT + B_GET_MODE_LIST:
		memcpy (buf,
			di->di_DispModes,
	...
	case MAXIOCTL_TRIDENT + B_PROPOSE_DISPLAY_MODE:
#define	PDM	((trident_propose_display_mode *) buf)

		retval = propose_video_mode
			  (di, PDM->pdm_Target, PDM->pdm_Lo, PDM->pdm_Hi);
		break;

	case MAXIOCTL_TRIDENT + B_SET_DISPLAY_MODE:
#define	SDM	((trident_set_display_mode *) buf)
	...
	case MAXIOCTL_TRIDENT + B_GET_FRAME_BUFFER_CONFIG:
#define	FBI	((frame_buffer_config *) buf)
	...
	case MAXIOCTL_TRIDENT + B_GET_PIXEL_CLOCK_LIMITS:
	 {
#define	PCL	((trident_pixel_clock_limits *) buf)
	...
	case MAXIOCTL_TRIDENT + B_SET_INDEXED_COLORS:
	 {
#define SIC	((trident_set_indexed_colors *) buf)	
		register int	count, idx;
		register uint8	*color;

		/*
		 * The chip can optionally display 16- and 32-bit pixels in
		...
		if (ci->ci_CurDispMode.space == B_CMAP8) {
			...
			while (count--) {
				setpaletteentry (&ci->ci_Regs->loregs.vga,
						 idx++,
						 color[0],
						 color[1],
						 color[2]);
	...
	/*  DPMS management  */
	case MAXIOCTL_TRIDENT + B_DPMS_MODE:
		switch (IDXREG_R
		         (&ci->ci_Regs->loregs.vga, VGA_GR, GR_PWRSTATUS) &
			FIELDMASK (GR_PWRSTATUS, DPMS))
		{
		case FIELDDEF (GR_PWRSTATUS, DPMS, ON):
			*(uint32 *) buf = B_DPMS_ON;
			break;
		case FIELDDEF (GR_PWRSTATUS, DPMS, STANDBY):
			*(uint32 *) buf = B_DPMS_STAND_BY;
			break;
		case FIELDDEF (GR_PWRSTATUS, DPMS, SUSPEND):
			*(uint32 *) buf = B_DPMS_SUSPEND;
			break;
		case FIELDDEF (GR_PWRSTATUS, DPMS, OFF):
			*(uint32 *) buf = B_DPMS_OFF;
			break;
		}
		retval = B_OK;
	...
	case MAXIOCTL_TRIDENT + B_SET_DPMS_MODE:
		if ((retval = setdpmsmode (&ci->ci_Regs->loregs.vga,
					   *(uint32 *) buf)) == B_OK)
		{
			ci->ci_DPMSMode = *(uint32 *) buf;
		}
		break;


	/*  Cursor managment  */
	case MAXIOCTL_TRIDENT + B_SET_CURSOR_SHAPE:
#define	SCS	((trident_set_cursor_shape *) buf)
	...
	case MAXIOCTL_TRIDENT + B_SHOW_CURSOR:
		showcursor (ci, *(bool *) buf);
		retval = B_OK;
		break;

	case MAXIOCTL_TRIDENT + B_MOVE_CURSOR:
		/*
		 * This is only called by the accelerant if interrupts aren't
		 * enabled on the card.
		 */
		movecursor (di,
			    di->di_TriCardCtl->cc_MousePosX,
			    di->di_TriCardCtl->cc_MousePosY);
		retval = B_OK;
		break;

#if _SUPPORTS_FEATURE_CURSOR_COLORS
	case MAXIOCTL_TRIDENT + B_SET_CURSOR_COLORS:
		setcursorcolors (di);
		retval = B_OK;
		break;
#endif
	 ...
	 */
	/*  Initialization  */
	case MAXIOCTL_TRIDENT + B_GET_CONFIG_INFO_SIZE:
	case MAXIOCTL_TRIDENT + B_GET_CONFIG_INFO:
	case MAXIOCTL_TRIDENT + B_SET_CONFIG_INFO:

	/*  Mode configuration  */
	case MAXIOCTL_TRIDENT + B_GET_DISPLAY_MODE:
	case MAXIOCTL_TRIDENT + B_MOVE_DISPLAY:
	case MAXIOCTL_TRIDENT + B_SET_GAMMA:

	/*  Cursor managment  */
	case MAXIOCTL_TRIDENT + B_MOVE_CURSOR:

	/*  Synchronization  */
	case MAXIOCTL_TRIDENT + B_ACCELERANT_ENGINE_COUNT:
	case MAXIOCTL_TRIDENT + B_ACQUIRE_ENGINE:
	case MAXIOCTL_TRIDENT + B_RELEASE_ENGINE:
	case MAXIOCTL_TRIDENT + B_WAIT_ENGINE_IDLE:
	case MAXIOCTL_TRIDENT + B_GET_SYNC_TOKEN:
	case MAXIOCTL_TRIDENT + B_SYNC_TO_TOKEN:

	/*  2D acceleration  */
	case MAXIOCTL_TRIDENT + B_SCREEN_TO_SCREEN_BLIT:
	case MAXIOCTL_TRIDENT + B_FILL_RECTANGLE:
	case MAXIOCTL_TRIDENT + B_INVERT_RECTANGLE:
	case MAXIOCTL_TRIDENT + B_FILL_SPAN:
	
	/*  3D acceleration  */
	case MAXIOCTL_TRIDENT + B_ACCELERANT_PRIVATE_START:
#endif
...
static status_t
setdpmsmode (register union vgaregs *vregs, int mode)
{
	uint8	dpmsctl, powerctl, dopanellight;

	dopanellight	= IDXREG_R (vregs, VGA_GR, GR_SOFTPWRCTL) &
	...
	powerctl	= IDXREG_R (vregs, VGA_GR, GR_PWRCTLSEL) &
			  ~FIELDMASK (GR_PWRCTLSEL, PANELLIGHT);
	dpmsctl		= IDXREG_R (vregs, VGA_GR, GR_PWRSTATUS) &
			  ~FIELDMASK (GR_PWRSTATUS, DPMS);

	switch (mode) {
	case B_DPMS_ON:		// H:  on, V:  on
		dpmsctl |= FIELDDEF (GR_PWRSTATUS, DPMS, ON);
		powerctl |= FIELDDEF (GR_PWRCTLSEL, PANELLIGHT, HW);
		screen_on (vregs);
		break;
	case B_DPMS_STAND_BY:	// H: off, V:  on
		dpmsctl |= FIELDDEF (GR_PWRSTATUS, DPMS, STANDBY);
		powerctl |= FIELDDEF (GR_PWRCTLSEL, PANELLIGHT, HW);
		goto scroff;	/*  Look down  */
	case B_DPMS_SUSPEND:	// H:  on, V: off
		dpmsctl |= FIELDDEF (GR_PWRSTATUS, DPMS, SUSPEND);
		powerctl |= FIELDDEF (GR_PWRCTLSEL, PANELLIGHT, HW);
		goto scroff;	/*  Look down  */
	case B_DPMS_OFF:	// H: off, V: off
		dpmsctl |= FIELDDEF (GR_PWRSTATUS, DPMS, OFF);
		powerctl |= FIELDDEF (GR_PWRCTLSEL, PANELLIGHT, SW);
scroff:		screen_off (vregs);
		break;
	default:
		return (B_BAD_VALUE);
	}
	if (dopanellight)
		IDXREG_W (vregs, VGA_GR, GR_PWRCTLSEL, powerctl);
	IDXREG_W (vregs, VGA_GR, GR_PWRSTATUS, dpmsctl);
	return (B_OK);
}
...
#if (DEBUG  ||  FORCE_DEBUG)
static int
tridump (int argc, char **argv)
{
	int i;
	
	kprintf ("Trident kernel driver\nThere are %d device(s)\n",
		 tridd->dd_NDevs);
	kprintf ("Driver-wide data: 0x%08lx\n", tridd);
	kprintf ("Driver-wide benahpore: %d/%d\n",
		 tridd->dd_DriverLock.b4_Sema4,
		 tridd->dd_DriverLock.b4_FastLock);
	kprintf ("Total calls to IRQ handler: %ld\n", ncalls);
	kprintf ("Total relevant interrupts: %ld\n", tridd->dd_NInterrupts);

	for (i = 0;  i < tridd->dd_NDevs;  i++) {
		devinfo		*di = &tridd->dd_DI[i];
		tri_card_info	*ci = di->di_TriCardInfo;
		tri_card_ctl	*cc = di->di_TriCardCtl;
		...
		if (ci) {
			kprintf ("      tri_card_info: 0x%08lx\n", ci);
			kprintf ("       tri_card_ctl: 0x%08lx\n", cc);
			kprintf ("  Base register 0,1,2: %p, %p, %p\n",
				 ci->ci_BaseAddr0,
				 ci->ci_BaseAddr1,
				 ci->ci_BaseAddr2);
