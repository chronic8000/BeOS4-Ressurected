...
init_locks (struct devinfo *di, team_id team)
{
	 ...
	 */
	if ((retval = initOwnedBena4 (&cc->cc_EngineLock,
...
init_driver (void)
{
	...
	if ((retval = initOwnedBena4 (&i740dd->dd_DriverLock,
				      "i740 kernel driver",
				      B_SYSTEM_TEAM)) < 0)
	{
		free (i740dd);
...
publish_devices (void)
{
	...
	return (const char **)(i740dd->dd_DevNames);
}
...
#endif
		disposeBena4 (&i740dd->dd_DriverLock);
		free (i740dd);
		i740dd = NULL;
	}
...
dprintf (("+++ lastclose() begins...\n"));
	...
	free (di->di_DispModes); /* FFB: Leo: why the extra parms? : , di->di_NDispModes * sizeof (display_mode)); */
...
dprintf (("+++ open_hook(%s, %ld, 0x%08lx)\n", name, flags, (uint32)cookie));
	...
	lockBena4 (&i740dd->dd_DriverLock);
...
err:	unlockBena4 (&i740dd->dd_DriverLock);
...
dprintf (("+++ free_hook() begins...\n"));
	...
	unlockBena4 (&i740dd->dd_DriverLock);
...
#define	FBI	((frame_buffer_config *) buf)
		...
		FBI->frame_buffer = di->di_I740CardInfo->ci_FBBase;
		FBI->frame_buffer_dma = di->di_I740CardInfo->ci_FBBase_DMA;
		FBI->bytes_per_row =
		 di->di_I740CardInfo->ci_BytesPerRow;
		retval = B_OK;
		break;
...
i740dump (int argc, char **argv)
{
	...
	for (i = 0;  i < (int)i740dd->dd_NDevs;  i++) {
		...
		if (ci) {
			...
			kprintf ("  Base register 0,1: 0x%08lx, 0x%08lx\n",
				 (uint32)ci->ci_BaseAddr0, (uint32)ci->ci_BaseAddr1);
