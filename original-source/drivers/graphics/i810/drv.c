...
init_hardware (void)
{
	     ...
	     idx++)
	{
		if (pcii.vendor_id == VENDORID_INTEL  &&
		    (pcii.device_id == DEVICEID_I810  ||
		     pcii.device_id == DEVICEID_I810_DC100  ||
		     pcii.device_id == DEVICEID_I810E  ||
		     pcii.device_id == DEVICEID_I815_FSB100  ||
		     pcii.device_id == DEVICEID_I815_NOAGP  ||
		     pcii.device_id == DEVICEID_I815))
		{
...
init_driver (void)
{
	...
	if ((retval = create_sem (1, "i810 kernel driver")) < 0)
		goto err2;	/*  Look down  */
	gdd->dd_DriverLock = retval;
	set_sem_owner (retval, B_SYSTEM_TEAM);
