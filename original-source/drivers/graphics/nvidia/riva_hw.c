...
//          state->general  = 0x00100100;
            state->general  = bpp == 16 ? 0x00101100 : 0x00100100; /*  ewhac  */
//          state->repaint1 = hDisplaySize < 1280 ? 0x06 : 0x02;
            state->repaint1 = hDisplaySize < 1280 ? 0x04 : 0x00; /* ewhac */
            break;
        case NV_ARCH_04:
    ...
    state->screen   = ((((width/8)*pixelDepth) & 0x800) >> 6)	/*  ewhac  */
                    | ((hTotal   & 0x040) >> 2)
                    | ((vDisplay & 0x400) >> 7)
                    | ((vStart   & 0x400) >> 8)
                    | ((vDisplay & 0x400) >> 9)
        ...
        case NV_ARCH_03:
            ...
            switch (state->bpp)
            {
                case 15:
                    ...
                    break;
                case 16:				/*  <ewhac>	*/
                    LOAD_FIXED_STATE_16BPP(nv3,PRAMIN);
                    LOAD_FIXED_STATE_16BPP(nv3,PGRAPH);
                    chip->Tri03 = 0L;
                    break;				/*  </ewhac>	*/
                case 24:
                case 32:
...
)
{
    ...
    tmp = VGA_RD08 (chip->PCIO, chip->IO + 0x0A);  /*  Prepare ATTR regs */
    VGA_WR08 (chip->PCIO, 0x3C0, 0);		   /*  unlock attr regs	*/
    tmp = VGA_RD08 (chip->PCIO, chip->IO + 0x0A);  /*  reset attr phase	*/

    if (chip->Architecture == NV_ARCH_03) {
        ...
        b1 = offset;
        offset >>= 8;
        b2 = chip->CurrentState->repaint0 & ~0x1f;
        b2 |= offset & 0x1f;
        chip->CurrentState->repaint0 = b2;
        b3 = chip->CurrentState->horiz & ~0x60;
        b3 |= offset & 0x60;
        chip->CurrentState->horiz = b3;

        /*  Bang registers as fast as we can.  */
        /*
         * Set start address.
         */
        VGA_WR16 (chip->PCIO, 0x3D4, (b1 << 8) | 0x0C);
        VGA_WR16 (chip->PCIO, 0x3D4, (b0 << 8) | 0x0D);
        VGA_WR16 (chip->PCIO, 0x3D4, (b2 << 8) | 0x19);
        VGA_WR16 (chip->PCIO, 0x3D4, (b3 << 8) | 0x2D);
    } else {
        chip->PCRTC[0x00000800/4] = start;
        chip->PCRTC[0x00000804/4] = 1;    /*  Non-VGA  */
    }
    ...
    VGA_WR08 (chip->PCIO, 0x3C0, 0x13);
    VGA_WR08 (chip->PCIO, 0x3C0, pan);
    VGA_WR08 (chip->PCIO, 0x3C0, 0x20);	/*  lock attr regs	*/
#endif
}
...
//  chip->CrystalFreqKHz   = (chip->PEXTDEV[0x00000000/4] & 0x00000020) ? 13500 : 14318;	/* this was reversed, not sure that it is right this way either (HCS) */
	...
	// Fix by ewhac (geez, what a yutz; it's right there in nv3ref.h...)
    ...
    chip->BUSYREG          = &(chip->PGRAPH[0x000006B0/4]);  /*  ewhac  */
    chip->BusyBit          = 0x01;                           /*  ewhac  */
    /*
     * Set chip functions.
     */
    chip->Busy            = nv3Busy;
...
)
{
    ...
    chip->BUSYREG          = &(chip->PGRAPH[0x00000700/4]);  /*  ewhac  */
    chip->BusyBit          = 0x01;                           /*  ewhac  */
    /*
     * Set chip functions.
     */
