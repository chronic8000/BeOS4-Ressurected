...
static void write_table(uint16 addr, const uint8 *table, int first, int last);
...
)
{
	...
	/*
	 * We let the code dance on the original, as there's an absolute
	 * pointer in RIVA_HW_INST pointing to the current RIVA_HW_STATE.
	 */
	mr = &di->di_GfxCardCtl->cc_ModeRegs;
	memcpy (&oldregs, mr, sizeof (oldregs));

	if ((retval = calcmoderegs (mr, dm, di)) < 0)
		goto err;	/*  Look down  */
	if ((retval = setmode (mr, di, ci)) < 0)
		goto err;	/*  Look down  */
	 ...
	 */
err:	if (retval < 0) {
		memcpy (mr, &oldregs, sizeof (oldregs));
	}
...
/*  1998.08.18: Made impossible by current R4 API  */
	 ...
	 * Not fully symbolized, but the includes should provide some clues.
	...
	mr->mr_CRTC[CR_HBLANKEND]	= SetBitField (htotal - 1, 4:0, 4:0) |
					  SetBit (7);
	mr->mr_CRTC[CR_HSYNCSTART_7_0]	= Set8Bits (hsyncstart - 1);
	mr->mr_CRTC[CR_HSYNCEND]	= SetBitField (htotal - 1, 5:5, 7:7) |
					  SetBitField (hsyncend - 1, 4:0, 4:0);
	...
	mr->mr_CRTC[CR_OVERFLOW]	= SetBitField (vtotal - 2, 8:8, 0:0) |
					  ...
					  SetBitField (vsyncstart - 1, 8:8, 2:2) |
	/*  No border  */		  SetBitField (vdisp - 1, 8:8, 3:3) |
					  SetBit (4) |
					  SetBitField (vtotal - 2, 9:9, 5:5) |
					  SetBitField (vdisp - 1, 9:9, 6:6) |
					  SetBitField (vsyncstart - 1, 9:9, 7:7);

	mr->mr_CRTC[CR_FONTHEIGHT]	= SetBitField (vdisp - 1, 9:9, 5:5) |
					  SetBit (6);
	mr->mr_CRTC[CR_VSYNCSTART_7_0]	= Set8Bits (vsyncstart - 1);
	mr->mr_CRTC[CR_VSYNCEND]	= SetBitField (vsyncend - 1, 3:0, 3:0) |
					  SetBit (5);
...
//	setdispoffset (ci, cc, dm);
	...
	/*
	 * Fenceposts are as I found them in the NVidia XFree code.  I have
	 * no idea why they did it this way, or why the NVidia hardware seems
	 * to take non-VGA-standard fenceposts.
	 */
	(di->di_GfxCardInfo->ci_HWInst.CalcStateExt)
	 (&di->di_GfxCardInfo->ci_HWInst,
	  &mr->mr_HWState,
	  depth,
	  dm->virtual_width,
	  dm->timing.h_display,
...
dprintf (("+++!!! Display buffer allocation failed: %ld (0x%08lx)\n", retval, retval));
...
dprintf (("+++!!! YIKES!  Can't restore framebuffer allocation!!  retval = %ld\n",
 retval));
		}
		return (retval);
	}
...
dprintf (("+++ ci->ci_FBBase allocated at offset 0x%08lx\n", ci->ci_FBBase));
	...
	/*
	 * Write CRTC registers; this should give us a stable VSync while
	 * we muck with other stuff.
	 */
	write_table (offsetof (vgaregs, w.VGA_CR_Idx),
	             mr->mr_CRTC, 0, NREGS_CRTC);

	/*  Wait for VSync  */
//	i = ~(VGA_INB (VGA_ST01)) & MASKFIELD (ST01, VBLACTIVE);
//	while (i ^ (VGA_INB (VGA_ST01) & MASKFIELD (ST01, VBLACTIVE)))
//		/*  Wait for state to change  */
//		;

	/*  Sequencer registers  */
	write_table (offsetof (vgaregs, w.VGA_SR_Idx),
	             seq_table, SR_CLOCK, SR_MODE);

	/*  Misc output register  */
	VGA_OUTB (VGA_Misc, mr->mr_Misc);

	/*  Graphics controller registers  */
	write_table (offsetof (vgaregs, w.VGA_GR_Idx),
	             gcr_table, GR_SETRESET, GR_LATCHMASK);

	/*  Load Attribute registers  */
	i = VGA_INB (VGA_ST01);
	for (i = 0;  i < NREGS_ATTR;  i++)
	{
		VGA_OUTB (VGA_AR_Idx, i);
		VGA_OUTB (VGA_AR_ValW, mr->mr_ATTR[i]);
	}
	VGA_OUTB (VGA_AR_Idx, DEF2FIELD (AR_IDX, ACCESS, LOCK));

	(hwi->LoadStateExt) (hwi, &mr->mr_HWState);
	 ...
	 */
	(hwi->SetStartAddress) (hwi, ci->ci_FBBase);
...
{
	VGA_OUTB (VGA_DACWIdx, idx);

	VGA_OUTB (VGA_DACVal, r);
	VGA_OUTB (VGA_DACVal, g);
	VGA_OUTB (VGA_DACVal, b);
}
...
setramppalette (register union vgaregs *vregs)
{
	...
	VGA_OUTB (VGA_DACWIdx, 0);	/*  Start at zero  */
	for (i = 0;  i < 256;  i++) {
		VGA_OUTB (VGA_DACVal, i);
		VGA_OUTB (VGA_DACVal, i);
		VGA_OUTB (VGA_DACVal, i);
	}
}
...
static void
write_table (uint16 portnum, const uint8 *table, int first, int last)
{
	int i;

	for (i = first;  i <= last;  i++)
		outw (portnum, ((uint16) table[i] << 8) | i);
}
...
{
	/*  Place chip in reset  */
	IDXREG_IOW (vregs, VGA_SR,
		    SR_RESET, DEF2FIELD (SR_RESET, ASYNC, ENABLE) |
			      DEF2FIELD (SR_RESET, SYNC, DISABLE));

	/*  Turn off display  */
	VGA_OUTB (VGA_SR_Idx, SR_CLOCK);
	IDXREG_IOW (vregs, VGA_SR,
		    SR_CLOCK,
		    VGA_INB (VGA_SR_Val) | MASKEXPAND (SR_CLOCK_VIDDISABLE));
...
)
{
	...
	uint32		row_bytes, pixel_bytes;
	uint32		limit_clock_lo, clock_max;
	...
	clock_max = di->di_GfxCardInfo->ci_MaxPixClkKHz;
...
revalidate:
	...
	if (pixel_bytes * target->timing.pixel_clock >
	    di->di_GfxCardInfo->ci_HWInst.RamBandwidthKBytesPerSec)
		result = B_BAD_VALUE;
...
vid_init (register struct devinfo *di)
{
	...
	ci->ci_HWInst.CurrentState = &cc->cc_ModeRegs.mr_HWState;

	ci->ci_MasterClockKHz	= ci->ci_HWInst.CrystalFreqKHz;
	ci->ci_MaxPixClkKHz	= ci->ci_HWInst.MaxVClockFreqKHz;
	ci->ci_MemSize		= ci->ci_HWInst.RamAmountKBytes << 10;
dprintf (("+++ ci_MemSize: %ld\n", ci->ci_MemSize));
	 ...
	 */
	gfbpi->pi_Pool_AID	= di->di_BaseAddr1ID;
	gfbpi->pi_Pool		= (void *) ci->ci_BaseAddr1;
	gfbpi->pi_Size		= ci->ci_MemSize;
	strcpy (gfbpi->pi_Name, "NVidia Framebuffer");
...
dprintf (("++! Failed to create genpool, retval = %ld (0x%08lx)\n", ret, ret));
	...
	/*
	 * Select I/O address 0x3D?, custom clock rate.
	 */
	cc->cc_ModeRegs.mr_Misc = VGA_INB (VGA_Misc) &
				   ~(MASKFIELD (MISC, VSYNCPOL) |
				     MASKFIELD (MISC, HSYNCPOL) |
	...
	VGA_OUTB (VGA_Misc, cc->cc_ModeRegs.mr_Misc);

	/*  Enable write access to standard VGA CRTC registers 0-7  */
	VGA_OUTB (VGA_CR_Idx, CR_VSYNCEND);
	VGA_OUTB (VGA_CR_Val, SETVAL2FIELD (VGA_INB (VGA_CR_Val),
					    CR_VSYNCEND, PROTECT, FALSE));
	ci->ci_HWInst.LockUnlock (&ci->ci_HWInst, 0);

	VGA_OUTB (VGA_DACMask, ~0);

	/*
	 * Write identification info.
	 */
	strcpy (ci->ci_ADI.name, di->di_SC->sc_Name);
	strcpy (ci->ci_ADI.chipset, di->di_SC->sc_ChipSet);
	ci->ci_ADI.dac_speed = ci->ci_MaxPixClkKHz / 1000;
...
#define	NSPACES		(sizeof (spaces) / sizeof (color_space))
	...
	overlays = (di->di_GfxCardInfo->ci_HWInst.Architecture >= NV_ARCH_10)
	         ? B_SUPPORTS_OVERLAYS
	         : 0;
