	...
	for (i=0; i<count; i++) {
		...
		if (port->canvasIsRotated) {
			...
			if (!empty_rect(r2)) {
				...
				subSrc.x = srcRect.bottom - ((r2.left - dstRect.left) + 0.5) * pixelX;
				subSrc.y = srcRect.left + ((r2.top - dstRect.top) + 0.5) * pixelY;
				
				/*	Convert this to a starting x and y, and fractional components thereof */
				fixedX = (int32)floor(subSrc.x * 65536 + 0.5);
				fixedY = (int32)floor(subSrc.y * 65536 + 0.5);
...
#endif	// ROTATE_DISPLAY
		{
			...
			if (!empty_rect(r2)) {
				/*	r2 is a subset of dstRect.  We have to fill it with samples
					based on our mapping.  First, figure out our UL corner in the source */
				subSrc.x = srcRect.left + (r2.left - dstRect.left + 0.5) * pixelX;
				subSrc.y = srcRect.top + (r2.top - dstRect.top + 0.5) * pixelY;
				
				/*	Convert this to a starting x and y, and fractional components thereof */
				fixedX = (int32)floor(subSrc.x*65536);
				fixedY = (int32)floor(subSrc.y*65536);
...
void grCopyPixels(RenderContext *context, rect f_rect, point to, bool clipSource)
{
	...
	while (port) {
		const int32 ox = (int32)floor(context->totalEffective_Origin.x + port->origin.x + 0.5);
		const int32 oy = (int32)floor(context->totalEffective_Origin.y + port->origin.y + 0.5);
		f_rect = src_rect;
		...
		rect t_rect;
		t_rect.top = port_to.x;
		t_rect.left = port_to.y;
		...
		if (port->canvasIsRotated) {
			...
			dh = port->rotater->RotateDV(port_to.x - f_rect.top);
			dv = port->rotater->RotateDH(port_to.y - f_rect.left);
		} else
#endif
		{
			dh = port_to.x - f_rect.left;
			dv = port_to.y - f_rect.top;
		}		
...
void grDrawPixels_FatThin( RenderContext *context, frect ff_rect, frect ft_rect, Pixels *src)
{
	...
	rect t_rect;
	t_rect.left		= (int32)floor(context->totalEffective_Origin.x + (ft_rect.left-0.5)*context->totalEffective_Scale + 1);
	t_rect.top 		= (int32)floor(context->totalEffective_Origin.y + (ft_rect.top-0.5)*context->totalEffective_Scale + 1);
	t_rect.right	= (int32)floor(context->totalEffective_Origin.x + (ft_rect.right+0.5)*context->totalEffective_Scale);
	t_rect.bottom	= (int32)floor(context->totalEffective_Origin.y + (ft_rect.bottom+0.5)*context->totalEffective_Scale);
	...
	if (src->pixelIsRotated)
	{
			...
			((f_rect.right - f_rect.left) == (t_rect.right - t_rect.left)))
		{
			...
			while (port) {
				rect tt_rect = t_rect;
				const int dh = port->rotater->RotateDV(port->origin.y);
				const int dv = port->rotater->RotateDH(port->origin.x);
...
#endif
	{
		if (((f_rect.bottom - f_rect.top) == (t_rect.bottom - t_rect.top)) &&
			...
			while (port) {
				rect tt_rect = t_rect;
				offset_rect(&tt_rect, port->origin.x, port->origin.y);
#if ROTATE_DISPLAY
				if (port->canvasIsRotated)
