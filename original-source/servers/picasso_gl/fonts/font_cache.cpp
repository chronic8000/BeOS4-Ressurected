...
void fc_load_font_cache() {
	...
	while (true) {
		...
		if (fread(fname, 1, fsize, fp) != (size_t)fsize) break;
		if (fread(sname, 1, ssize, fp) != (size_t)ssize) break;
		fname[fsize] = 0;
		sname[ssize] = 0;
...
void fc_save_font_cache() {
	...
	for (i=0; i<set_table_count; i++) {
		if ((se->char_count >= 0) &&
			...
			else if (fwrite(fname, 1, fsize, fp) != (size_t)fsize) exit = 1;
			else if (fwrite(sname, 1, ssize, fp) != (size_t)ssize) exit = 1;
...
void fc_get_app_settings(int bpp, long pid, fc_font_cache_settings *set) {
	...
	while (true) {
		...
		if (cs == 0) break;
		if (cs->bits_per_pixel == (uint32)bpp) {
			cs->get_settings(set);
			return;
		}
	}
