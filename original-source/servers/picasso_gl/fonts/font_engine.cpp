...
static void get_kerning_profile(int32		bpp,
	...
	int32		left[32];
	int32		right[32];
	int32		i, min, max, width, row, pt2, pt3, delta, tmax, total;
	uint16		*p_table;
...
void fc_lock_string(fc_context *context,
	...
	y = *yy+32767.0;
	for (i=0; i<count; i++) {
		draw->offsets[i].x = (int)(x+e_table.offsets[i].x)-32767;
		draw->offsets[i].y = (int)(y+e_table.offsets[i].y)-32767;
...
void fc_get_escapement(fc_context      *context,
	...
	int			delta_t, index, delta, sp_count, sp_width, sp_step;
			...
			/* || (context->shear() != 0.0) --- Let's try to handle shearing also... */
				...
				/* use the rounded size to process the ratio */
					for (i=0; i<count; i++) {
						...
						table->escapes[i].x_escape = my_char->escape.x_escape*escape_ratio;
						table->escapes[i].y_escape = my_char->escape.y_escape*escape_ratio;
						table->offsets[i].x = 0.0625*(float)my_char->spacing.char_offset;
						table->offsets[i].y = 0.0;
					}
				else for (i=0; i<count; i++) {
				/* use the real size to process the ratio */
					my_char = draw->chars[i];
			...
			/* bitmap metric analysis */
				...
				/* use the rounded size to process the ratio */
					for (i=0; i<count; i++) {
						...
						table->offsets[i].x = -my_char->spacing.offset;
						table->offsets[i].y = 0.0;
					}
				else for (i=0; i<count; i++) {
				/* use the real size to process the ratio */
