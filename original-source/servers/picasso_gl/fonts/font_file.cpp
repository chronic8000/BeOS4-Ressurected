...
#define SIZE_TO(type, field) \
	( ((size_t)&((type*)0)->field) + sizeof(type::field) )
...
{					   
	...
	int 				  j, family_name_length=0, style_name_length=0, count=0;
	int					  value;
...
//	_sPrintf("Table: ");
	for (int32 i = 0; (i < (int32)cTables) && (i < 40); i++) {
...
/*		_sPrintf("%c%c%c%c ",
		...
		case tag_HMetHeader :
			...
			if (fread((char *)buffer, 1, 4*count, fp) != 4*(size_t)count) {
				grFree(buffer);
				_bad_font_printf1("##### ERROR ##### Unexected EOF (HMetric header) !\n");
				goto skip_HMetric;
			}
		...
		case tag_OS2Header :
			...
			if (readsize >= SIZE_TO(sfnt_OS2Header, fsSelection)) {
				fsSelection |= fc_read16(&OS2Header.fsSelection);
				gotFaces = true;
			}
			if (readsize >= SIZE_TO(sfnt_OS2Header, UnicodeRange)) {
	...
	if (gotFamily && gotStyle && gotMeasurements && gotHeader) {
		...
		name = name_buffer;
		if (((size_t)family_name_length > sizeof(name_buffer)) || ((size_t)style_name_length > sizeof(name_buffer))) {
...
//			 fe->gasp[4], fe->gasp[5], fe->gasp[6], fe->gasp[7]);
			...
			if (bitmap) {
				...
				for (j=0; j<seg_count; j++) {
					if (sg->idRangeOffset == 0) {
						...
						//		sg->startCount, sg->endCount,
						//		sg->startCount/0x100, sg->endCount/0x100);
						for (int i=0; i<=(sg->endCount-sg->startCount); i++) {
		...
		if (fread((char *)name, 1, family_name_length, fp) != (size_t)family_name_length) {
		...
		if (fread((char *)name, 1, style_name_length, fp) != (size_t)style_name_length) {
...
static bool get_tuned_info(fc_file_entry *fe, FILE *fp) {
	...
	if (fread(names, 1, length, fp) != (size_t)length) {
		_bad_font_printf1("##### ERROR ##### Tuned Font : Unexpected EOF !\n");
		goto exit_false;
	}
	...
	if ((strlen(names+0) != (size_t)family_length) || (strlen(names+family_length+1) != (size_t)style_length)) {
		_bad_font_printf1("##### ERROR ##### Tuned Font : invalid names !\n");
		goto exit_false;
	}
