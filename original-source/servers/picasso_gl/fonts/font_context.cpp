...
//		set_uid++;
 ...
 last_end:
	...
	if (*set_hit_ref == 0)
		*set_hit_ref = set_hit;
	else if (*set_hit_ref == (uint32)se->hit_ref)
		*set_hit_ref = 0;
	se->hit_ref = set_hit;
	return se;
}
...
void fc_context::debug() {
	...
	xprintf("flags:%ld\n", _flags);
	xprintf("final_flags:%ld\n", _final_flags);
	xprintf("style_faces:%d\n", _style_faces);
	xprintf("spacing mode:%d\n", _spacing_mode);
	xprintf("file_flags:%ld\n", _file_flags);
	xprintf("file type:%d\n", file_type);
	xprintf("file id:%d\n", file_id);
	xprintf("ascent:%f\n", _ascent);
	xprintf("descent:%f\n", _descent);
	...
	xprintf("locker:%p\n", locker);
	xprintf("matrix xxmult:%ld\n", _matrix.xxmult);
	xprintf("matrix yxmult:%ld\n", _matrix.yxmult);
	xprintf("matrix xymult:%ld\n", _matrix.xymult);
	xprintf("matrix yymult:%ld\n", _matrix.yymult);
	xprintf("matrix xoffset:%ld\n", _matrix.xoffset);
	xprintf("matrix yoffset:%ld\n", _matrix.yoffset);
