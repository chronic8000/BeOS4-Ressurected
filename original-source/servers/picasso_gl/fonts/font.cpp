...
struct font_body
{
	...
	font_body(const font_body* o)
		: context(&(o->context)), local_overlay(o->local_overlay),
		  seq(o->seq), refs(0)
	{
		#if SHOW_REFS
		DebugPrintf(("Created font_body #%ld\n", atomic_add(&bodyCount, 1)));
	...
	font_body(int family_id, int style_id, int size, bool allow_disable)
		: context(family_id, style_id, size, allow_disable),
		  seq(gSettingsID), refs(0)
	{
		#if SHOW_REFS
		DebugPrintf(("Created font_body #%ld\n", atomic_add(&bodyCount, 1)));
	...
	font_body(const fc_context_packet* p)
		: context(p), seq(gSettingsID), refs(0)
	{
		#if SHOW_REFS
		DebugPrintf(("Created font_body #%ld\n", atomic_add(&bodyCount, 1)));
		#endif
...
void SFont::LoadPrefs()
{
	...
	if (fp) {
		...
		while (1) {
			...
			if (fread(buffer, 1, len, fp) != (size_t)len) goto finish;
	...
	if (fp) {
		...
		for (i=0; i<B__NUM_FONT; i++) {
			...
			if (f && len > 0 && f->Flatten(buffer, len) == B_OK) {
				...
				if (fwrite(buffer, 1, len, fp) != (size_t)len) goto finish;
			}
		}
	...
	if (which == 99) {
		*into = SFont();
		...
		if (fb) {
			...
			int family_id = -1;
			for (uint32 i=0; i<(sizeof(ideograph_fonts)/sizeof(ideograph_fonts[0])); i++) {
				family_id = fc_get_family_id(ideograph_fonts[i]);
				if (family_id >= 0)
					break;
			}
			if (family_id >= 0) {
				for (uint32 i=0; i<(sizeof(ideograph_ranges)/sizeof(ideograph_ranges[0])); i++) {
...
SFont::SFont(const SFont& o)
	: BFlattenable(o), BAbstractFont(o), fLocked(NULL)
{
