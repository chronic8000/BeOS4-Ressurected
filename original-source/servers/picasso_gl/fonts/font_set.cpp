...
int fc_save_full_set(fc_context    *context,
	...
	if (context->tuned_file_id == 0xffff) {
		...
		if (fseek(fp, 4+sizeof(fc_tuned_file_header), SEEK_SET) != 0)
			goto exit_bad_file;
		if (fwrite(family_name, 1, family_name_length+1, fp) != (size_t)family_name_length+1)
			goto exit_bad_file;
		if (fwrite(style_name, 1, style_name_length+1, fp) != (size_t)style_name_length+1)
			goto exit_bad_file;
		...
		for (i=0; i<count; i++)
			if (htable[i].offset >= 0) {
				...
				if (length > 0) {
					if (fwrite(((char*)ch)+CHAR_HEADER_SIZE, 1, length, fp) != (size_t)length)
						goto exit_bad_file;
					j += length;
				}
			}
...
bool fc_read_char_from_file(FILE *fp, fc_set_entry *set, FC_GET_MEM get_mem) {
	...
	*((uint32*)&f_table[3]) = fc_read32(&ch.escape.y_escape);
	...
	if (bitmap_size > 0)
		if (fread((void*)((char*)fchar+CHAR_HEADER_SIZE), 1, bitmap_size, fp) != (size_t)bitmap_size)
...
bool fc_write_char_to_file(FILE *fp, fc_char *fchar) {
	...
	bitmap_size = ((size_h+1)>>1)*size_v;
	if (bitmap_size > 0)
		if (fwrite(((char*)fchar)+CHAR_HEADER_SIZE, 1, bitmap_size, fp) != (size_t)bitmap_size)
			return false;
	return true;
}
