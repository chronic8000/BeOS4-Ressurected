...
void fc_font_processor::process_spacing(int32 bpp, fc_char *my_char, fc_set_entry *se) {
...
int	fc_font_renderer::first_time_init(fc_file_entry *fe) {
	if (!reset())
		return false;
	return true;
}
...
int fc_font_renderer::render_char(const ushort *code, fc_set_entry *se) {
 ...
 char_not_found:
	...
	if (se->params.bits_per_pixel() == FC_GRAY_SCALE ||
		...
		uchar			*buf, *from;
		uchar			right_mask;
		int32			h, h0, h1, v, row, row4, size_h, size_v;
...
int fc_font_renderer::set_specs(int def_flag) {	
	...
	if (safety & 1)
		return true;
	if (ctxt->size() >= gray_status.limit_size)
		level = 1;
	else
		level = 0;
