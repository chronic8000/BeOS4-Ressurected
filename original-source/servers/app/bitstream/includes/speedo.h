...
*                                                                         
*     $Header: //bliss/user/archive/rcs/speedo/speedo.h,v 33.0 97/03/17 17:48:04 shawn Release $                                                                       *
*                                                                                    
*     $Log:	speedo.h,v $
*       Revision 33.0  97/03/17  17:48:04  shawn
*       TDIS Version 6.00
*       
*       Revision 32.4  97/03/11  11:37:33  shawn
*       Added Emulation control macros (PCL5/PCL5e/PCL6). PCL6 is the default.
*       
*       Revision 32.3  96/12/19  13:24:52  shawn
*       Added the element 'boldValue' to the specs_t structure for emboldening.
*       
*       Revision 32.2  96/07/02  16:59:42  shawn
*       Changed boolean to btsbool.
*       
*       Revision 32.1  96/07/01  09:36:06  shawn
*       Added support for the Gray Writer output module.
*       Positioned Multi-Device support for global use.
*       Added sp_set_bitmap_pixels() function pointer to bitmap function pointer array.
*       
...
*       New macros for REENTRANT_ALLOC.
*       
...
/* -------------Processor Exclusion Macro:--------------*
/*      This will turn off all undefined processors     */
...
/* to shut off the TrueDoc processor, #define PROC_TRUEDOC 0 in useropt.h */

#ifndef PROC_TRUEDOC
#define PROC_TRUEDOC	1
#endif

/* TDIS compile conditional initializations */

/* Default printer emulation is PCL6 */
/* Change to PCL5 with #define PCL5 1 in useropt.h */
/* Change to PCL5e with #define PCL5e 1 in useropt.h */

#ifndef PCL5
#define PCL5    0                       /* 0 indicates a non-PCL5 printer */
#endif

#ifndef PCL5e
#define PCL5e   0                       /* 0 indicates a non-PCL5e printer */
#endif

#ifndef PCL6
#if PCL5 || PCL5e
#define PCL6    0                       /* 0 indicates a non-PCL6 printer */
#else
#define PCL6    1                       /* 1 indicates a PCL6 printer */
#endif
#endif
...
#ifndef INTEL
#define INTEL	0                       /* 0 indicates a non-Intel platform */
#endif

#ifndef WINDOWS_4IN1
#define	WINDOWS_4IN1	0               /* 0 indicates a non-Windows platform */
#endif

#ifndef	APOLLO
#define	APOLLO	0                       /* 0 indicates a non-UNIX platform */
#endif
...
#ifndef INCL_GRAY
#define INCL_GRAY	0		/* 1 to include graywriter output support */
#endif						/* 0 to omit */
...
#ifndef PROTOS_AVAIL
#define PROTOS_AVAIL	0		/* 1 to use function prototyping */
#endif                                          /* 0 to suppress it */
...
#define MODE_GRAY       MODE_QUICK + INCL_QUICK
...
typedef struct constr_tag
    ...
    btsbool font_id_valid;      /* TRUE if font id valid                   */
    btsbool data_valid;         /* TRUE if calculated data valid           */
    btsbool active;             /* TRUE if constraints enabled             */
...
typedef struct specs_tag
    ...
    ufix32  flags;              /* Mode flags:                           */
                                ...
                                /*   Bits 16-31   not used               */
    fix31   boldValue;          /* Font emboldening value                */
    void   *out_info;		/* information for output module         */
    } specs_t;                  /* Specs structure for set_specs()       */
...
#if PROC_TRUEDOC
    btsbool xHintsActive;
    btsbool yHintsActive;
    btsbool suppressOvershoots;
#endif /* PROC_TRUEDOC */
...
#if INCL_SCREEN || INCL_QUICK || INCL_GRAY
...
typedef struct bitmap_tag
    ...
    void (*p_set_pixels)(PROTO_DECL2 fix15 y, fix15 xbit1, fix15 num_grays, void *gray_buf);
...
typedef struct outline_tag 
    ...
    void (*p_start_contour)(PROTO_DECL2 fix31 x, fix31 y, btsbool outside);
...
typedef	 btsbool (*init_outFunc)(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
typedef  btsbool (*begin_charFunc)(PROTO_DECL2 point_t Psw, point_t Pmin, point_t Pmax);
typedef  btsbool (*end_charFunc)(PROTO_DECL1);

typedef  void    (*begin_sub_charFunc)(PROTO_DECL2 point_t Psw, point_t Pmin, point_t Pmax);
typedef  void    (*begin_contourFunc)(PROTO_DECL2 point_t P1, btsbool outside);
...
struct speedo_global_data 
    ...
    btsbool		gMustTranslate;	/* whether frontend.c must translate protocols */
    ...
    btsbool             bb_someCall;            /* flag->some accumulation of extrema occurred in generic bbox acquisition */
...
struct set_width_tag
    ...
    btsbool  first_pass;        /* TRUE during first pass thru outline data */
    btsbool  extents_running;   /* T if extent accumulation for each vector */
    ...
    btsbool   ww_contour_init;          /* set TRUE by begin_contour */
    ...
    fix15     ww_normal[NORTS][2];      /* a table of normals for the white writer algorithm */
#endif /* INCL_WHITE */
    ...
    btsbool  intercept_oflo;   /* TRUE if intercepts data lost */

#endif /* endif incl_black, incl_screen, incl_2d, incl_white, incl_gray */
...
/* bounding box now used by all output modules, including outline */
    ...
    btsbool  x_scan_active;    /* X scan flag during scan conversion */
#endif
    ...
    btsbool  specs_valid;    /* TRUE if fw_set_specs() successful */
    btsbool  curves_out;     /* Allow curves to output module */
    fix15    output_mode;    /* Output module selector */
    btsbool  normal;         /* TRUE if 0 obl and mult of 90 deg rot  */
...
#if INCL_QUICK 
    btsbool sp_banding_active;          /* Set if current point is outside current band */
    ufix16  sp_first_sub_char_offset;   /* Index of first intercept in current sub-char */
    ufix16  sp_first_contour_offset;    /* Index of first intercept in current contour */
    ...
    btsbool sp_clipped_salient;         /* Set if salient detected since leaving band */
    btsbool sp_clipped_first_salient;   /* Set if salient detected outside 
                                         * band before first intercept in 
                                         * current contour */
    btsbool sp_intercept_monitoring;    /* Set to test for intercept overflow at each intercept */
    btsbool sp_horiz_gap_found;         /* Set if horizontal gap detected in current band */
#endif
    ...
    btsbool     outline_device_set;
#endif
#if INCL_BLACK || INCL_SCREEN || INCL_2D || INCL_WHITE || INCL_QUICK || INCL_GRAY
    bitmap_t    bitmap_device;
    btsbool     bitmap_device_set;
#endif
#endif
    ...
    union
        ...
        /* set_trns.c data definitions */
            ...
            btsbool  c_act[MAX_CONSTR];         /* TRUE if constraint currently active */
            ufix16   c_pix[MAX_CONSTR];         /* Size of constrained zone if active */
#endif

#if INCL_ISW
            btsbool import_setwidth_act;        /* boolean to indicate imported setwidth */
            btsbool isw_modified_constants;
...
#if INCL_SQUEEZING
            btsbool squeezing_compound;         /* flag to indicate a compound character */
#endif
...
/* prr modif end */
        ...
        struct
            ...
            btsbool bogus_mode;         /* Linear transformation requested */
            ...
            btsbool non_linear_X;       /* True if X values require non-linear transformation */
            btsbool non_linear_Y;       /* True if Y values require non-linear transformation */
        ...
        struct
            { /* beginning of Type 1 section of union */

/* prr modif begin : needed to protect the true_type global on time initialised tables */
			char	pipo[sizeof(struct pipo_truetype)];
/* prr modif end */
                /**** tr_mkchr.c ****/
            ...
            btsbool  flex_active;	/* Flex mechanism active */
            ...
            stack_item stack[24];	/* BuildChar stack */
            ...
            btsbool  vstem3_active;	/* True if vstem3 hint set */
            btsbool  hstem3_active;	/* True if hstem3 hint set */
            ...
            fix31    y_trans_mode;	/* Mode for calculating transformed Y */
                                        ...
                                        /* 3: function of Y only            */
                                        /* 4: function of -Y only           */
            btsbool  x_trans_ready;	/* True if X transformation data updated from hints */
            btsbool  y_trans_ready;	/* True if Y transformation data updated from hints */
            btsbool  non_linear_X;	/* True if X values require non-linear transformation */
            btsbool  non_linear_Y;	/* True if Y values require non-linear transformation */
            ...
            fix31    Xpix[MAXSTEMZONES];	/* List of X pixel values at breakpoints */
            fix31    Ypix[MAXSTEMZONES];	/* List of Y pixel values at breakpoints */
            ufix32   Xmult[MAXSTEMZONES + 1];	/* List of X interpolation coefficients */
            ufix32   Ymult[MAXSTEMZONES + 1];	/* List of Y interpolation coefficients */
            fix31    Xoffset[MAXSTEMZONES + 1];	/* List of X interpolation constants */
            fix31    Yoffset[MAXSTEMZONES + 1];	/* List of Y interpolation constants */
        ...
        struct
            { /* beginning of TrueDoc section of union */

            void       *CspBufPtr;      /* Pointer to Csp buffer */

            } truedoc;          /* end of truedoc section of union */

#endif  /* PROC_TRUEDOC */

        } processor; /* end of union */

    };          /* End of speedo globals structure typedef */
...
/***********************************************************************************
 *
 *  Speedo global data structure and pointer allocation 
 *
 ***********************************************************************************/
...
GLOBAL_PROTO btsbool sp_make_char(PROTO_DECL2 ufix16 char_index);

#if  INCL_ISW       
GLOBAL_PROTO fix31 sp_compute_isw_scale(PROTO_DECL1);
GLOBAL_PROTO btsbool sp_make_char_isw(PROTO_DECL2 ufix16 char_index, ufix32 imported_width);
#endif
...
GLOBAL_PROTO btsbool sp_get_char_bbox(PROTO_DECL2 ufix16 char_index, bbox_t *bbox);
#endif

GLOBAL_PROTO btsbool sp_is_remote_char(PROTO_DECL2 ufix16 char_index);

        /* do_trns.c functions */

GLOBAL_PROTO void sp_read_bbox(PROTO_DECL2 ufix8 FONTFAR *STACKFAR *pp1, ufix8 FONTFAR *STACKFAR *pp2, point_t STACKFAR *pPmin, point_t STACKFAR *pPmax, btsbool set_flag);
GLOBAL_PROTO void sp_proc_outl_data(PROTO_DECL2 ufix8 FONTFAR *p1, ufix8 FONTFAR *p2);

        /* out_blk.c functions */

#if INCL_BLACK
GLOBAL_PROTO btsbool sp_init_black(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_black(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_black(PROTO_DECL2 point_t P1,btsbool outside);
GLOBAL_PROTO void sp_line_black(PROTO_DECL2 point_t P1);
GLOBAL_PROTO btsbool sp_end_char_black(PROTO_DECL1);
#endif

        /* out_scrn.c functions */

#if INCL_SCREEN
GLOBAL_PROTO btsbool sp_init_screen(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_screen(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_screen(PROTO_DECL2 point_t P1,btsbool outside);
...
GLOBAL_PROTO btsbool sp_end_char_screen(PROTO_DECL1);
#endif
...
#if INCL_OUTLINE
#if INCL_MULTIDEV
GLOBAL_PROTO btsbool sp_set_outline_device(PROTO_DECL2 outline_t *ofuncs, ufix16 size);
#endif

GLOBAL_PROTO btsbool sp_init_outline(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_outline(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_sub_char_outline(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_outline(PROTO_DECL2 point_t P1,btsbool outside);
...
GLOBAL_PROTO btsbool sp_end_char_outline(PROTO_DECL1);
#endif

        /* out_bl2d.c functions */

#if INCL_2D
GLOBAL_PROTO btsbool sp_init_2d(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_2d(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_2d(PROTO_DECL2 point_t P1,btsbool outside);
GLOBAL_PROTO void sp_line_2d(PROTO_DECL2 point_t P1);
GLOBAL_PROTO btsbool sp_end_char_2d(PROTO_DECL1);
#endif

        /* out_wht.c functions */

#if INCL_WHITE
GLOBAL_PROTO btsbool sp_init_white(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_white(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_white(PROTO_DECL2 point_t P1,btsbool outside);
GLOBAL_PROTO void sp_line_white(PROTO_DECL2 point_t P1);
GLOBAL_PROTO void sp_end_contour_white(PROTO_DECL1);
GLOBAL_PROTO btsbool sp_end_char_white(PROTO_DECL1);
#endif

        /* out_util.c functions */

#if INCL_BLACK || INCL_SCREEN || INCL_2D || INCL_WHITE || INCL_GRAY || INCL_QUICK
        
#if INCL_MULTIDEV
GLOBAL_PROTO btsbool sp_set_bitmap_device(PROTO_DECL2 bitmap_t *bfuncs, ufix16 size);
#endif
...
GLOBAL_PROTO btsbool sp_next_band_out(PROTO_DECL1);
#endif

        /* out_qck.c functions */

#if INCL_QUICK
GLOBAL_PROTO btsbool sp_init_quick(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_quick(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_sub_char_quick(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_quick(PROTO_DECL2 point_t P1,btsbool outside);
...
GLOBAL_PROTO btsbool sp_end_char_quick(PROTO_DECL1);
#endif

        /* out_gry.c functions */

#if INCL_GRAY
GLOBAL_PROTO btsbool sp_init_gray(PROTO_DECL2 specs_t GLOBALFAR *specsarg);
GLOBAL_PROTO btsbool sp_begin_char_gray(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_sub_char_gray(PROTO_DECL2 point_t Psw,point_t Pmin,point_t Pmax);
GLOBAL_PROTO void sp_begin_contour_gray(PROTO_DECL2 point_t P1, btsbool outside);
GLOBAL_PROTO void sp_curve_gray(PROTO_DECL2 point_t P1,point_t P2,point_t P3, fix15 depth);
GLOBAL_PROTO void sp_line_gray(PROTO_DECL2 point_t P1);
GLOBAL_PROTO void sp_end_contour_gray(PROTO_DECL1);
GLOBAL_PROTO void sp_end_sub_char_gray(PROTO_DECL1);
GLOBAL_PROTO btsbool sp_end_char_gray(PROTO_DECL1);
#endif

#if INCL_USEROUT
GLOBAL_PROTO btsbool sp_init_userout(PROTO_DECL2 specs_t *specsarg);
#endif
...
GLOBAL_PROTO btsbool sp_set_specs(PROTO_DECL2 specs_t STACKFAR *specsarg);
GLOBAL_PROTO void sp_type_tcb(PROTO_DECL2 tcb_t GLOBALFAR *ptcb);
GLOBAL_PROTO btsbool sp_setup_consts(PROTO_DECL2 fix15 xmin, fix15 xmax, fix15 ymin, fix15 ymax);
...
GLOBAL_PROTO void WDECL sp_set_bitmap_pixels(PROTO_DECL2 fix15 y, fix15 xbit1, fix15 num_grays, void *gray_buf);
...
GLOBAL_PROTO void sp_start_contour(PROTO_DECL2 fix31 x,fix31 y,btsbool outside);
...
#if INCL_LCD
GLOBAL_PROTO buff_t STACKFAR * WDECL sp_load_char_data(PROTO_DECL2 fix31 file_offset, fix15 no_bytes, fix15 cb_offset);
#endif

#if INCL_PLAID_OUT
GLOBAL_PROTO void sp_record_xint(PROTO_DECL2 fix15 int_num);
GLOBAL_PROTO void sp_record_yint(PROTO_DECL2 fix15 int_num);
GLOBAL_PROTO void sp_begin_plaid_data(PROTO_DECL1);
GLOBAL_PROTO void sp_begin_ctrl_zones(PROTO_DECL2 fix15, no_X_zones, fix15 no_Y_zones);
GLOBAL_PROTO void sp_record_ctrl_zone(PROTO_DECL2 fix31 start, fix31 end, fix15 constr);
GLOBAL_PROTO void sp_begin_int_zones(PROTO_DECL2 fix15 no_X_int_zones, fix15 no_Y_int_zones);
GLOBAL_PROTO void sp_record_int_zone(PROTO_DECL2 fix31 start, fix31 end);
GLOBAL_PROTO void sp_end_plaid_data(PROTO_DECL1);
#endif
