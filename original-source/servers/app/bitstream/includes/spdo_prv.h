...
*                                                                         
*     $Header: //bliss/user/archive/rcs/speedo/spdo_prv.h,v 33.0 97/03/17 17:47:54 shawn Release $
*                                                                                    
*     $Log:	spdo_prv.h,v $
*       Revision 33.0  97/03/17  17:47:54  shawn
*       TDIS Version 6.00
*       
*       Revision 32.1  96/07/01  09:37:50  shawn
*       Modified Multi-device support Macros to include sp_set_bitmap_pixels() function pointer.
*       
...
#define NEXT_WORD(A) \
        ((fix15)(sp_globals.processor.speedo.key32 \
        ^ ((A) += 2, ((fix15)((A)[-1]) << 8) | (fix15)((A)[-2]))))
...
#define NEXT_BYTES(A, B) \
        (((B = (ufix16)(*(A)++) \
        ^ sp_globals.processor.speedo.key7) >= 248)? \
        ((ufix16)(B & 0x07) << 8) + ((*(A)++) \
        ^ sp_globals.processor.speedo.key8) + 248: B)
...
#define set_bitmap_pixels(y, xbit1, num_grays, pGray_buf)(*sp_globals.bitmap_device.p_set_pixels)(PARAMS2 y, xbit1, num_grays, pGray_buf)
...
#define set_bitmap_pixels(y, xbit1, num_grays, pGray_buf) sp_set_bitmap_pixels(PARAMS2 y, xbit1, num_grays, pGray_buf)
