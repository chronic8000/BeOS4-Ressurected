...
*                                                                                    *
*     $Header: //bliss/user/archive/rcs/true_type/sfnt.h,v 14.0 97/03/17 17:54:51 shawn Release $                                                                       *
*                                                                                    *
*     $Log:	sfnt.h,v $
 * Revision 14.0  97/03/17  17:54:51  shawn
 * TDIS Version 6.00
 * 
 * Revision 10.1  96/03/14  15:54:54  shawn
 * Removed alignment pragma (line 105) to avoid compiler warnings
 * 
 * Revision 10.0  95/02/15  16:25:28  roberte
 * Release
 * 
 * Revision 9.1  95/01/04  16:38:08  roberte
 * Release
 * 
 * Revision 8.2  95/01/03  13:17:46  shawn
 * Converted to ANSI 'C'
 * Modified for support by the K&R conversion utility
 * 
 * Revision 8.1  94/07/19  10:46:12  roberte
 * Additional data structure definitions for mort table access, now
 * used for vertical character substitution if INCL_VERTICAL.
 * Additional data structure definitions for kern table acces, now
 * used for tt_get_pair_kern() if INCL_METRICS. 
 * 
 * Revision 8.0  94/05/04  09:33:12  roberte
 * Release
 * 
 * Revision 7.0  94/04/08  11:59:21  roberte
 * Release
 * 
 * Revision 6.91  93/08/30  14:53:10  roberte
 * Release
 * 
 * Revision 6.45  93/05/21  09:48:44  roberte
 * Added INCL_PCLETTO logic so this option can add PCLETTO handling on top of TrueType handling.
 * For function prototypes only.
 * 
...
typedef struct
	...
	uint32	defaultFlags;
	uint32	chainLength;
	uint16	nFeatureEntries;
	uint16	nSubtables;
        } sfnt_MortChainHeader;
...
typedef struct
	...
	uint16	length;
	uint16	coverage;
	uint32	subFeatureFlags;
        } sfnt_FeatureSubTableHeader;

typedef struct
        {
	sfnt_MortChainHeader	mortChainHeader;
	sfnt_FeatureTable	FeatureArray[1];
        } sfnt_MortChain;
...
typedef struct
	...
	uint16	rangeShift;
	uint16	LookupSingle[1];	/* uint16 LookupSingle [nUnits*2+2] */
        } sfnt_GlyphLookup;

typedef struct
        {
	sfnt_FeatureSubTableHeader	subHeader;
	sfnt_GlyphLookup		lookupTable;
        } sfnt_FeatureSubTable;

/* Kerning data structures */
...
#ifdef PCLETTO
GLOBAL_PROTO char *tt_get_char_addr(uint16 char_index, int32 *length);
GLOBAL_PROTO uint16 tt_UnicodeToIndex(uint16 char_code);
#else
#if INCL_PCLETTO
/* same as above */
GLOBAL_PROTO char *tt_get_char_addr (uint16 char_index, int32 *length);
