 ...
 * $Header: //bliss/user/archive/rcs/type1/tr_trans.c,v 33.0 97/03/17 18:00:22 shawn Release $
 *
 * $Log:	tr_trans.c,v $
 * Revision 33.0  97/03/17  18:00:22  shawn
 * TDIS Version 6.00
 * 
 * Revision 32.5  96/07/02  12:52:53  shawn
 * Changed boolean to btsbool.
 * 
 * Revision 32.4  96/02/13  16:48:32  shawn
 * Recoded statements in real_trans_a() to fix incorrect use of matrix pointer.
 * Removed two MAX() statements in function set_fixed_point() that were not used.
 * 
 * Revision 32.3  95/11/17  11:05:48  shawn
 * Changed "FUNCTION static" declarations to "FUNCTION LOCAL_PROTO"
 * 
 * Revision 32.2  95/09/21  10:36:55  john
 * Corrected SHOW macros for ansi compatibility.
 * Added prototypes for debug functions print_[xy]_trans().
 * Corrected calculation of half pixel in vertical stem snap
 * table setup within the function set_stem_tables(). It was 
 * using y_pix_per_oru rather than x_pix_per_oru.
 * 
 * Revision 32.1  95/09/13  11:27:48  john
 * Modified set_mode_flags() to enforce linear mode when pixels per oru
 * is one or more. Decisions for X and Y are independent. This is a workaround
 * to a problem with the blue value mechanism that can result in alignment
 * adjustments of more than one pixel at large sizes. The long-term solution
 * is to replace the blue value mechanism with one like that used in TrueDoc.
 * 
 * Revision 32.0  95/02/15  16:47:05  shawn
 * Release
 * 
 * Revision 31.4  95/01/24  17:46:51  shawn
 * Corrected spelling of Macro SPGLPBPTR2 in do_vstem3()
 * 
 * Revision 31.3  95/01/10  09:26:53  roberte
 * Updated copyright header.
 * 
 * Revision 31.2  95/01/05  17:09:30  shawn
 * Add LOCAL_PROTO for set_mode_flags()
 * 
 * Revision 31.1  95/01/04  16:39:54  roberte
 * Release
 * 
 * Revision 30.2  94/12/16  15:25:16  shawn
 * Converted to ANSI 'C'
 * Modified for support by the K&R conversion utility
 * 
 * Revision 30.1  94/10/25  09:57:55  roberte
 * Remove include of stdef.h. Data types now seen in nested include of btstypes.h from speedo.h.
 * 
 * Revision 30.0  94/05/04  09:33:37  roberte
 * Release
 * 
 * Revision 29.0  94/04/08  12:11:09  roberte
 * Release
 * 
 * Revision 28.31  93/10/12  12:57:13  roberte
 * Added more explicit casting of some assignments and parameters passed to functions.
 * 
 * Revision 28.30  93/08/30  15:45:30  roberte
 * Release
 * 
 * Revision 28.26  93/05/28  09:19:45  roberte
 * Added #if PROC_TYPE1 conditional around entire file, after include of spdo_prv.h
 * 
 * Revision 28.25  93/04/29  10:10:15  roberte
 * Shortened long lines and prettied up.
 * 
...
static char     rcsid[] = "$Header: //bliss/user/archive/rcs/type1/tr_trans.c,v 33.0 97/03/17 18:00:22 shawn Release $";
...
#ifndef TR_LINEAR_PPO_THRESH
#define TR_LINEAR_PPO_THRESH 0.9999 /* Min scale factor for linear scaling */
#endif
...
/* useful macros formerly in stdef.h: */
...
static btsbool  vstem3_active;	/* True if vstem3 hint set */
static btsbool  hstem3_active;	/* True if hstem3 hint set */
...
static btsbool  x_trans_ready;	/* True if X transformation data updated fromhints */
static btsbool  y_trans_ready;	/* True if Y transformation data updated from hints */
static btsbool  non_linear_X;	/* True if X values require non-linear transformation */
static btsbool  non_linear_Y;	/* True if Y values require non-linear transformation */
...
static fix31    Xpix[MAXSTEMZONES];	/* List of X pixel values at breakpoints */
static fix31    Ypix[MAXSTEMZONES];	/* List of Y pixel values at breakpoints */
static ufix32   Xmult[MAXSTEMZONES + 1];	/* List of X interpolation coefficients */
static ufix32   Ymult[MAXSTEMZONES + 1];	/* List of Y interpolation coefficients */
static fix31    Xoffset[MAXSTEMZONES + 1];	/* List of X interpolation constants */
static fix31    Yoffset[MAXSTEMZONES + 1];	/* List of Y interpolation constants */
...
LOCAL_PROTO void real_trans_a(real STACKFAR *matrix, real STACKFAR *x, real STACKFAR *y);
LOCAL_PROTO void set_fixed_point(SPGLOBPTR2 real STACKFAR *matrix, fbbox_t STACKFAR *font_bbox);
LOCAL_PROTO void set_blue_values(SPGLOBPTR2 font_hints_t STACKFAR *pfont_hints);
LOCAL_PROTO void set_stem_tables(SPGLOBPTR2 font_hints_t STACKFAR *pfont_hints);
LOCAL_PROTO void set_mode_flags(SPGLOBPTR2 fbbox_t STACKFAR *font_bbox);

#if EDGE_ALIGN
LOCAL_PROTO void align_hstem(SPGLOBPTR2 fix31 y1, fix31 y2, fix31 STACKFAR *pcy_pix_bottom, fix31 STACKFAR *pcy_pix_top);
#else
LOCAL_PROTO void align_hstem(SPGLOBPTR2 fix31 y1, fix31 y2, fix31 STACKFAR *pcy_pix);
#endif

LOCAL_PROTO void align_vstem(SPGLOBPTR2 fix31 x1, fix31 x);
LOCAL_PROTO	fix31 vstemweight(SPGLOBPTR2 fix31 dx);
LOCAL_PROTO	void add_x_constraint(SPGLOBPTR2 fix31 orus, fix31 pix);
LOCAL_PROTO	fix31 hstemweight(SPGLOBPTR2 fix31 dy);
LOCAL_PROTO	void add_y_constraint(SPGLOBPTR2 fix31 orus, fix31 pix);
LOCAL_PROTO	void update_x_trans(SPGLOBPTR1);
LOCAL_PROTO	void update_y_trans(SPGLOBPTR1);

#if DEBUG
LOCAL_PROTO	void print_x_trans(void);
LOCAL_PROTO	void print_y_trans(void);
#endif


FUNCTION void init_trans_a(SPGLOBPTR2 real STACKFAR *matrix, fbbox_t STACKFAR *font_bbox)
/*
 ...
 */
{
	...
	for (i = 0; i < 6; i++) {
		sp_globals.processor.type1.local_matrix_i[i] = 
				(fix31) ROUND(matrix[i] * (real) sp_globals.processor.type1.mk_onepix);
	}
	...
	printf("%6.1f %6.1f %8.3f\n",
	       sp_globals.processor.type1.vstem_std.min_orus,
		   sp_globals.processor.type1.vstem_std.max_orus,
		   sp_globals.processor.type1.vstem_std.pix);

	printf("\nStandard horizontal stem control\n");
	printf("%6.1f %6.1f %8.3f\n",
	       sp_globals.processor.type1.hstem_std.min_orus,
		   sp_globals.processor.type1.hstem_std.max_orus,
		   sp_globals.processor.type1.hstem_std.pix);
	...
	if (sp_globals.processor.type1.no_vstem_snaps != 0) {
		...
		for (i = 0; i < sp_globals.processor.type1.no_vstem_snaps; i++) {
			printf("%6.1f %6.1f %8.3f\n",
			       sp_globals.processor.type1.vstem_snaps[i].min_orus,
				   sp_globals.processor.type1.vstem_snaps[i].max_orus,
				   sp_globals.processor.type1.vstem_snaps[i].pix);
		}
	}
	if (sp_globals.processor.type1.no_hstem_snaps != 0) {
		...
		for (i = 0; i < sp_globals.processor.type1.no_hstem_snaps; i++) {
			printf("%6.1f %6.1f %8.3f\n",
			       sp_globals.processor.type1.hstem_snaps[i].min_orus,
				   sp_globals.processor.type1.hstem_snaps[i].max_orus,
				   sp_globals.processor.type1.hstem_snaps[i].pix);
		}
	}
#endif
}
...
FUNCTION LOCAL_PROTO void set_mode_flags(SPGLOBPTR2 fbbox_t STACKFAR *font_bbox)
/*
 ...
 */
{
	...
	if (dxdx > thresh) {	/* Transformed X increases over X range? */
		...
		if (dxdy < -thresh)	/* Transformed X decreases over Y range? */
			goto next;
        if (sp_globals.processor.type1.local_matrix[0] >= TR_LINEAR_PPO_THRESH)
            goto next;
		sp_globals.processor.type1.x_pix_per_oru = 
            sp_globals.processor.type1.local_matrix[0];
		sp_globals.processor.type1.non_linear_X = TRUE;
		sp_globals.processor.type1.x_trans_mode = 1; /* X is a function of +X only */
		goto next;
	}
	if (dxdx < -thresh) {	/* Transformed X decreases over X range? */
		...
		sp_globals.processor.type1.x_pix_per_oru = 
            -sp_globals.processor.type1.local_matrix[0];
		sp_globals.processor.type1.non_linear_X = TRUE;
		sp_globals.processor.type1.x_trans_mode = 2; /* X is a function of -X only */
		goto next;
	}
	if (dxdy > thresh) {	/* Transformed X increases over Y range? */
        if (sp_globals.processor.type1.local_matrix[2] >= TR_LINEAR_PPO_THRESH)
            goto next;
		sp_globals.processor.type1.y_pix_per_oru = 
            sp_globals.processor.type1.local_matrix[2];
		sp_globals.processor.type1.non_linear_Y = TRUE;
		sp_globals.processor.type1.x_trans_mode = 3; /* X is a function of +Y only */
		goto next;
	}
	if (dxdy < -thresh) {	/* Transformed X decreases over Y range? */
        if (-sp_globals.processor.type1.local_matrix[2] >= TR_LINEAR_PPO_THRESH)
		...
		sp_globals.processor.type1.y_pix_per_oru = 
            -sp_globals.processor.type1.local_matrix[2];
		sp_globals.processor.type1.non_linear_Y = TRUE;
		sp_globals.processor.type1.x_trans_mode = 4; /* X is a function of -Y only */
		goto next;
	}
...
next:
	if (dydx > thresh) {	/* Transformed Y increases over X range? */
		...
		if (dydy < -thresh)	/* Transformed Y decreases over Y range? */
			return;
        if (sp_globals.processor.type1.local_matrix[1] >= TR_LINEAR_PPO_THRESH)
            return;
		sp_globals.processor.type1.x_pix_per_oru = 
            sp_globals.processor.type1.local_matrix[1];
		sp_globals.processor.type1.non_linear_X = TRUE;
		sp_globals.processor.type1.y_trans_mode = 1; /* Y is a function of +X only */
		return;
	}
	if (dydx < -thresh) {	/* Transformed Y decreases over X range? */
		...
		sp_globals.processor.type1.x_pix_per_oru = 
            -sp_globals.processor.type1.local_matrix[1];
		sp_globals.processor.type1.non_linear_X = TRUE;
		sp_globals.processor.type1.y_trans_mode = 2; /* Y is a function of -X only */
		return;
	}
	if (dydy > thresh) {	/* Transformed Y increases over Y range? */
        if (sp_globals.processor.type1.local_matrix[3] >= TR_LINEAR_PPO_THRESH)
            return;
		sp_globals.processor.type1.y_pix_per_oru = 
            sp_globals.processor.type1.local_matrix[3];
		sp_globals.processor.type1.non_linear_Y = TRUE;
		sp_globals.processor.type1.y_trans_mode = 3; /* Y is a function of +Y only */
		return;
	}
	if (dydy < -thresh) {	/* Transformed Y decreases over Y range */
        if (-sp_globals.processor.type1.local_matrix[3] >= TR_LINEAR_PPO_THRESH)
		...
		sp_globals.processor.type1.y_pix_per_oru = 
            -sp_globals.processor.type1.local_matrix[3];
		sp_globals.processor.type1.non_linear_Y = TRUE;
		sp_globals.processor.type1.y_trans_mode = 4; /* Y is a function of -Y only */
		return;
	}
}


FUNCTION LOCAL_PROTO void real_trans_a(real STACKFAR *matrix, real STACKFAR *x, real STACKFAR *y)
{
	real X, Y;
...
FUNCTION LOCAL_PROTO void set_fixed_point(SPGLOBPTR2 real STACKFAR *matrix, fbbox_t STACKFAR *font_bbox)
/*
 ...
 */
{
	...
	sp_globals.processor.type1.x_pix_per_oru_i = 
			(fix31) (sp_globals.processor.type1.x_pix_per_oru * (real) sp_globals.processor.type1.tr_onepix);
	sp_globals.processor.type1.y_pix_per_oru_i =
			(fix31) (sp_globals.processor.type1.y_pix_per_oru * (real) sp_globals.processor.type1.tr_onepix);
	sp_globals.processor.type1.x_pix_per_oru_r =
			sp_globals.processor.type1.x_pix_per_oru * (real) sp_globals.processor.type1.tr_onepix;
	sp_globals.processor.type1.y_pix_per_oru_r =
			sp_globals.processor.type1.y_pix_per_oru * (real) sp_globals.processor.type1.tr_onepix;
	sp_globals.processor.type1.fudge_x =
			(fix31) (sp_globals.processor.type1.x_pix_per_oru * (real) sp_globals.processor.type1.mk_onepix) - (sp_globals.processor.type1.x_pix_per_oru_i << 4);
	sp_globals.processor.type1.fudge_y =
			(fix31) (sp_globals.processor.type1.y_pix_per_oru * (real) sp_globals.processor.type1.mk_onepix) - (sp_globals.processor.type1.y_pix_per_oru_i << 4);
...
FUNCTION LOCAL_PROTO void set_blue_values(SPGLOBPTR2 font_hints_t STACKFAR *pfont_hints)
/*
 * Sets up data in sp_globals.processor.type1.top_zones[] and 
 * sp_globals.processor.type1.bottom_zones[] to control vertical
 * alignment 
 */
{
	int             i, j;
	btsbool         suppr_oversht;	/* True if overshoot supression active */
	...
	for (j = 2; j < pfont_hints->no_blue_values;) {
		sp_globals.processor.type1.top_zones[i].bottom_orus =
				(fix15) ROUND(pfont_hints->pblue_values[j++] * 16.0);
		sp_globals.processor.type1.top_zones[i].top_orus =
				(fix15) ROUND(pfont_hints->pblue_values[j++] * 16.0);
		i++;
	}
	...
	if (pfont_hints->no_blue_values >= 2) {
		sp_globals.processor.type1.bottom_zones[i].bottom_orus =
				(fix15) ROUND(pfont_hints->pblue_values[0] * 16.0);
		sp_globals.processor.type1.bottom_zones[i].top_orus =
				(fix15) ROUND(pfont_hints->pblue_values[1] * 16.0);
		i++;
	}
	for (j = 0; j < pfont_hints->no_other_blues;) {
		sp_globals.processor.type1.bottom_zones[i].bottom_orus =
				(fix15) ROUND(pfont_hints->pother_blues[j++] * 16.0);
		sp_globals.processor.type1.bottom_zones[i].top_orus =
				(fix15) ROUND(pfont_hints->pother_blues[j++] * 16.0);
		i++;
	}
	...
	if (pfont_hints->blue_shift > -1) {	/* BlueShift specified in
						 * font? */
		blue_shift = (fix15)(pfont_hints->blue_shift << 4);
		thresh = ((fix31)orus_per_half_pix > (pfont_hints->blue_shift << 4)) ?
			(fix15)(pfont_hints->blue_shift << 4) :
			orus_per_half_pix;
	...
	if (pfont_hints->blue_fuzz > -1) {	/* BlueFuzz specified in
						 * font? */
		blue_fuzz = (int)(pfont_hints->blue_fuzz << 4);	/* Use value from font */
	} else {		/* No BlueFuzz keyword in font? */
		blue_fuzz = 1 << 4;	/* Use default value of 1 */
	}

	for (i = 0; i < sp_globals.processor.type1.no_top_zones; i++) {
		sp_globals.processor.type1.top_zones[i].pix =
				(sp_globals.processor.type1.top_zones[i].top_orus * sp_globals.processor.type1.y_pix_per_oru_i
		...
		if (suppr_oversht) {
			...
			sp_globals.processor.type1.top_zones[i].top_orus =
					(fix15)(((fix31) ((real) (sp_globals.processor.type1.top_zones[i].pix >> sp_globals.processor.type1.mk_shift) * 1.0 / sp_globals.processor.type1.y_pix_per_oru) << 4) + orus_per_half_pix);
			sp_globals.processor.type1.top_zones[i].bottom_orus =
					sp_globals.processor.type1.top_zones[i].bottom_orus + blue_shift;
	...
	for (i = 0; i < sp_globals.processor.type1.no_bottom_zones; i++) {
		sp_globals.processor.type1.bottom_zones[i].pix =
				(sp_globals.processor.type1.bottom_zones[i].top_orus * sp_globals.processor.type1.y_pix_per_oru_i
		...
		if (suppr_oversht) {
			...
			sp_globals.processor.type1.bottom_zones[i].bottom_orus =
					sp_globals.processor.type1.bottom_zones[i].top_orus - thresh - blue_fuzz;
		}
	}
...
FUNCTION LOCAL_PROTO void set_stem_tables(SPGLOBPTR2 font_hints_t STACKFAR *pfont_hints)
/*
 ...
 */
{
	...
	if (sp_globals.processor.type1.non_linear_X) {	/* Hints active in character X dimension? */
		...
		sp_globals.processor.type1.vstem_std.min_orus =
				((thresh1 > thresh2) ? thresh1 : thresh2) / sp_globals.processor.type1.x_pix_per_oru;
	...
	if (sp_globals.processor.type1.non_linear_Y) {	/* Hints active in character Y dimension? */
		...
		sp_globals.processor.type1.hstem_std.min_orus =
				((thresh1 > thresh2) ? thresh1 : thresh2) / sp_globals.processor.type1.y_pix_per_oru;
	...
	if (sp_globals.processor.type1.non_linear_X) {	/* Hints active in character X dimension? */
		thresh = 0.5 / sp_globals.processor.type1.x_pix_per_oru;	/* one half pixel */
		...
		for (i = 0; i < sp_globals.processor.type1.no_vstem_snaps; i++) {
			...
			sp_globals.processor.type1.vstem_snaps[i].pix =
					floor(pfont_hints->pstem_snap_v[i] * sp_globals.processor.type1.x_pix_per_oru + 0.5);
	...
	if (sp_globals.processor.type1.non_linear_Y) {	/* Hints active in character Y dimension? */
		...
		for (i = 0; i < sp_globals.processor.type1.no_hstem_snaps; i++) {
			...
			sp_globals.processor.type1.hstem_snaps[i].pix =
					floor(pfont_hints->pstem_snap_h[i] * sp_globals.processor.type1.y_pix_per_oru + 0.5);
	...
	sp_globals.processor.type1.i_minvstemweight =
			(fix31) ROUND(sp_globals.processor.type1.minvstemweight * (real) sp_globals.processor.type1.mk_onepix);
	sp_globals.processor.type1.i_minhstemweight =
			(fix31) ROUND(sp_globals.processor.type1.minhstemweight * (real) sp_globals.processor.type1.mk_onepix);

	sp_globals.processor.type1.i_vstem_std.min_orus =
		(fix15) ROUND(sp_globals.processor.type1.vstem_std.min_orus * 16.0);
	sp_globals.processor.type1.i_vstem_std.max_orus =
			(fix15) ROUND(sp_globals.processor.type1.vstem_std.max_orus * 16.0);
	sp_globals.processor.type1.i_vstem_std.pix =
			(fix31) ROUND(sp_globals.processor.type1.vstem_std.pix * (real) sp_globals.processor.type1.mk_onepix);

	sp_globals.processor.type1.i_hstem_std.min_orus =
			(fix15) ROUND(sp_globals.processor.type1.hstem_std.min_orus * 16.0);
	sp_globals.processor.type1.i_hstem_std.max_orus =
			(fix15) ROUND(sp_globals.processor.type1.hstem_std.max_orus * 16.0);
	sp_globals.processor.type1.i_hstem_std.pix =
			(fix31) ROUND(sp_globals.processor.type1.hstem_std.pix * (real) sp_globals.processor.type1.mk_onepix);

	if (sp_globals.processor.type1.no_vstem_snaps != 0) {
		for (i = 0; i < sp_globals.processor.type1.no_vstem_snaps; i++) {
			sp_globals.processor.type1.i_vstem_snaps[i].min_orus =
					(fix15) ROUND(sp_globals.processor.type1.vstem_snaps[i].min_orus * 16.0);
			sp_globals.processor.type1.i_vstem_snaps[i].max_orus =
					(fix15) ROUND(sp_globals.processor.type1.vstem_snaps[i].max_orus * 16.0);
			sp_globals.processor.type1.i_vstem_snaps[i].pix =
					(fix31) ROUND(sp_globals.processor.type1.vstem_snaps[i].pix * (real) sp_globals.processor.type1.mk_onepix);
		}
	}
	if (sp_globals.processor.type1.no_hstem_snaps != 0) {
		for (i = 0; i < sp_globals.processor.type1.no_hstem_snaps; i++) {
			sp_globals.processor.type1.i_hstem_snaps[i].min_orus =
					(fix15) ROUND(sp_globals.processor.type1.hstem_snaps[i].min_orus * 16.0);
			sp_globals.processor.type1.i_hstem_snaps[i].max_orus =
					(fix15) ROUND(sp_globals.processor.type1.hstem_snaps[i].max_orus * 16.0);
			sp_globals.processor.type1.i_hstem_snaps[i].pix =
					(fix31) ROUND(sp_globals.processor.type1.hstem_snaps[i].pix * (real) sp_globals.processor.type1.mk_onepix);
		}
	}
}
...
FUNCTION void clear_constraints(SPGLOBPTR1)
/*
...
FUNCTION void do_hstem(SPGLOBPTR2 fix31 sby, fix31 y, fix31 dy)
/*
...
FUNCTION void do_hstem3(SPGLOBPTR2 fix31 sby, fix31 y0, fix31 dy0, fix31 y1, fix31 dy1, fix31 y2, fix31 dy2)
{
	fix31           bottom;
	fix31           temp_bottom;
	...
	fix31           strkwt_pix;	/* Thickness of center stroke indevice units */
	...
	hint_sort3((fix31 STACKFAR*)&y0,
				(fix31 STACKFAR*)&dy0,
				(fix31 STACKFAR*)&y1,
				(fix31 STACKFAR*)&dy1,
				(fix31 STACKFAR*)&y2,
				(fix31 STACKFAR*)&dy2);	/* Sort strokes into
							 			* ascending order */
	...
	temp_bottom = (bottom * sp_globals.processor.type1.y_pix_per_oru_i) 
					+ ((bottom >> 4) * sp_globals.processor.type1.fudge_x);
	pix_bottom = (temp_bottom + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 								* boundary */
	...
	temp_bottom = (bottom * sp_globals.processor.type1.y_pix_per_oru_i) 
					+ ((bottom >> 4) * sp_globals.processor.type1.fudge_x);
	bpix_bottom = (temp_bottom + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 * boundary */
	...
	temp_bottom = (bottom * sp_globals.processor.type1.y_pix_per_oru_i) 
					+ ((bottom >> 4) * sp_globals.processor.type1.fudge_x);
	tpix_bottom = (temp_bottom + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 * boundary */
	...
	if (t1 > t2) {		/* adjust narrower if necessary - bottom
				 * stroke */
		t = t1 - t2;
		bpix_bottom = (bpix_bottom + t + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
		bpix_top = (bpix_top + t + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
	} else if (t2 > t1) {	/* adjust narrower if necessary - top stroke */
		t = t2 - t1;
		tpix_bottom = (tpix_bottom - t + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
		tpix_top = (tpix_top - t + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
	}
...
FUNCTION void do_hstem3(SPGLOBPTR2 fix31 sby, fix31 y0, fix31 dy0, fix31 y1, fix31 dy1, fix31 y2, fix31 dy2)
{
	fix31           cy1;	/* Centerline of middle stroke in outline units */
	fix31           cy0_pix;/* Centerline of lower stroke in device units */
	fix31           cy1_pix;/* Centerline of middle stroke in device units */
	fix31           cy2_pix;/* Centerline of upper stroke in device units */
	fix31           strkwt_pix;	/* Thickness of center stroke in device units */
	...
	if (strkwt_pix & sp_globals.processor.type1.mk_onepix) {	/* Odd number of pixels? */
		cy1_pix = (cy1_pix & sp_globals.processor.type1.mk_fix) 
					+ sp_globals.processor.type1.mk_rnd;	/* Round to nearest
							 								* pixel center */
	} else {
		cy1_pix = ((cy1_pix + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix);	/* Round to nearest
								 								* pixel boundary */
	}
...
FUNCTION void do_vstem(SPGLOBPTR2 fix31 sbx, fix31 x, fix31 dx)
/*
...
FUNCTION void do_vstem3(SPGLOBPTR2 fix31 sbx, fix31 x0, fix31 dx0, fix31 x1, fix31 dx1, fix31 x2, fix31 dx2)
{
	...
	temp_left = (left * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((left >> 4) * sp_globals.processor.type1.fudge_x);
	pix_left = (temp_left + sp_globals.processor.type1.mk_rnd) 
				& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 							* boundary */
	...
	temp_left = (left * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((left >> 4) * sp_globals.processor.type1.fudge_x);
	lpix_left = (temp_left + sp_globals.processor.type1.mk_rnd) 
				& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 							* boundary */
	...
	temp_left = (left * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((left >> 4) * sp_globals.processor.type1.fudge_x);
	rpix_left = (temp_left + sp_globals.processor.type1.mk_rnd) 
				& sp_globals.processor.type1.mk_fix;	/* Round to pixel
							 							* boundary */
	...
	if (t1 > t2) {		/* adjust narrower if necessary - left stroke */
		t = t1 - t2;
		lpix_left = (lpix_left + t + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;
		lpix_right = (lpix_right + t + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;
	} else if (t2 > t1) {	/* adjust narrower if necessary - right
				 			* stroke */
		t = t2 - t1;
		rpix_left = (rpix_left - t + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;
		rpix_right = (rpix_right - t + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;
	}
...
FUNCTION void do_vstem3(SPGLOBPTR2 fix31 sbx, fix31 x0, fix31 dx0, fix31 x1, fix31 dx1, fix31 x2, fix31 dx2)
{
	fix31           cx1;	/* Centerline of middle stroke in outline units */
	fix31           cx1_pix;/* Centerline of middle stroke in device units */
	fix31           strkwt0_pix;	/* Thickness of left and right strokes in device units */
	fix31           strkwt1_pix;	/* Thickness of center stroke in device units */
	fix31           xleft_pix;
	fix31           xright_pix;
	fix31           halfspan;	/* Center left stem to center middle stem in outline units */
	fix31           halfspan_pix;	/* Center left stem to center middle stem in device units */
	fix31           cx1_temp;
	fix31           halfspan_temp;
	...
	cx1_temp = (cx1 * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((cx1 >> 4) * sp_globals.processor.type1.fudge_x);
	if (strkwt1_pix & sp_globals.processor.type1.mk_onepix) {	/* Center stroke has odd number of
					 											* pixels? */
		cx1_pix = (cx1_temp & sp_globals.processor.type1.mk_fix) 
					+ sp_globals.processor.type1.mk_rnd;	/* Round centerline to
							 								* pixel center */
	} else {
		cx1_pix = (cx1_temp + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round centerline to
							 								* pixel boundary */
	}
	...
	halfspan = (x2 - x0) >> 1;
	halfspan_temp = (halfspan * sp_globals.processor.type1.x_pix_per_oru_i) 
					+ ((halfspan >> 4) * sp_globals.processor.type1.fudge_x);
	if ((strkwt0_pix + strkwt1_pix) & sp_globals.processor.type1.mk_onepix) {/* Left + center stroke
							 												* has odd number of
							 												* pixels? */
		halfspan_pix = (halfspan_temp & sp_globals.processor.type1.mk_fix) 
						+ sp_globals.processor.type1.mk_rnd;	/* Round halfspan to
									 							* nearest half pixel */
	} else {
		halfspan_pix = (halfspan_temp + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;	/* Round halfspan to
									 							* nearest pixel */
	}
...
FUNCTION void hint_sort3(fix31 STACKFAR *px0, fix31 STACKFAR *pdx0, fix31 STACKFAR *px1,
...
FUNCTION void hint_sort2(fix31 STACKFAR *px0, fix31 STACKFAR *pdx0, fix31 STACKFAR *px1, fix31 STACKFAR *pdx1)
/*
...
FUNCTION LOCAL_PROTO void align_vstem(SPGLOBPTR2 fix31 x1, fix31 x2)
/*
 * Aligns a vertical stem to whole pixel boundaries Returns pixel coordinates
 * of centerline of aligned stem 
 */
{
	fix31           strkwt_pix;	/* Thickness of stroke in device units */
	fix31           cx_pix_left;	/* left edge of stroke in device units */
	fix31           cx_temp_left;	/* right edge of stroke in device units */
	 ...
	 * accuracy 
	 */
	cx_temp_left = (x1 * sp_globals.processor.type1.x_pix_per_oru_i) 
					+ ((x1 >> 4) * sp_globals.processor.type1.fudge_x);
	cx_pix_left = (cx_temp_left + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to nearest
							 								* pixel boundary */
 ...
 */
{
	fix31           cx;	/* Position of centerline of stroke in outline units */
	fix31           strkwt_pix;	/* Thickness of stroke in device units */
	fix31           cx_pix;	/* Position of centerline of stroke in device units */
	fix31           cx_temp;/* Position of centerline of stroke in device units */
	 ...
	 */
	cx = (x1 + x2) >> 1;
	cx_temp = (cx * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((cx >> 4) * sp_globals.processor.type1.fudge_x);
	if (strkwt_pix & sp_globals.processor.type1.mk_onepix) {	/* Odd number of pixels? */
		cx_pix = (cx_temp & sp_globals.processor.type1.mk_fix) 
					+ sp_globals.processor.type1.mk_rnd;	/* Round to nearest
							 								* pixel center */
	} else {
		cx_pix = (cx_temp + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to nearest
							 								* pixel boundary */
	}
...
FUNCTION LOCAL_PROTO fix31 vstemweight(SPGLOBPTR2 fix31 dx)
{
	int             i;
	fix31           dx_pix;
	...
	dx_pix = ((dx * sp_globals.processor.type1.x_pix_per_oru_i) 
				+ ((dx >> 4) * sp_globals.processor.type1.fudge_x1) 
				+ sp_globals.processor.type1.mk_rnd) & sp_globals.processor.type1.mk_fix;
...
FUNCTION LOCAL_PROTO void add_x_constraint(SPGLOBPTR2 fix31 orus, fix31 pix)
/*
 ...
 */
{
	...
	/*
	 * Shuffle entries in sp_globals.processor.type1.Xorus and 
	 * sp_globals.processor.type1.Xpix up one position starting at
	 * entry point 
	 */
	for (j = (int)sp_globals.processor.type1.no_x_breaks; j > i; j--) {
...
FUNCTION LOCAL_PROTO void align_hstem(SPGLOBPTR2 fix31 y1, fix31 y2,
								 fix31 STACKFAR *pcy_pix_bottom, fix31 STACKFAR *pcy_pix_top)
/*
 ...
 */
{
	fix31           cy_bottom;	/* bottom Position of stroke in outline units */
	fix31           cy_top;	/* top Position of stroke in outline units */
	fix31           strkwt_pix;	/* Thickness of stroke in device units */
	fix31           cy_pix_bottom;	/* bottom Position of  stroke in device units */
	fix31           cy_pix_top;	/* top Position  of stroke in device units */
	fix31           cy_temp_bottom;	/* Position of bottom of stroke in device units */
	fix31           cy_temp_top;	/* Position of top of stroke in device units */
	int             i;
	fix31           temp_bottom;	/* temp variable for calculation of bottom edge */
	...
	if (cy_bottom <= 0) {	/* bottom edge below or at baseline */
		...
		cy_temp_bottom = (cy_bottom * sp_globals.processor.type1.y_pix_per_oru_i) 
						+ ((cy_bottom >> 4) * sp_globals.processor.type1.fudge_y);
		cy_pix_bottom = (cy_temp_bottom + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
		 ...
		 * edge 
		 */
		cy_temp_top = (cy_top * sp_globals.processor.type1.y_pix_per_oru_i) 
						+ ((cy_top >> 4) * sp_globals.processor.type1.fudge_y);
		cy_pix_top = (cy_temp_top + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;	/* Round to nearest
								 								* pixel boundary */
	 ...
	 */
	if (cy_pix_top == 0) {
		temp_bottom = (cy_bottom * sp_globals.processor.type1.y_pix_per_oru_i) 
						+ ((cy_bottom >> 4) * sp_globals.processor.type1.fudge_y);
		temp_bottom = (temp_bottom + sp_globals.processor.type1.mk_rnd) 
						& sp_globals.processor.type1.mk_fix;
		if (temp_bottom == 0) {
			cy_pix_bottom = 0;
...
FUNCTION LOCAL_PROTO void align_hstem(SPGLOBPTR2 fix31 y1, fix31 y2, fix31 STACKFAR *pcy_pix)
/*
 * Aligns a horizontal stem to whole pixel boundaries Returns pixel
 * coordinate of centerline of aligned stem Uses sp_globals.processor.type1.top_zones[] and
 * sp_globals.processor.type1.bottom_zones[] arrays for blue value alignment 
 */
{
	fix31           cy;	/* Position of centerline of stroke in outline units */
	fix31           strkwt_pix;	/* Thickness of stroke in device units */
	fix31           hstrkwt_pix;	/* Half thickness of stroke in device units */
	fix31           cy_pix;	/* Position of centerline of stroke in device units */
	fix31           cy_temp;/* Position of centerline of stroke in device units */
	int             i;
	 ...
	 * accuracy 
	 */
	cy_temp = (cy * sp_globals.processor.type1.y_pix_per_oru_i) 
				+ ((cy >> 4) * sp_globals.processor.type1.fudge_y);
	if (strkwt_pix & sp_globals.processor.type1.mk_onepix) {	/* Odd number of pixels? */
		cy_pix = (cy_temp & sp_globals.processor.type1.mk_fix) 
					+ sp_globals.processor.type1.mk_rnd;	/* Round to nearest
							 								* pixel center */
	} else {
		cy_pix = (cy_temp + sp_globals.processor.type1.mk_rnd) 
					& sp_globals.processor.type1.mk_fix;	/* Round to nearest
							 								* pixel boundary */
	}
...
FUNCTION LOCAL_PROTO fix31 hstemweight(SPGLOBPTR2 fix31 dy)
{
	int             i;
	fix31           dy_pix;
	...
	/* Round to nearest whole number of pixels */
	dy_pix = ((dy * sp_globals.processor.type1.y_pix_per_oru_i) 
				+ ((dy >> 4) * sp_globals.processor.type1.fudge_y1) 
				+ sp_globals.processor.type1.mk_rnd) & sp_globals.processor.type1.mk_fix;
...
FUNCTION LOCAL_PROTO void add_y_constraint(SPGLOBPTR2 fix31 orus, fix31 pix)
/*
 ...
 */
{
	...
	/*
	 * Shuffle entries in sp_globals.processor.type1.Yorus and 
	 * sp_globals.processor.type1.Ypix up one position starting at this
	 * point 
	 */
	for (j = (int)sp_globals.processor.type1.no_y_breaks; j > i; j--) {
...
FUNCTION void do_trans_a(SPGLOBPTR2 fix31 STACKFAR *pX, fix31 STACKFAR *pY)
/*
...
#if DEBUG
	btsbool         x_print_flag = FALSE;
	btsbool         y_print_flag = FALSE;
#endif
	...
	if (sp_globals.processor.type1.non_linear_X) {	/* Non-linear transformation of X
		...
		for (i = 0; i < sp_globals.processor.type1.no_x_breaks; i++) {
			if (X < sp_globals.processor.type1.Xorus[i]) {
				xtrans = X * (ufix32) sp_globals.processor.type1.Xmult[i] 
							+ sp_globals.processor.type1.Xoffset[i];
				goto L1;
			}
		}
		xtrans = X * (ufix32) sp_globals.processor.type1.Xmult[sp_globals.processor.type1.no_x_breaks] 
					+ sp_globals.processor.type1.Xoffset[sp_globals.processor.type1.no_x_breaks];
L1:
		;
	}
	if (sp_globals.processor.type1.non_linear_Y) {	/* Non-linear transformation of Y
		...
		for (i = 0; i < sp_globals.processor.type1.no_y_breaks; i++) {
			if (Y < sp_globals.processor.type1.Yorus[i]) {
				ytrans = Y * (ufix32) sp_globals.processor.type1.Ymult[i] 
							+ sp_globals.processor.type1.Yoffset[i];
				goto L2;
			}
		}
		ytrans = Y * (ufix32) sp_globals.processor.type1.Ymult[sp_globals.processor.type1.no_y_breaks] 
					+ sp_globals.processor.type1.Yoffset[sp_globals.processor.type1.no_y_breaks];
L2:
		;
	}
#if DEBUG
	...
	default:		/* X is a linear of x and y */
		*pX = X * (sp_globals.processor.type1.local_matrix_i[0] >> 4) 
				+ Y * (sp_globals.processor.type1.local_matrix_i[2] >> 4) 
				+ sp_globals.processor.type1.local_matrix_i[4];
		break;
	}
	...
	default:		/* Y is a linear of x and y */
		*pY = X * (sp_globals.processor.type1.local_matrix_i[1] >> 4) 
				+ Y * (sp_globals.processor.type1.local_matrix_i[3] >> 4) 
				+ sp_globals.processor.type1.local_matrix_i[5];
		break;
	}
}


FUNCTION LOCAL_PROTO void update_x_trans(SPGLOBPTR1)
/*
 ...
 */
{
	...
	if (sp_globals.processor.type1.no_x_breaks == 0) {
...
#endif
		sp_globals.processor.type1.Xmult[0] = sp_globals.processor.type1.x_pix_per_oru_i;
		sp_globals.processor.type1.Xoffset[0] = 
				sp_globals.processor.type1.Xpix[0] 
				- (sp_globals.processor.type1.Xorus[0] * sp_globals.processor.type1.x_pix_per_oru_i 
				+ ((sp_globals.processor.type1.Xorus[0] >> 4) * sp_globals.processor.type1.fudge_x));

		for (i = 1; i < sp_globals.processor.type1.no_x_breaks; i++) {
			sp_globals.processor.type1.Xmult[i] = 
					((sp_globals.processor.type1.Xpix[i] 
					- sp_globals.processor.type1.Xpix[i - 1]) / (sp_globals.processor.type1.Xorus[i] 
					- sp_globals.processor.type1.Xorus[i - 1]));
			sp_globals.processor.type1.Xoffset[i] = 
					sp_globals.processor.type1.Xpix[i] 
					- (sp_globals.processor.type1.Xorus[i] * (ufix32) sp_globals.processor.type1.Xmult[i] 
					+ ((sp_globals.processor.type1.Xorus[i] >> 4) * sp_globals.processor.type1.fudge_x));
		}
		...
		sp_globals.processor.type1.Xoffset[i] = 
				...
				- (sp_globals.processor.type1.Xorus[sp_globals.processor.type1.no_x_breaks - 1] * sp_globals.processor.type1.x_pix_per_oru_i 
				+ ((sp_globals.processor.type1.Xorus[sp_globals.processor.type1.no_x_breaks - 1] >> 4) * sp_globals.processor.type1.fudge_x));
	}
...
FUNCTION LOCAL_PROTO void update_y_trans(SPGLOBPTR1)
/*
 * Updates the interpolation tables used for piecewise linear transformation
 ...
 */
{
	...
	if (sp_globals.processor.type1.no_y_breaks == 0) {
		...
		sp_globals.processor.type1.Ymult[0] = sp_globals.processor.type1.y_pix_per_oru_i;
		sp_globals.processor.type1.Yoffset[0] = 
				sp_globals.processor.type1.Ypix[0] 
				- (sp_globals.processor.type1.Yorus[0] * sp_globals.processor.type1.y_pix_per_oru_i 
				+ ((sp_globals.processor.type1.Yorus[0] >> 4) * sp_globals.processor.type1.fudge_y));

		for (i = 1; i < sp_globals.processor.type1.no_y_breaks; i++) {
			sp_globals.processor.type1.Ymult[i] =
					((sp_globals.processor.type1.Ypix[i] 
					- sp_globals.processor.type1.Ypix[i - 1]) / (sp_globals.processor.type1.Yorus[i] 
					- sp_globals.processor.type1.Yorus[i - 1]));
			sp_globals.processor.type1.Yoffset[i] = 
					sp_globals.processor.type1.Ypix[i] 
					- (sp_globals.processor.type1.Yorus[i] * (ufix32) sp_globals.processor.type1.Ymult[i] 
					+ ((sp_globals.processor.type1.Yorus[i] >> 4) * sp_globals.processor.type1.fudge_y));
		}
		...
		sp_globals.processor.type1.Yoffset[i] = 
				...
				- (sp_globals.processor.type1.Yorus[sp_globals.processor.type1.no_y_breaks - 1] * sp_globals.processor.type1.y_pix_per_oru_i 
				+ ((sp_globals.processor.type1.Yorus[sp_globals.processor.type1.no_y_breaks - 1] >> 4) * sp_globals.processor.type1.fudge_y));
	}
...
FUNCTION LOCAL_PROTO void print_x_trans(void)
/*
 ...
 */
{
	...
	for (i = 0; i < sp_globals.processor.type1.no_x_breaks; i++) {
		...
		printf("%10.1f %12.1f %12.5f %12.5f %12.5f\n",
		       (real) sp_globals.processor.type1.Xorus[i] * 0.0625,
			   (real) sp_globals.processor.type1.Xpix[i] / (real) sp_globals.processor.type1.mk_onepix,
			   (real) sp_globals.processor.type1.Xmult[i] / (real) sp_globals.processor.type1.tr_onepix,
			   (real) sp_globals.processor.type1.Xoffset[i] / (real) sp_globals.processor.type1.mk_onepix,
			   (real) q1 / (real) sp_globals.processor.type1.mk_onepix);
	}
	printf("                        %12.5f %12.5f\n",
	       (real) sp_globals.processor.type1.Xmult[sp_globals.processor.type1.no_x_breaks] / (real) sp_globals.processor.type1.tr_onepix,
		   (real) sp_globals.processor.type1.Xoffset[sp_globals.processor.type1.no_x_breaks] / (real) sp_globals.processor.type1.mk_onepix);
	return;
}
#endif
...
FUNCTION LOCAL_PROTO void print_y_trans(void)
/*
 ...
 */
{
	...
	for (i = 0; i < sp_globals.processor.type1.no_y_breaks; i++) {
		...
		printf("%10.1f %12.1f %12.5f %12.5f %12.5f\n",
			   ...
			   (real) sp_globals.processor.type1.Ypix[i] / (real) sp_globals.processor.type1.mk_onepix,
			   (real) sp_globals.processor.type1.Ymult[i] / (real) sp_globals.processor.type1.tr_onepix,
			   (real) sp_globals.processor.type1.Yoffset[i] / (real) sp_globals.processor.type1.mk_onepix,
			   (real) q2 / (real) sp_globals.processor.type1.mk_onepix);
	}
	printf("                        %12.5f %12.5f\n",
	       (real) sp_globals.processor.type1.Ymult[sp_globals.processor.type1.no_y_breaks] / (real) sp_globals.processor.type1.tr_onepix,
		   (real) sp_globals.processor.type1.Yoffset[sp_globals.processor.type1.no_y_breaks] / (real) sp_globals.processor.type1.mk_onepix);
	return;
}
#endif
...
FUNCTION fix15 set_shift_const(SPGLOBPTR1)
{
