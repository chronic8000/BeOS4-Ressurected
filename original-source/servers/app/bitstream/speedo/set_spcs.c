...
*                                                                                    *
*     $Header: //bliss/user/archive/rcs/speedo/set_spcs.c,v 33.0 97/03/17 17:47:41 shawn Release $                                                                       *
*                                                                                    *
*     $Log:	set_spcs.c,v $
 * Revision 33.0  97/03/17  17:47:41  shawn
 * TDIS Version 6.00
 * 
 * Revision 32.5  97/01/14  17:45:30  shawn
 * Apply casts to avoid compiler warnings.
 * 
 * Revision 32.4  96/07/02  12:02:49  shawn
 * Changed boolean to btsbool.
 * 
 * Revision 32.3  96/04/11  16:39:36  shawn
 * Moved routines set_bitmap_device() and set_outline_device() to the module
 *      out_util.c for use by all font technologies.
 * 
 * Revision 32.2  96/03/28  11:40:21  shawn
 * Added support for the Gray Writer output module.
 * 
 * Revision 32.1  95/11/17  10:11:43  shawn
 * Changed "FUNCTION static" declarations to "FUNCTION LOCAL_PROTO"
 * 
 * Revision 32.0  95/02/15  16:39:01  shawn
 * Release
 * 
 * Revision 31.2  95/01/12  11:31:12  roberte
 * Updated Copyright header.
 * 
 * Revision 31.1  95/01/04  16:41:38  roberte
 * Release
 * 
 * Revision 30.3  94/12/21  15:01:19  shawn
 * Conversion of all function prototypes and declaration to true ANSI type. New macros for REENTRANT_ALLOC.
 * 
 * Revision 30.2  94/11/22  12:37:04  roberte
 * For setting sp_globals.processor.speedo.kern tkorg and pkorg,
 * use a temporary pointer and more explicit instructions. This done
 * only to avoid INTERNAL COMPILER ERROR on MS C/C++ compiler when
 * DYNAMIC model is used. Code is functionally equivalent to previous code.
 * 
 * Revision 30.1  94/10/24  10:23:05  roberte
 * Added conditional #if PROC_SPEEDO so processor may be excluded in 4-in-1 builds.
 * 
 * Revision 30.0  94/05/04  09:40:51  roberte
 * Release
 * 
 * Revision 29.2  94/04/08  14:40:25  roberte
 * Release
 * 
 * Revision 29.0  94/04/08  11:46:03  roberte
 * Release
 * 
 * Revision 28.51  94/04/01  15:58:00  roberte
 * Moved some static function prototypes from speedo.h to here.
 * 
 * Revision 28.50  94/02/10  13:23:39  roberte
 *  Upgrade for MUTABLE fonts.
 * 
 * Revision 28.49  94/01/05  12:24:52  roberte
 * Changed font signature checking to allow D1.0 NULL NULL or D2.0 1 NULL.
 * To support new Mfonts and newly compiled (smaller) fonts.
 * 
 * Revision 28.48  93/10/18  10:24:44  roberte
 * Set sp_globals.normal to FALSE #if INCL_WHITE if output_mode == MODE_WHITE only.
 * 
 * Revision 28.47  93/08/31  09:39:23  roberte
 * Release
 * 
 * Revision 1.3  93/08/16  14:31:20  roberte
 * Added INCL_QUICK flag in MULTIDEV conditional declaration of set_bitmap_device()
 * 
 * Revision 1.2  93/07/13  17:46:42  ruey
 * add MODE_QUICK
 * 
 * Revision 1.1  93/07/13  16:17:04  tony
 * Initial revision
 * 
*       Revision 28.40  93/06/10  09:24:02  roberte
*       In sp_setup_constants(), initialize "num" to (ufix31)-1, and test
*       in for loop for (pxival > (fix31)num). Avoids problem of not initializing
*       xx and yy when pixval and num both equal zero.
*       
*       Revision 28.39  93/05/04  10:23:02  roberte
*       sp_setup_tcb now a public function, and may it ever remain so!
*       
*       Revision 28.38  93/04/23  17:38:44  roberte
*       Moved #define SET_SPCS from set_spcs.c to nsample.c and test.c
*       
...
#ifdef RCSSTATUS
static char rcsid[] = "$Header: //bliss/user/archive/rcs/speedo/set_spcs.c,v 33.0 97/03/17 17:47:41 shawn Release $";
#endif
...
LOCAL_PROTO fix15 sp_setup_mult(SPGLOBPTR2 fix31 input_mult);
LOCAL_PROTO fix31 sp_setup_offset(SPGLOBPTR2 fix31 input_offset);



FUNCTION btsbool sp_set_specs(SPGLOBPTR2 specs_t STACKFAR *specsarg)
/* 
...
ufix16	signature;		/* two byte signature following D?.0\r\n */
...
signature = sp_read_word_u(PARAMS2 sp_globals.processor.speedo.font_org + FH_FMVER + 6);
if ( !((signature == 0x0000) || (signature == 0x0100)) )
    {
    sp_report_error(PARAMS2 4);           /* Font format error */
    return FALSE;
    }
...
#if INCL_METRICS
{
ufix8 FONTFAR *tmpPtr;
/* use of tmpPtr avoids internal compiler error on MS compiler with our DYNAMIC_ALLOC model */
tmpPtr = sp_globals.processor.speedo.font_org;
tmpPtr += sp_read_long(PARAMS2 sp_globals.processor.speedo.hdr2_org + FH_OFFTK);
sp_globals.processor.speedo.kern.tkorg = tmpPtr;
tmpPtr = sp_globals.processor.speedo.font_org;
tmpPtr += sp_read_long(PARAMS2 sp_globals.processor.speedo.hdr2_org + FH_OFFPK);
sp_globals.processor.speedo.kern.pkorg = tmpPtr;
...
#if INCL_MUTABLE
sp_globals.processor.speedo.nIntTerms = 0;               /* Turn off all interpolation */
#endif
...
FUNCTION btsbool sp_setup_consts(SPGLOBPTR2 fix15 xmin, fix15 xmax, fix15 ymin, fix15 ymax)
/*
...
FUNCTION void sp_setup_tcb(SPGLOBPTR2 tcb_t GLOBALFAR *ptcb)
/* 
...
FUNCTION LOCAL_PROTO fix15 sp_setup_mult(SPGLOBPTR2 fix31 input_mult)
/*
...
FUNCTION LOCAL_PROTO fix31 sp_setup_offset(SPGLOBPTR2 fix31 input_offset)
/*
...
FUNCTION void sp_type_tcb(SPGLOBPTR2 tcb_t GLOBALFAR *ptcb)
{
fix15   x_trans_type;
fix15   y_trans_type;
fix15   xx_mult;
...
#if INCL_WHITE
if (sp_globals.output_mode == MODE_WHITE)
...
FUNCTION fix31 sp_read_long(SPGLOBPTR2 ufix8 FONTFAR *pointer)
/*
...
FUNCTION fix15 sp_read_word_u(SPGLOBPTR2 ufix8 FONTFAR *pointer)
/*
