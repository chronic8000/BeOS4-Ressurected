...
extern RenderFuncs recordingRenderFuncs;
...
{
	while (vCount--) {
		vertices->h = floor(vertices->h + 0.5);
		vertices->v = floor(vertices->v + 0.5);
		vertices++;
	};
};
...
void grRenderXform_ScaleRound(RenderContext *context, fpoint *vertices, int32 vCount)
{
	float scale = context->totalEffective_Scale;
	while (vCount--) {
		vertices->h = floor(vertices->h*scale + 0.5);
		vertices->v = floor(vertices->v*scale + 0.5);
		vertices++;
	};
};
...
RenderState::RenderState()
{
	...
	localPictureClip = NULL;

	globalEffective_Scale = 1.0;
	localScale = 1.0;

	globalEffective_Origin.h = globalEffective_Origin.v = 0.0;
	localOrigin.h = localOrigin.v = 0.0;
};
...
RenderState::~RenderState()
{
	...
	if (localPictureClip) localPictureClip->ServerUnlock();
};
...
RenderState& RenderState::operator=(const RenderState &original)
{
	...
	localPictureClipLastRasterBounds = original.localPictureClipLastRasterBounds;
	localPictureClipOffset = original.localPictureClipOffset;	
	localPictureClipInverted = original.localPictureClipInverted;
	if (original.localPictureClip) {
		localPictureClip = new SPicture;
		localPictureClip->Copy(original.localPictureClip);
	} else
		localPictureClip = 0;
...
void grPickRenderFuncs(RenderContext *context)
{
	...
	if (context->record) {
		context->renderFuncs = &recordingRenderFuncs;
	} else if (context->totalEffective_PenSize > 1.0) {
...
void grUpdatePictureClipStackBounds(RenderContext *context)
{
	rect r;
	context->buriedPictureClip = false;
	...
	while (state) {
		if (state->localPictureClip) {
			if (empty_rect(state->localPictureClipLastRasterBounds)) {
				r.top = r.left = MAX_OUT;
				r.bottom = r.right = -MAX_OUT;
			} else {
				if (!context->buriedPictureClip)
					r = state->localPictureClipLastRasterBounds;
				else
					sect_rect(r,state->localPictureClipLastRasterBounds,&r);
			};
			context->buriedPictureClip = true;
		};
		state = state->next;
	};
	
	context->pictureClipStackRasterBounds = r;
};
	...
	if (context->portLocked) {
		...
		while (port) {	
			if (context->totalEffective_Clip) {
				offset_region(context->totalEffective_Clip, port->origin.h, port->origin.v);
			...
			port->phase_x = ((int32)(-port->origin.h)) & 0x7;
			port->phase_y = ((int32)(-port->origin.v)) & 0x7;
			if (count) {
				tmpRect = port->drawingClip->Bounds();
				offset_rect(&tmpRect, -port->origin.h, -port->origin.v);
				union_rect(&tmpRect, &ueberPort->drawingBounds, &ueberPort->drawingBounds);
			} else {
				ueberPort->drawingBounds = port->drawingClip->Bounds();
				offset_rect(&ueberPort->drawingBounds, -port->origin.h, -port->origin.v);
			}
...
void grPictureToLocalClip(RenderContext *context, rect bounds)
{
	...
	fpoint origin;
	origin.h =
		context->totalEffective_Origin.h +
		context->localPictureClipOffset.h * scale;
	origin.v =
		context->totalEffective_Origin.v +
		context->localPictureClipOffset.v * scale;

	context->localPictureClipLastRasterBounds = bounds;

	context->localPictureClip->Rasterize(
		context, context->localPictureClipInverted, (context->stateFlags & RENDER_ROUND_COORDS),
		&context->localPictureClipLastRasterBounds, origin, scale, context->localEffective_Clip);
};

/*	This will walk back through the state stack and re-rasterize all clipping pictures in
	order to build a new clipping region in globalEffective_Clip.  It's not as bad as it
	seems, because the pictures cache rasterized versions of themselves.  And it won't
	happen very often that this needs to be invoked. */
void grRasterizePictureClipStack(RenderContext *context, rect bounds)
{
	BArray<RenderState*> stateStack;
	RenderState *state = context->stateStack;
	while (state) {
		stateStack.AddItem(state);
		state = state->next;
	};
	...
	r.top = r.left = MAX_OUT;
	r.bottom = r.right = -MAX_OUT;
	for (int32 i=stateStack.CountItems()-1;i>=0;i--) {
		state = stateStack[i];
		if (state->globalEffective_Clip) {
			if (building)
				copy_region(building,state->globalEffective_Clip);
			else {
				kill_region(state->globalEffective_Clip);
				state->globalEffective_Clip = NULL;
			};
		} else {
			if (building) {
				state->globalEffective_Clip = newregion();
				copy_region(building,state->globalEffective_Clip);
			};
		};
		scale = state->globalEffective_Scale;
		scale *= state->localScale;
		origin = state->globalEffective_Origin;
		origin.h += state->localOrigin.h * scale;
		origin.v += state->localOrigin.v * scale;
		hasClipping = false;
		if (state->localPictureClip) {
			origin.h += state->localPictureClipOffset.h * scale;
			origin.v += state->localPictureClipOffset.v * scale;
			state->localPictureClipLastRasterBounds = bounds;
			state->localPictureClip->Rasterize(
				context, state->localPictureClipInverted, (context->stateFlags & RENDER_ROUND_COORDS),
				&state->localPictureClipLastRasterBounds, origin, scale, local);
			if (empty_rect(state->localPictureClipLastRasterBounds)) {
				r.top = r.left = MAX_OUT;
				r.bottom = r.right = -MAX_OUT;
			} else {
			...
			context->buriedPictureClip = true;
			hasClipping = true;
		} else if (state->localRegionClip) {
			clear_region(local);
			const int32 count = state->localRegionClip->CountRects();
			const rect *data = state->localRegionClip->Rects();
			rect r;
			...
			if (scale != 1.0) {
				...
				copy_region(context->localRegionClip,local);
				h = floor(h); v = floor(v);
				if ((h != 0) || (v != 0)) offset_region(local,(int32)h,(int32)v);
			};
		...
		if (hasClipping) {
			if (!building) {
				building = newregion();
				copy_region(local,building);
			} else {
				and_region(local,building,tmpR);
				region *swap = building;
				building = tmpR;
				tmpR = swap;
			};
		};
	...
	context->globalEffective_Clip = building;
	context->pictureClipStackRasterBounds = r;
	kill_region(tmpR);
	kill_region(local);
};
...
void grComputeEffectiveLocalClip(RenderContext *context)
{
	if (context->localRegionClip) {
		...
		float h = context->totalEffective_Origin.h;
		float v = context->totalEffective_Origin.v;
		...
		if (context->portLocked) grGiveUpLocks(context);
		return;
	} else {
		if (context->localEffective_Clip)
...
void grClipToRegion(RenderContext *context, region *reg)
{
	...
	if (context->localPictureClip) {
		context->localPictureClip->ServerUnlock();
		context->localPictureClip = NULL;
	}
	...
	if (context->totalEffective_Scale == 1.0) {
		...
		if ((context->totalEffective_Origin.h != 0) ||
			(context->totalEffective_Origin.v != 0)) {
			offset_region(context->localEffective_Clip,
				(int32)floor(context->totalEffective_Origin.h+0.5),
				(int32)floor(context->totalEffective_Origin.v+0.5));
		}
		grComposeTotalClip(context);
		return;
	}
...
void grClipToPicture(RenderContext *context, SPicture *picture, fpoint offset, bool inverted)
{
	if (!picture) {
		grClearClip(context);
		return;
	...
	RDebugPrintf((context,"grClipToPicture(%08x,(%f,%f),%s)\n",picture,offset.h,offset.v,inverted?"true":"false"));
	...
	context->localPictureClip = picture;
	context->localPictureClipInverted = inverted;
	context->localPictureClipOffset = offset;
	context->localPictureClipLastRasterBounds.left =
	context->localPictureClipLastRasterBounds.top = MAX_OUT;
	context->localPictureClipLastRasterBounds.right =
	context->localPictureClipLastRasterBounds.bottom = -MAX_OUT;
	grComputeEffectiveLocalClip(context);
};
...
void grClearClip(RenderContext *context)
{
	...
	if (!context->localRegionClip && !context->localPictureClip) return;
...
void grMergeClip(RenderContext *context)
{
	...
	if (context->localPictureClip) {
		context->localPictureClip->ServerUnlock();
		if (!context->buriedPictureClip) {
			context->pictureClipStackRasterBounds = context->localPictureClipLastRasterBounds;
			context->buriedPictureClip = true;
		} else {
			rect r = context->pictureClipStackRasterBounds;
			if (empty_rect(context->localPictureClipLastRasterBounds)) {
				r.top = r.left = MAX_OUT;
				r.bottom = r.right = -MAX_OUT;
			} else
				sect_rect(r,context->localPictureClipLastRasterBounds,&r);
			context->pictureClipStackRasterBounds = r;
		};
	};
	context->localPictureClip = NULL;
...
void grMergeOrigin(RenderContext *context)
{
	context->globalEffective_Origin.h = context->totalEffective_Origin.h;
	context->globalEffective_Origin.v = context->totalEffective_Origin.v;
	context->localOrigin.h = 
	context->localOrigin.v = 0.0;
	context->localEffective_Origin.h = 
	context->localEffective_Origin.v = 0.0;
};

/* Sync's the context's current state with the picture being recorded */
void grSyncRecordingState(RenderContext *context)
{
#if USE_FONTS
	if (!context->fontContext.IsValid()) SFont::GetStandardFont(B_PLAIN_FONT, &context->fontContext);
#endif
	context->record->SyncState(context);
};
...
void grGetLocalClip(RenderContext *context, region *clip)
{
	...
	if (context->localEffective_Clip) {
		...
		offset_region(clip,
			(int32)floor(-context->totalEffective_Origin.h),
			(int32)floor(-context->totalEffective_Origin.v));
	} else 
		clear_region(clip);
};
...
void grGetUpdateRegion(RenderContext *context, region *update)
{
	...
	while (port) {
		offset_region(port->drawingClip,-port->origin.h,-port->origin.v);
...
void grSetPenLocation(RenderContext *context, fpoint penLoc)
{
	...
	if (context->record) context->record->PenLocationChanged();
};

void grMovePenLocation(RenderContext *context, fpoint delta)
{
	context->penLocation.h += delta.h;
	context->penLocation.v += delta.v;
	context->changedFlags |= LOCATION_CHANGED;
	if (context->record) {
		context->record->BeginOp(SPIC_MOVE_PEN);
		context->record->AddCoord(delta);
...
{
	if ((context->localOrigin.h == origin.h) &&
		(context->localOrigin.v == origin.v)) return;
	context->localOrigin = origin;
	context->localEffective_Origin.h = context->totalEffective_Scale * context->localOrigin.h;
	context->localEffective_Origin.v = context->totalEffective_Scale * context->localOrigin.v;
	context->totalEffective_Origin.h = context->globalEffective_Origin.h + context->localEffective_Origin.h;
	context->totalEffective_Origin.v = context->globalEffective_Origin.v + context->localEffective_Origin.v;
	grComputeEffectiveLocalClip(context);
...
void grInitRenderContext(RenderContext *render)
{
	...
	render->localOrigin.h = render->localOrigin.v = 0.0;
	render->globalEffective_Origin.h = render->globalEffective_Origin.v = 0.0;
	render->localEffective_Origin.h = render->localEffective_Origin.v = 0.0;
	render->totalEffective_Origin.h = render->totalEffective_Origin.v = 0.0;
	*((uint64*)&render->stipplePattern.data[0]) = 0xFFFFFFFFFFFFFFFFLL;
#if ROTATE_DISPLAY
	*((uint64*)&render->realStipplePattern.data[0]) = 0xFFFFFFFFFFFFFFFFLL;
#endif
	render->penSize = 1.0;
	render->penLocation.h = 0;
	render->penLocation.v = 0;
	...
	render->localPictureClip = NULL;
	render->totalEffective_Clip = NULL;
	render->localEffective_Clip = NULL;
	render->buriedPictureClip = false;
	render->pictureClipStackRasterBounds.left =
	render->pictureClipStackRasterBounds.top =
	render->localPictureClipLastRasterBounds.left =
	render->localPictureClipLastRasterBounds.top = MAX_OUT;
	render->pictureClipStackRasterBounds.right =
	render->pictureClipStackRasterBounds.bottom =
	render->localPictureClipLastRasterBounds.right =
	render->localPictureClipLastRasterBounds.bottom = -MAX_OUT;
...
void grDestroyRenderContext(RenderContext *render)
{
	...
	if (render->localPictureClip) render->localPictureClip->ServerUnlock();
	if (render->lineThickener) delete render->lineThickener;

	SPicture *pic = render->record;
	while (pic) {
		render->record = pic->OlderPicture();
		delete pic;
		pic = render->record;
	};
...
void grSaveState(RenderContext *context, RenderState *state)
{
	...
	state->localPictureClipLastRasterBounds = context->localPictureClipLastRasterBounds;
	state->localPictureClipOffset = context->localPictureClipOffset;
	state->localPictureClipInverted = context->localPictureClipInverted;
	
	SPicture *oldClip = state->localPictureClip;
	state->localPictureClip = context->localPictureClip;
	if (state->localPictureClip) state->localPictureClip->ServerLock();
	if (oldClip) oldClip->ServerUnlock();
...
void grRestoreState(RenderContext *context, RenderState *state)
{
	...
	context->localPictureClipLastRasterBounds = state->localPictureClipLastRasterBounds;
	context->localPictureClipOffset = state->localPictureClipOffset;
	context->localPictureClipInverted = state->localPictureClipInverted;
	
	region *oldRegionClip = context->localRegionClip;
	SPicture *oldPictureClip = context->localPictureClip;

	context->localPictureClip = state->localPictureClip;
	if (context->localPictureClip) context->localPictureClip->ServerLock();
	if (oldPictureClip) oldPictureClip->ServerUnlock();
	...
	if (oldRegionClip || oldPictureClip || context->localRegionClip)
		grComputeEffectiveLocalClip(context);
...
void grPushState(RenderContext *context)
{
	...
	if (context->record) {
		...
		context->record->RecordStatePush(context);
	};
...
void grPopState(RenderContext *context)
{
	SPicture *pic = NULL;
	if (context->record) {
		/*	Small hack.  Since pictures understand pushes and pops,
			we don't need to restore all the state explicitly.
			We set the picture to NULL here so that the state
			doesn't get explicitly set in the picture. */
		pic = context->record;
		context->record = NULL;
	};
	...
	if (state == NULL) {
		...
		if (context->localPictureClip) grUpdatePictureClipStackBounds(context);
		delete state;
	};

	if (pic) {
		context->record = pic;
		context->record->RecordStatePop(context);
		context->renderFuncs = &recordingRenderFuncs;
	};
...
void grStartRecording(RenderContext *context)
{
	RDebugPrintf((context,"grStartRecording\n"));
	context->record = new SPicture(context->record);
	grPickRenderFuncs(context);
//	grHoldPicks(context);
	context->changedFlags = 0xFFFFFFFF;
};

void grContinueRecording(RenderContext *context, SPicture *picture)
{
	picture->Usurp(context->record);
	context->record = picture;
//	grHoldPicks(context);
	context->changedFlags = 0xFFFFFFFF;
	grPickRenderFuncs(context);
};

SPicture *grStopRecording(RenderContext *context)
{
	SPicture *pic = context->record;
	if (pic) context->record = pic->OlderPicture();
		else context->record = NULL;
	grPickRenderFuncs(context);
//	grAllowPicks(context);
	RDebugPrintf((context,"grStopRecording\n"));
	return pic;
};
