							...
							int32 connectRule)
{
	fpoint p1,p2,tmp;
	status_t r=B_OK;
	int32 s,segs,firstLine,lastLine,lastOffset=0,nextOffset=0;
	arc a[3];
...
status_t SEdges::AddEdges(fpoint *vertices, int32 vCount)
{
	...
	for (;i<(vCount-1);i++) {
		p1 = vertices[i];
		p2 = vertices[i+1];
		if (floor(p1.v+0.5) != floor(p2.v+0.5)) goto gotOne;
	};
	p1 = vertices[vCount-1];
	p2 = vertices[0];
	if (floor(p1.v+0.5) == floor(p2.v+0.5)) return B_OK;
