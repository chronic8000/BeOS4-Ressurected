...
void grPictureToLocalClip(RenderContext *context, rect b1);
...
leaving:
	...
	if (context->localPictureClip || context->buriedPictureClip) {
		bool surface, buried;
		rect b1;
		goto midLoop;
		while ((!empty_rect(ueberPort->portBounds)) && (surface || buried)) {
			context->fUnlockRenderPort(context->portVendor,context->renderPort,context->ownerThread);
			if (surface) grPictureToLocalClip(context, b1);
			if (buried) grRasterizePictureClipStack(context, b1);
			grComposeTotalClip(context);
			ueberPort = context->fLockRenderPort(context->portVendor,&context->portToken,pid);
			context->renderPort = ueberPort;
		midLoop:
			b1 = ueberPort->portBounds;
			surface = (context->localPictureClip && !enclose(&context->localPictureClipLastRasterBounds,&b1));
			buried = (context->buriedPictureClip &&	!enclose(&context->pictureClipStackRasterBounds,&b1));
		}
	}
