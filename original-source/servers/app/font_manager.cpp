...
bool fc_write_dir_table(FILE *fp) {
	...
	for (i=0; i<dir_table_count; i++) {
		if (dir_table[i].dirname == 0L) {
			...
			if (fwrite(buffer, 1, 2, fp) != 2) return FALSE;
			if (fwrite(dir_table[i].dirname, 1, length, fp) != length) return FALSE;
...
bool fc_read_dir_table(FILE *fp) {
	...
	for (i=0; i<dir_table_count; i++) {
		...
		if (length != 0) {
			...
			if (fread(dir_table[i].dirname, 1, length, fp) != length) return FALSE;
...
bool fc_write_file_table(FILE *fp) {
	...
	for (i=0; i<file_table_count; i++) {
		...
		if (file_table[i].filename == 0L) {
			...
			fc_write16(buffer, length);
			if (fwrite(buffer, 1, 2, fp) != 2) return FALSE;
			if (fwrite(file_table[i].filename, 1, length, fp) != length) return FALSE;
			...
			if (length > 0) {
				...
				if (fwrite(file_table[i].block_bitmaps, 1, length, fp) != length) return FALSE;
			}
...
bool fc_read_file_table(FILE *fp) {
	...
	for (i=0; i<file_table_count; i++) {
		...
		if (length != 0) {
			...
			if (fread(file_table[i].filename, 1, length, fp) != length) return FALSE;
			...
			if (length > 0) {
				...
				if (fread(file_table[i].block_bitmaps, 1, length, fp) != length) goto exit_error;
			} else {
			...
			if (length < 0 || length > 0x7fffffff/sizeof(fc_char_range)) goto exit_error;
			if (length > 0) {
